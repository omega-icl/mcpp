...
mc_ffinv(py::module_& m)
{
  ...
  pyFIExceptions.def("ierr", &FI::Exceptions::ierr, "Error flag")
      .def("what", &FI::Exceptions::what, "Error description");

  py::enum_<FI::Options::NLINV>(pyFIOptions, "NLINV",
                                "Invertible nonlinear operations")
  ...
  py::class_<std::set<FI::Options::NLINV>>(m, "NLINVSet")
      ...
      .def(
          ...
          { s.insert(k); }, "Add an element to the set")
      .def(
          "remove",
          [](std::set<FI::Options::NLINV>& s, FI::Options::NLINV const& k)
          {
      ...
      .def(
          "discard",
          [](std::set<FI::Options::NLINV>& s, FI::Options::NLINV const& k)
          { s.erase(k); }, "Remove an element if present")
      .def("__contains__",
           [](std::set<FI::Options::NLINV>& s, FI::Options::NLINV const& k)
  ...
  pyFIOptions.def(py::init<>())
      .def(py::init<FI::Options const&>())
      .def(
          "reset", [](FI::Options& self) { self = FI::Options(); },
  ...
  pyFFInv
      ...
      .def(py::init<double const>(), py::arg("val"),
           "Constructor for real constant")
      .def(py::init<FI const&>(), "Copy constructor")

      // Modifiers
      .def(
          "indep", [](FI& self, int const ndx) { return self.indep(ndx); },
          py::arg("ndx"), "Initialize as independent variable with index 'ndx'")

      // Accessors
      .def_readwrite_static("options", &FI::options)
      .def(
          "inv", [](FI const& self, int const ndx) { return self.inv(ndx); },
          py::arg("ndx"),
          "Determine invertibility of variable with index 'ndx'")
      .def(
          "inv", [](FI const& self) { return self.inv(); },
          "Dictionary of invertibilities")
      .def("update", &FI::update, "Update invertibility")
  ...
  m.def("prod", [](unsigned n, std::vector<FI> const& x)
