...
mc_ffpoly(py::module_& m)
{
  ...
  py::class_<SP> pyFFPoly(m, "FFPoly", R"doc(
Sparse multivariate polynomial over DAG variables (FFVar).

FFPoly is the same sparse polynomial representation as SPoly, except that
the participating variables are FFVar nodes of a DAG (FFGraph) instead of
plain integer indices. The polynomial is stored as a sparse coefficient
map from monomials (FFMon objects) to float coefficients, ordered in
graded lexicographic order; it is exposed through the ``coefmon``
property, and the set of participating FFVar variables through ``setvar``.
...
``FFPoly.options.BASIS``; this choice affects arithmetic, display,
differentiation and evaluation. Use convert() to change the basis of a
given polynomial.

Polynomials are built from constants, DAG variables, monomials or
coefficient dictionaries via the constructors, and combined with the
overloaded arithmetic operators +, -, *, / (division only by a scalar or
by a polynomial that divides exactly, otherwise FFPoly.Exceptions is
raised) and ** with a nonnegative integer exponent. FFPoly objects also
arise from the sparse lifting interface, e.g. as the numerator and
denominator polynomials of SLiftVar.
...
>>> DAG = pymcpp.FFGraph()
>>> X = DAG.add_vars(2, "x")
>>> p = pymcpp.FFPoly(X[0]) * pymcpp.FFPoly(X[1]) + pymcpp.FFPoly(X[0], 2.)
>>> print(p)
2.00000e+00 [x0] + 1.00000e+00 [x0]·[x1]
>>> p.eval({X[0]: 1.0, X[1]: 3.0})
5.0
)doc");

  // --- Exceptions Nested Class ---
  py::class_<SP::Exceptions> pySPExceptions(pyFFPoly, "Exceptions", R"doc(
Exception information for errors raised during FFPoly arithmetic.

Instances carry an error code (ierr) and a human-readable description
(what).
)doc");
  py::enum_<SP::Exceptions::TYPE>(pySPExceptions, "TYPE",
                                  "Error codes for FFPoly exceptions")
...
Return the error code as an integer (see FFPoly.Exceptions.TYPE).
)doc")
...
Return a string describing the error.
  ...
  py::class_<SP::Options> pySPOptions(pyFFPoly, "Options", R"doc(
Options controlling sparse polynomial arithmetic and display.

Options take effect through the static member ``FFPoly.options``, shared
by all FFPoly objects.
)doc");
...
FFPoly.Options.BASIS_TYPE). Default is 0.
)doc")
...
Static options shared by all FFPoly objects (see FFPoly.Options).
)doc")
      // Constructors
      .def(py::init<>(), R"doc(
...
Parameters
----------
coefmon : dict of FFMon to float
...
Returns a dict mapping each monomial (FFMon) to its float coefficient. The
monomial keys are interpreted in the basis given by
``FFPoly.options.BASIS``.
)doc")
      .def_property_readonly("setvar",
                             py::overload_cast<>(&SP::setvar, py::const_),
                             R"doc(
Set of all participating DAG variables (FFVar).
)doc")
      // Methods
...
``FFPoly.options.BASIS`` and converted to `basis`; no conversion is
performed if the two coincide. Note that ``FFPoly.options.BASIS`` itself
is not modified.
...
----------
x : FFVar
    DAG variable to factor with respect to.

Returns
-------
factors : dict of int to FFPoly
...
----------
x : FFVar
    DAG variable to differentiate with respect to.
...
Parameters
----------
coefmon : list of tuple of (FFMon, float)
...
Parameters
----------
x : dict of FFVar to float
    Value of each variable, keyed by DAG variable. Every participating
...
FFPoly overload: square of a sparse polynomial over DAG variables,
computed in the basis given by FFPoly.options.BASIS.
)doc");
  m.def(
      "pow", [](SP const& s, unsigned n) { return mc::pow(s, n); }, R"doc(
FFPoly overload: n-th power of a sparse polynomial over DAG variables for
a nonnegative integer n, computed in the basis given by
FFPoly.options.BASIS.
)doc");
  m.def(
      "cheb", [](SP const& s, unsigned n) { return mc::cheb(s, n); }, R"doc(
FFPoly overload: Chebyshev polynomial of the first kind T_n applied to a
sparse polynomial over DAG variables, using the recurrence
T_n = 2*s*T_{n-1} - T_{n-2}.
)doc");
  m.def(
      "prod",
      ...
      R"doc(
FFPoly overload: product v[0]*...*v[n-1] of sparse polynomials over DAG
variables.
)doc");
}
