...
mc_spoly(py::module_& m)
{
  ...
  py::class_<SP> pySPoly(m, "SPoly");

  // --- Exceptions Nested Class ---
  py::class_<SP::Exceptions> pySPExceptions(pySPoly, "Exceptions");
  ...
  pySPExceptions.def("ierr", &SP::Exceptions::ierr, "Return error flag")
      .def("what", &SP::Exceptions::what, "Return error description");

  // --- Options Nested Struct ---
  py::class_<SP::Options> pySPOptions(pySPoly, "Options");
  ...
  pySPOptions.def(py::init<>())
      .def_readwrite("BASIS", &SP::Options::BASIS,
                     "Basis representation of sparse polynomial: 0-Monomials "
                     "[Default], 1-Chebyshev")
      .def_readwrite("REMZERO", &SP::Options::REMZERO,
                     "Whether to remove zeros entries from sparse polynomials "
                     "[Default: True]")
      .def_readwrite("DISPLEN", &SP::Options::DISPLEN,
                     "Number of digits in output stream [Default: 5]")
      .def_readwrite(
          "DISPLINE", &SP::Options::DISPLINE,
          "Whether to display polynomial on single line [Default: True]");

  // --- SPoly Main Class ---
  pySPoly
      // Static Options
      .def_readwrite_static("options", &SP::options,
                            "Static options of sparse polynomials")
      // Constructors
      .def(py::init<>(), "Default constructor")
      .def(py::init<double const>(), py::arg("coef"),
      ...
      .def(py::init<SP const&>(), "Copy constructor")
      // Accessors
      .def("maxord", &SP::maxord, "Maximal degree of monomial terms")
      .def("minord", py::overload_cast<>(&SP::minord, py::const_),
           "Minimal degree of monomial terms")
      .def("minord", py::overload_cast<KEY const&>(&SP::minord, py::const_),
           py::arg("x"), "Minimal degree of variable monomial terms")
      .def_property_readonly("nmon", &SP::nmon,
                             "Total number of monomial terms")
      .def("coef", &SP::coef, py::arg("mon") = SM(),
           "Get coefficient of monomial")
      .def("nvar", &SP::nvar, "Total number of participating variables")
      // Properties
      .def_property_readonly(
          "coefmon", py::overload_cast<>(&SP::mapmon, py::const_),
          "Dictionary of monomials and associated coefficients")
      .def_property_readonly("setvar",
                             py::overload_cast<>(&SP::setvar, py::const_),
                             "Set of participating variables")
      // Methods
      .def("swap", &SP::swap, py::arg("spoly"), "Swap contents")
      .def("clean", &SP::clean, py::arg("tol") = 0.,
           "Remove entries below tolerance")
      .def("convert", &SP::convert, py::arg("basis"),
           "Convert to desired basis from options.BASIS")
      .def("ismultiple", &SP::ismultiple, py::arg("spoly"),
           "Check if polynomial is multiple of 'spoly'")
      .def("factor", &SP::factor, py::arg("x"),
           "Factor polynomial with respect to variable 'x'")
      .def("diff", &SP::diff, py::arg("x"),
           "Differentiate polynomial with respect to variable 'x'")
      .def("var", &SP::var, py::arg("x"), "Set equal to variable x")
      .def_static(
          "display",
  ...
  m.def("prod", [](unsigned n, std::vector<SP> const& v)
