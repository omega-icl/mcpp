...
mc_ellimage(py::module& m)
{
  ...
  pyEllipsoid.def(py::init<>())
      ...
      .def_static(
          ...
          py::arg("box"), "construct an ellipsoid enclosing an interval vector")
      .def(py::init<ELL const&>(), "copy constructor")
      .def_readwrite_static("options", &ELL::options,
      ...
      .def(
          ...
          py::arg("box"), py::return_value_policy::reference_internal,
      ...
      .def("reset_aux", &ELL::reset_aux,
           py::return_value_policy::reference_internal,
           "clear cached decompositions")
      .def(
          "extend", [](ELL& self, py::handle Qi, double const& ci) -> ELL&
          { return self.extend(arma::rowvec(_to_arma_vec(Qi).t()), ci); },
          py::arg("Qi"), py::arg("ci") = 0.,
          py::return_value_policy::reference_internal,
          "append one dimension using lower-triangular row Qi and center entry "
          "ci")
      .def("O", &ELL::O,
           "return the centred ellipsoid with the same shape matrix")
      ...
      .def("Qij", py::overload_cast<unsigned, unsigned>(&ELL::Q, py::const_),
           py::arg("i"), py::arg("j"), "shape matrix coefficient")
      .def(
          "set_ci",
          ...
          py::return_value_policy::reference_internal, "set center coefficient")
      .def(
          "set_Qij",
          ...
          py::return_value_policy::reference_internal,
          "set symmetric shape matrix coefficient and clear cached "
          "decompositions")
      .def("trQ", &ELL::trQ, "trace of the shape matrix")
      .def("psdQ", &ELL::psdQ,
           "test whether the shape matrix is positive semidefinite")
      .def("rankQ", &ELL::rankQ,
           "rank of the shape matrix with current tolerances")
      .def(
          "eigQ",
          [](ELL& self)
          {
      ...
      .def(
          ...
          py::arg("complete") = false, "matrix square root of Q")
      .def(
          "svdQ",
          [](ELL& self)
          {
  ...
  pyEllipsoidOptions.def(py::init<>())
      .def(py::init<ELL::Options const&>())
      .def(
          "reset", [](ELL::Options& self) { self = ELL::Options(); },
          "reset options to defaults")
      .def_readwrite("PSDCHK", &ELL::Options::PSDCHK,
                     "whether to check positive semidefiniteness of shape "
                     "matrices [Default: false]")
      .def_readwrite(
          "PSDTOL", &ELL::Options::PSDTOL,
          "absolute tolerance for PSD checks [Default: 1e2*machprec()]")
      .def_readwrite(
          "RKTOLA", &ELL::Options::RKTOLA,
          "absolute rank/regularisation tolerance [Default: machprec()]")
      .def_readwrite(
          "RKTOLR", &ELL::Options::RKTOLR,
          "relative rank/regularisation tolerance [Default: machprec()]")
      .def_readwrite(
          "ROOTTOL", &ELL::Options::ROOTTOL,
          "absolute stopping tolerance for root finding [Default: 1e-10]")
      .def_readwrite("ROOTSECANT", &ELL::Options::ROOTSECANT,
                     "whether to use secant root finding before golden-section "
                     "fallback [Default: false]")
      .def_readwrite(
          "ROOTMAXIT", &ELL::Options::ROOTMAXIT,
          "maximum root-finding iterations; 0 means no maximum [Default: 0]");
  ...
  pyEllImg.def(py::init<>())
      ...
      .def(
          ...
          py::return_value_policy::reference_internal,
          "reset lifted image to the underlying ellipsoid")
      .def(
          "get",
  ...
  pyEllImgOptions.def(py::init<>())
      .def(py::init<EI::Options const&>())
      .def(
          "reset", [](EI::Options& self) { self = EI::Options(); },
          "reset options to defaults")
      .def_readwrite("PREALLOC", &EI::Options::PREALLOC,
                     "number of rows to preallocate in the lifted shape matrix "
                     "and center [Default: 0]")
      .def_readwrite("MINK_TOL", &EI::Options::MINK_TOL,
                     "tolerance in Minkowski sums [Default: 1e-10]")
      .def_readwrite("REMEZ_USE", &EI::Options::REMEZ_USE,
                     "whether to use degree-1 Remez linearisation for "
                     "univariate terms [Default: true]")
      .def_readwrite("REMEZ_MAXIT", &EI::Options::REMEZ_MAXIT,
                     "maximum Remez iterations [Default: 5]")
      .def_readwrite("REMEZ_TOL", &EI::Options::REMEZ_TOL,
                     "Remez stopping tolerance [Default: 1e-5]")
      .def_readwrite(
          "REMEZ_MIG", &EI::Options::REMEZ_MIG,
          "minimum interval diameter for invoking Remez [Default: 1e-10]")
      .def_readwrite("DCPROD_USE", &EI::Options::DCPROD_USE,
                     "whether to use DC decomposition to lift bilinear terms "
                     "[Default: false]");
  ...
  pyEllVar.def(py::init<>())
      .def(py::init<EV const&>(), "copy constructor")
      .def(py::init<double const&>(), py::arg("cst"),
           "constructor for constant scalar")
      .def(py::init<I const&>(), py::arg("range"),
           "constructor for interval/range")
      ...
      .def(py::init<EI&, unsigned const>(), py::arg("image"), py::arg("index"),
           py::keep_alive<1, 2>(), "constructor for image variable")
      .def(py::init<EI&, unsigned const, I const&>(), py::arg("image"),
           py::arg("index"), py::arg("range"), py::keep_alive<1, 2>(),
           "constructor for image variable with tailored range")
      .def(
      ...
      .def_property_readonly("range", &EV::range,
                             "interval/range enclosure of the variable")
      .def_property_readonly(
          "image", &EV::image, py::return_value_policy::reference_internal,
          "associated ellipsoidal image, or None for constants/ranges")
      .def_property_readonly(
          "index", &EV::index,
          "row index in the lifted image, or -1 for constants/ranges")
  ...
  m.def(
      "acos", [](EV const& x) { return mc::acos(x); }, py::keep_alive<0, 1>());
  m.def(
      "asin", [](EV const& x) { return mc::asin(x); }, py::keep_alive<0, 1>());
  m.def(
      "atan", [](EV const& x) { return mc::atan(x); }, py::keep_alive<0, 1>());
  m.def(
      "cosh", [](EV const& x) { return mc::cosh(x); }, py::keep_alive<0, 1>());
  m.def(
      "sinh", [](EV const& x) { return mc::sinh(x); }, py::keep_alive<0, 1>());
  m.def(
      "tanh", [](EV const& x) { return mc::tanh(x); }, py::keep_alive<0, 1>());
  m.def("erf", [](EV const& x) { return mc::erf(x); }, py::keep_alive<0, 1>());
  m.def(
      "erfc", [](EV const& x) { return mc::erfc(x); }, py::keep_alive<0, 1>());
  m.def(
  ...
  m.def(
      ...
      py::arg("ellipsoids"), py::arg("direction_or_directions"),
      "external approximation of a Minkowski sum of ellipsoids for one "
      "direction, or for an iterable of directions");
  m.def(
      "minksum_box",
      ...
      py::arg("tol") = 1e-10, py::arg("eps") = mc::machprec());
  m.def(
      "minksum_interval",
  ...
  m.def(
      ...
      { return mc::dist(E, _to_hp(hp)); }, py::arg("E"), py::arg("hp"));
  m.def(
