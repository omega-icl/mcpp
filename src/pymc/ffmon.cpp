...
py::enum_<FFMon::Exceptions::TYPE>( pyFFMonExceptions, "TYPE" )
 .value( "SUB",  FFMon::Exceptions::SUB,  "Subtraction of a monomial that is not a proper subset" )
 .value( "DIV",  FFMon::Exceptions::DIV,  "Division by a factor greater than the greatest common exponent" )
 .value( "CONV", FFMon::Exceptions::CONV, "Conversion with different variable indexing failed" )
 .export_values();

pyFFMonExceptions
 .def( "ierr", &FFMon::Exceptions::ierr, "Return error flag" )
...
// Accessors
 ...
 .def(
   "__getitem__", 
   []( FFMon const& self, KEY const& var ){ return self[ var ]; },
 ...
 .def( "__eq__",  []( FFMon const& Mon1, FFMon const& Mon2 ){ return   Mon1 == Mon2;  }, py::is_operator() )
 .def( "__ne__",  []( FFMon const& Mon1, FFMon const& Mon2 ){ return !(Mon1 == Mon2); }, py::is_operator() )

 // String representation
 .def( "__str__",  []( FFMon const& self ){ return self.display(FFPoly::options.BASIS); } )
 .def( "__repr__", []( FFMon const& self ){ return self.display(FFPoly::options.BASIS); } )
;
