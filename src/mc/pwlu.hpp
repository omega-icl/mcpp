...
The class mc::PWLU provides an implementation of piecewise linear univariate
...
operations and compositions with convex/concave monotonic terms, their
simplification or reduction, and their bounding and evaluation.
*/
...
////////////////////////////////////////////////////////////////////////
{
 ...
 public:
  ...
  static struct Options
  {
    ...
    double BKPTRTOL;
    //! @brief Method for breakpoint reduction - <=0: Standard library
    //! (default); >0: Tailored heap implementation over threshold
    int REDUCEMETH;
  ...
  PWLU() : _x0(0.), _y0(0.), _dx(), _dy() {}

  //! @brief Constructor of constant
  PWLU(double const& xL, double const& xU, double const& y)
      : _x0(xL), _y0(y), _dx({xU - xL}), _dy({0.})
  {
#ifdef MC__PWLU_CHECK
    if (xU <= xL) throw Exceptions(Exceptions::RANGE);
#endif
  }

  //! @brief Constructor of variable
  PWLU(double const& xL, double const& xU, size_t const N = 1)
      : _x0(xL),
        _y0(xL),
        _dx(N > 1 ? N : 1, N > 1 ? (xU - xL) / N : (xU - xL)),
        _dy(N > 1 ? N : 1, 1.)
  {
#ifdef MC__PWLU_CHECK
    if (xU < xL) throw Exceptions(Exceptions::RANGE);
  ...
  //! @brief Constructor of variable
  PWLU(double const& xL, std::vector<double> const& dx)
  ...
  //! @brief Constructor of variable
  PWLU(double const& xL, double const& xU, double const& yL, double const& yU)
  ...
  //! @brief Constructor of variable
  PWLU(double const& xL, double const& yL, std::vector<double> const& dx,
       std::vector<double> const& dy)
      : _x0(xL), _y0(yL), _dx(dx), _dy(dy)
  {
#ifdef MC__PWLU_CHECK
    if (_dx.size() != _dy.size()) throw Exceptions(Exceptions::SIZE);
    for (auto const& dxi : _dx)
      if (dxi <= 0.) throw Exceptions(Exceptions::RANGE);
#endif
  }
  ...
  PWLU(PWLU const& var) : _x0(var._x0), _y0(var._y0), _dx(var._dx), _dy(var._dy)
  {
  }
  ...
  //! @brief Set constant estimator
  PWLU&
  set(double const& xL, double const& xU, double const& y)
  {
#ifdef MC__PWLU_CHECK
    if (xU < xL) throw Exceptions(Exceptions::RANGE);
#endif
  ...
  PWLU&
  set(double const& xL, double const& xU, size_t const N = 1)
  {
#ifdef MC__PWLU_CHECK
    if (xU < xL) throw Exceptions(Exceptions::RANGE);
#endif
  ...
  //! @brief Set variable estimator
  PWLU&
  set(double const& xL, std::vector<double> const& dx)
  {
#ifdef MC__PWLU_CHECK
  ...
  //! @brief Set variable estimator
  PWLU&
  set(double const& xL, double const& xU, double const& yL, double const& yU)
  {
#ifdef MC__PWLU_CHECK
    if (xU < xL) throw Exceptions(Exceptions::RANGE);
#endif
  ...
  //! @brief Set variable estimator
  PWLU&
  set(double const& xL, double const& yL, std::vector<double> const& dx,
      std::vector<double> const& dy)
  {
#ifdef MC__PWLU_CHECK
    if (dx.size() != dy.size()) throw Exceptions(Exceptions::SIZE);
    for (auto const& dxi : _dx)
      if (dxi <= 0.) throw Exceptions(Exceptions::RANGE);
#endif
    _x0 = xL;
  ...
  PWLU&
  insert(std::vector<double> const& x)
  {
    _dx.reserve(_dx.size() + x.size());
    _dy.reserve(_dy.size() + x.size());
    for (auto const& xi : x) insert(xi);
    return *this;
  }

  //! @brief Insert breakpoint
  PWLU&
  insert(double const& x)
  {
#ifdef MC__PWLU_CHECK
    if (_dx.empty()) throw Exceptions(Exceptions::RANGE);
    if (_dx.size() != _dy.size()) throw Exceptions(Exceptions::SIZE);
#endif
    if (isequal(x, _x0, options.BKPTATOL, options.BKPTRTOL))
      return *this;  // no insertion
    double xi = _x0;
    for (auto idx = _dx.begin(), idy = _dy.begin(); idx != _dx.end();
         ++idx, ++idy)
    {
  ...
  val(double const& x) const
  {
    ...
    for (auto idx = _dx.cbegin(), idy = _dy.cbegin(); idx != _dx.cend();
         ++idx, ++idy)
    {
      if (xi + *idx < x)
      {
      ...
      // std::cout << "x: " << x << "  xi: " << xi << "  yi: " << yi << "  y: "
      // << yi + (x-xi) * *idy << std::endl;
      return yi + (x - xi) * *idy;
    }
    // std::cout << "x: " << x << "  xi: " << xi << "  yi: " << yi << std::endl;
    if (isequal(xi, x, options.BKPTATOL, options.BKPTRTOL)) return yi;
    throw Exceptions(Exceptions::EXTRAPOL);
  }

  //! @brief Evaluate estimator upper bound at point
  double
  l(double const& x) const
  {
    return val(x);
  }
          ...
          size_t const dispnum = options.DISPNUM) const
  {
    ...
    os << "\nx0: " << _x0 << "  y0: " << _y0;
    os << "\ndx:";
    for (auto idx = _dx.cbegin(); idx != _dx.cend(); ++idx) os << " " << *idx;
    os << "\ndy:";
    for (auto idy = _dy.cbegin(); idy != _dy.cend(); ++idy) os << " " << *idy;
    return os << std::endl;
    /*
          double xi = _x0, yi = _y0;
          os << "{" << std::scientific << std::setprecision(dispnum) <<
       std::right
             << std::setw(dispnum+8) << xi << ":" << std::setw(dispnum+8) << yi;
          for( auto idx=_dx.cbegin(), idy=_dy.cbegin(); idx!=_dx.cend(); ++idx,
       ++idy ) os << ", " << std::setw(dispnum+8) << (xi += *idx) << ":" <<
       std::setw(dispnum+8) << (yi += *idx * *idy); return os << " }";
    */
  }
...
#endif
    double xi = _x0;
    for (auto idx = _dx.cbegin(); idx != _dx.cend(); ++idx) xi += *idx;
    return xi;
  }
  ...
  {
#ifdef MC__PWLU_CHECK
    if (_dx.empty() || var._dx.empty()) throw Exceptions(Exceptions::RANGE);
    if (_dx.size() != _dy.size() || var._dx.size() != var._dy.size())
      throw Exceptions(Exceptions::SIZE);
#endif

    if (!isequal(_x0, var._x0)) throw Exceptions(Exceptions::RANGE);
    ...
    {
      // std::cout << std::scientific << std::setprecision(14) << "*idx1=" <<
      // *idx1 << "  *idx2-d2=" << *idx2-d2 << std::endl;
    ...
    if (idx1 != _dx.end())
    {  //&& isequal( *idx1, 0., options.BKPTATOL, options.BKPTRTOL ) ){
#ifdef MC__PWLU_CHECK
    ...
    {  //|| idx2 != var._dx.cend() ){
#ifdef MC__PWLU_CHECK
      if (idx1 != _dx.end()) std::cout << " dx = " << *idx1 << std::endl;
          ...
          bool const inc = 0, double const& xmid = 0.0)
  {
    ...
    if (cvx == 0 || cvx == 1) return _compose(f, df, under, cvx, inc);

    // univariate is either concavoconvex or convexoconcave with inflection at
    // xmid
    else if (cvx == 2 || cvx == 3)
    {
      ...
      {
        double const z = f(x), t = fmid + dfmid * (x - xmid);
        return z > t ? z : t;
      };
      auto const& dfcv = [&](const double& x)
      {
        double const z = f(x), t = fmid + dfmid * (x - xmid);
        return z > t ? df(x) : dfmid;
      };
      auto const& fcc = [&](const double& x)
      {
        double const z = f(x) - fmid, t = dfmid * (x - xmid);
        return z < t ? z - t : 0;
      };
      auto const& dfcc = [&](const double& x)
      {
        double const z = f(x) - fmid, t = dfmid * (x - xmid);
        return z < t ? df(x) - dfmid : 0;
      };
      PWLU copy(*this);
      if (cvx == 2)
        return this->_compose(fcv, dfcv, under, 1, inc) +=
               copy._compose(fcc, dfcc, under, 0, 1);
      else
        return this->_compose(fcv, dfcv, under, 1, inc) +=
               copy._compose(fcc, dfcc, under, 0, 0);
    }
    ...
    else if (cvx == 4 || cvx == 5)
    {
      ...
      PWLU copy(*this);
      if (cvx == 4)
        return this->_compose(fr, dfr, under, 0, 0) +=
               copy._compose(fl, dfl, under, 0, 1);
      else
        return this->_compose(fr, dfr, under, 1, 1) +=
               copy._compose(fl, dfl, under, 1, 0);
    }
  ...
  clean(bool const under)
  {
    ...
    // display();
    //  Initial stage - Merge leading negligible segments, if any, with
    //  subsequent ones
    ...
    for (;;)
    {
      if (isequal(*idx, 0., options.BKPTATOL, options.BKPTRTOL))
      {
        ...
        // std::cout << "**PWLU::clean dx = " << dx << "  dy = " << dy <<
        // std::endl;
        idx = _dx.erase(idx);
        idy = _dy.erase(idy);
  ...
  reduce(bool const under, size_t const nseg)
  {
    ...
    // display();
    clean(under);
    if (!nseg || _dx.size() <= nseg || _dx.size() < 3) return *this;
 ...
 private:
  static void
  _average(double& dx, double& s, double const& dx0, double const& s0)
  {
    s *= dx;
    s += (s0 * dx0);
    dx += dx0;
  ...
  static void _loss(bool const isUnder, double const& slpDiff1,
                    double const& slpDiff2, double const& incA0,
                    double const& slpA0, double const& incA1,
                    double const& slpA1, double const& incA2,
                    double const& slpA2,
                    std::tuple<double, int, double, double>& loss);

  static double
  _area(double const& inc1, double const& inc2, double const& slpdiff)
  {
    return inc1 * inc2 * slpdiff;
  }

  static std::pair<double, double>
  _intersect(double const& x1, double const& y1, double const& x2,
             double const& y2, double const& x3, double const& y3,
             double const& x4, double const& y4)
  {
    double const d = (y1 - y2) * (x4 - x3) + (y4 - y3) * (x2 - x1);
    // Avoid division by zero
    double x = !isequal(d, 0.) ? ((y1 * x2 - y2 * x1) * (x4 - x3) +
                                  (y4 * x3 - y3 * x4) * (x2 - x1)) /
                                     d
                               : 0.5 * (x2 + x3);
  ...
  static void _underestimate_in_window3(
      ...
      const unsigned int pos, const double p1AreaLoss, const double p2AreaLoss,
      std::vector<double>& data, const bool isUnder);

  static bool _cmp_indexed_areaLoss(const std::pair<double, int>& a,
                                    const std::pair<double, int>& b);

  static std::pair<double, int> _heap_pop(
      std::vector<std::pair<double, int>>& heap, std::vector<int>& hind);

  static void _modify_heap(std::vector<std::pair<double, int>>& heap,
                           const int pos, std::vector<int>& hind);

  static void _heap_bubble_up(std::vector<std::pair<double, int>>& heap,
                              int pos, std::vector<int>& hind);

  static void _heap_bubble_down(std::vector<std::pair<double, int>>& heap,
                                int pos, std::vector<int>& hind);

  template <typename UNIV, typename DUNIV>
  PWLU&
  _compose(UNIV const& f, DUNIV const& df, bool const under, bool const cvx,
           bool const inc)
  {
    ...
    else
    {
           ...
           ++idx, ++idy)
      {
        ...
        // if( isequal( f1 + 0.5 * df1 * *idy * *idx, f2 - 0.5 * df2 * *idy *
        // *idx, options.BKPTATOL, options.BKPTRTOL ) ){
        if (isequal(f1, f2 - df2 * *idy * *idx, options.BKPTATOL,
                    options.BKPTRTOL) &&
            isequal(f1 + df1 * *idy * *idx, f2, options.BKPTATOL,
                    options.BKPTRTOL))
        {
          // if( isequal( df1 * *idy * *idx, df2 * *idy * *idx,
          // options.BKPTATOL, options.BKPTRTOL ) ){
  ...
  PWLU& _reduce_heap(bool const under, size_t const nseg);

  PWLU& _reduce_std(bool const under, size_t const nseg);
};
...
PWLU::_reduce_std(bool const under, size_t const nseg)
{
  ...
  static thread_local std::vector<std::tuple<double, int, double, double>>
      vloss;
  ...
  {
    _loss(under, *lcv2, *lcv3, *incA0, *slpA0, *incA1, *slpA1, *incA2, *slpA2,
          *ploss);
#ifdef MC__PWLU_DEBUG
  ...
  for (;;)
  {
    ...
    for (auto itloss = vloss.cbegin(); itloss != vloss.cend(); ++itloss)
      std::cout << std::get<0>(*itloss) << " " << std::get<1>(*itloss) << " "
                << std::get<2>(*itloss) << " " << std::get<3>(*itloss) << " "
                << std::endl;
#endif

    // Determine minimal element
    auto ilossopt =
        std::min_element(vloss.cbegin(), vloss.cend(),
                         [=](std::tuple<double, int, double, double> const& a,
                             std::tuple<double, int, double, double> const& b)
    ...
    std::cout << std::get<0>(*ilossopt) << " " << std::get<1>(*ilossopt) << " "
              << std::get<2>(*ilossopt) << " " << std::get<3>(*ilossopt) << " "
              << std::endl;
#endif
    ...
    lcv[1] = under ? (*slpA0) - (*slpA1) : (*slpA1) - (*slpA0);
    lcv[2] = under ? (*slpA1) - (*slpA2) : (*slpA2) - (*slpA1);
    _loss(under, lcv[1], lcv[2], *incA0, *slpA0, *incA1, *slpA1, *incA2, *slpA2,
          *itloss);
#ifdef MC__PWLU_DEBUG
    ...
    _loss(under, lcv[2], lcv[0], *incA1, *slpA1, *incA2, *slpA2, *incA0, *slpA0,
          *(++itloss));
#ifdef MC__PWLU_DEBUG
    std::cout << std::get<0>(*itloss) << std::endl;
#endif
    ...
    slpA1  = &(*(++idy));
    lcv[1] = under ? (*slpA0) - (*slpA1) : (*slpA1) - (*slpA0);
    _loss(under, lcv[0], lcv[1], *incA2, *slpA2, *incA0, *slpA0, *incA1, *slpA1,
          *(++itloss));
#ifdef MC__PWLU_DEBUG
    std::cout << std::get<0>(*itloss) << std::endl;
#endif
...
inline void
PWLU::_loss(bool const isUnder, double const& slpDiff1, double const& slpDiff2,
            double const& incA0, double const& slpA0, double const& incA1,
            double const& slpA1, double const& incA2, double const& slpA2,
            std::tuple<double, int, double, double>& loss)
{
  if (slpDiff1 >= 0. && slpDiff2 >= 0.)
  {  // ccv ccv
  ...
  {  // cvx ccv
    ...
    if (slpDiff3 > 1e-14)
    {
      ...
      {
        loss = std::make_tuple(slpDiff2 * incA1 * _tmpL, 0,
                               incA0 + incA1 + _tmpL, _tmpR);
        return;
      }
    }
  ...
  {  // ccv cvx
    ...
    if (slpDiff3 > 1e-14)
    {
      ...
      {
        loss = std::make_tuple((slpA0 - slpA1) * incA1 * _tmpR, 0, _tmpL,
                               _tmpR + incA1 + incA2);
        return;
      }
    }
  ...
  else
  {
    ...
    loss = std::make_tuple(-slpDiff3 * deltaXL * deltaXR, 0, deltaXL + incA0,
                           deltaXR + incA2);
  }
...
//! @brief Linear interpolation for aligning a univariate linear estimator with
//! a univariate piecewise linear estimator w.r.t. the same variable
//! @param[in] pos is
//! @param[in] p1AreaLoss is
//! @param[in] p2AreaLoss is t
//! @param[out] data
inline void
PWLU::_underestimate_in_window3(const std::vector<double>& incA,
                                const std::vector<double>& slpA,
                                const unsigned int pos, const double slpDiff1,
                                const double slpDiff2,
                                std::vector<double>& data, const bool isUnder)
{
  ...
  {  // cvx ccv
    ...
    if (slpDiff3 > 1e-14)
    {
      // std::cerr << "case 2.1" << std::endl;
      double const _tmpL =
          ((slpA[pos1] - slpA[pos]) * incA[pos1]) / (slpA[pos] - slpA[pos2]);
  ...
  {  // ccv cvx
    ...
    if (slpDiff3 > 1e-14)
    {
      // std::cerr << "case 3.1" << std::endl;
      double const _tmpR =
          ((slpA[pos1] - slpA[pos2]) * incA[pos1]) / (slpA[pos2] - slpA[pos]);
      // if(_tmpR < 0){
      //   std::cout << isUnder << std::endl;
      //   std::cout << _tmpR << std::endl;
      //   std::cout << slpA[pos] << ", " << slpA[pos1] << ", " << slpA[pos2] <<
      //   std::endl; std::cout << incA[pos] << ", " << incA[pos1] << ", " <<
      //   incA[pos2] << std::endl; assert(_tmpR >=0.);
      // }
...
//! @param[out] second
inline PWLU&
PWLU::_reduce_heap(bool const isUnder, size_t const nseg)
{
  ...
  for (unsigned int jj = 1; jj < first.size(); jj++)
    if (first[jj] < 1e-15)
    {
      std::cout << jj << " in " << first.size() << first[jj] << std::endl;
      assert(first[jj] >= 1e-15);
    };
#endif
  ...
  slpDiff1 = isUnder
                 ? (slpA[0] - slpA[1])
                 : (slpA[1] -
                    slpA[0]);  // isUnder?(slpA[1]-slpA[2]):(slpA[2]-slpA[1]);
  for (unsigned int j = 0; j < n - 3; j++)
  {
    slpDiff2 =
        isUnder ? (slpA[j + 1] - slpA[j + 2]) : (slpA[j + 2] - slpA[j + 1]);
    ...
    //           << " incA[j] " << incA[j]
    //           << std::endl;
    yMinOrMax = isUnder ? std::min(yMinOrMax, yval[j + 1])
                        : std::max(yMinOrMax, yval[j + 1]);
    slpDiff1  = slpDiff2;
  }
  ...
  for (unsigned int jj = 1; jj < yval.size(); jj++)
    if (std::fabs(yval[jj]) > 1000)
    {
      std::cout << jj << " in " << yval.size() << " " << yval[jj] << std::endl;
      assert(std::fabs(yval[jj]) < 1000);
    };
#endif
  ...
  std::vector<int> hind(
      n - 3);  // mapping each window index to the position in the heap areaLoss
  ...
  auto& aMin =
      areaLoss[0];  // bind the reference to the top of the heap,
                    // heappop(areaLoss,hind) in python std::cout << "aMin " <<
                    // aMin.first << " at " << aMin.second << std::endl;
                    // std::cout << "areaLoss len" << areaLoss.size() <<
                    // std::endl; std::cout << "aMin1 " << areaLoss[1].first <<
                    // " at " << areaLoss[1].second << std::endl;
  // unsigned int ctr = areaLoss.size();

  // if (isUnder){
  //   while (yval[aMin.second] + slpXL[aMin.second]*deltaXL[aMin.second] +
  //   1e-14 < yMinOrMax){
  //     areaLoss[0].first = DBL_MAX;
  //     _modify_heap(areaLoss,0,hind);
  //     ctr --;
  //     if(ctr == 0 ){
  //       // _lbnd.first = yval[aMin.second] +
  //       slpXL[aMin.second]*deltaXL[aMin.second];
  //       // yMinOrMax = _lbnd.first;
  //       yMinOrMax = yval[aMin.second] +
  //       slpXL[aMin.second]*deltaXL[aMin.second]; break;
  //     }
  //     // std::cout << "aMin " << aMin.first << " at " << aMin.second <<
  //     std::endl;
  //     // if(areaLoss.size() <= 2) {
  //     //   std::cout << " here we only have 1-2 choices" << std::endl;
  //     //   throw typename PWLU::Exceptions( PWLU::Exceptions::UNDEF );
  //     // }
  //     // aMin = areaLoss[0]; // as the reference binds to that, we do not
  //     need assignment
  //   }
  // }
  // else{
  ...
  // std::cout <<  "            aMin.second " << aMin.second << " with " <<
  // std::scientific << std::setprecision(14) << aMin.first << std::endl;
  //  updating the indexes so that we can skip the second element in the window,
  //  and
  ...
  for (unsigned int i = 1; i < n - k - 1; i++)
  {
    ...
    if (pos > 0)
    {
      ...
      _underestimate_in_window3(incIn, slpIn, 0, slpDiff1, slpDiff2, data,
                                isUnder);
    ...
    // std::cout << "aMin " << areaLoss[0].first << " at " << areaLoss[0].second
    // << std::endl; std::cout << "aMin1 " << areaLoss[1].first << " at " <<
    // areaLoss[1].second << std::endl;

    // // if(aMin.first != areaLoss[0].first) std::cout << "error" << std::endl;
    // // test to make sure the reference binds to the heap top
    // // aMin = areaLoss[0]; // as the reference binds to that, we do not need
    // assignment, heappop(areaLoss,hind) in python unsigned int ctr =
    // areaLoss.size(); if (isUnder){
    //   while (yval[aMin.second] + slpXL[aMin.second]*deltaXL[aMin.second] +
    //   1e-14 < yMinOrMax){
    //     areaLoss[0].first = DBL_MAX;
    //     _modify_heap(areaLoss,0,hind);
...
inline bool
PWLU::_cmp_indexed_areaLoss(const std::pair<double, int>& a,
                            const std::pair<double, int>& b)
{
...
PWLU::_heap_pop(std::vector<std::pair<double, int>>& heap,
                std::vector<int>& hind)
{
...
inline void
PWLU::_modify_heap(std::vector<std::pair<double, int>>& heap, const int pos,
                   std::vector<int>& hind)
{
  if (pos == 0)
  {
...
inline void
PWLU::_heap_bubble_up(std::vector<std::pair<double, int>>& heap, int pos,
                      std::vector<int>& hind)
{
...
inline void
PWLU::_heap_bubble_down(std::vector<std::pair<double, int>>& heap, int pos,
                        std::vector<int>& hind)
{
  // print("bubble_down")
  ...
  while (childpos < endpos)
  {
    ...
    // print("left: heap[childpos] ", heap[childpos], "heap[rightpos]",
    // heap[rightpos])
...
inline std::ostream&
operator<<(std::ostream& os, PWLU const& var)
{
  var.display(os);
  return os;
}
...
inline PWLU
operator+(PWLU const& var1, PWLU const& var2)
{
...
*/
inline PWLU
operator+(PWLU&& var1, PWLU const& var2)
{
...
inline PWLU
operator+(PWLU const& var1, double const& cst2)
{
  PWLU res(var1);
  res += cst2;
  return res;
...
inline PWLU
operator+(PWLU&& var1, double const& cst2)
{
  PWLU res(std::move(var1));
  res += cst2;
  return res;
...
inline PWLU
operator+(double const& cst1, PWLU const& var2)
{
  PWLU res(var2);
  res += cst1;
  return res;
...
inline PWLU
operator+(double const& cst1, PWLU&& var2)
{
  PWLU res(std::move(var2));
  res += cst1;
  return res;
