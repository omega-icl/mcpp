...
//! @brief C++ class defining linear combinations as external DAG operations in
//! MC++.
////////////////////////////////////////////////////////////////////////
...
class FFLin : public FFOp
{
 private:
  ...
  FFVar&
  _set(size_t const nVar, FFVar const* pVar, double const* pCoef,
       double const& Bias, int policy)
  {
    ...
    if (policy > 0)
      _ptrCoef =
          static_cast<FFLin<T>*>(pOp)->_ptrCoef;  // set pointer to DAG copy
    else if (policy < 0)
  ...
  // set the object and related operation in DAG
  FFVar&
  _set(size_t const nVar, FFVar const* const* ppVar, double const* pCoef,
       double const& Bias, int policy)
  {
    if (_ownCoef && _ptrCoef) delete _ptrCoef;
    _ownCoef      = (policy > 0 ? true : false);
  ...
  // set the object and related operation in DAG
  FFVar&
  _set(size_t const nVar, FFVar const* pVar, double const& Coef,
       double const& Bias)  //, int policy )
  {
#ifdef MC__FFLIN_TRACE
    std::cout << "FFLin::set vector\n";
  ...
  // set the object and related operation in DAG
  FFVar&
  _set(size_t const nVar, FFVar const* const* ppVar, double const& Coef,
       double const& Bias)  //, int policy )
  {
    if (_ownCoef && _ptrCoef) delete _ptrCoef;
    _ownCoef      = true;  //( policy>0? true: false );
  ...
  // generic evaluation
  template <typename U>
  void _eval(size_t const nRes, U* vRes, size_t const nVar, U const* vVar,
 ...
 public:
  ...
  enum POLICY_TYPE
  {
    ...
    TRANSFER =
        -1  //!< Shallow copy of DAG object in FFGraph (with ownership transfer)
  };
  ...
  // Copy constructor
  FFLin(FFLin const& other) : FFOp(other), _valBias(other._valBias)
  {
#ifdef MC__FFLIN_TRACE
    ...
    if (!other._nCoef || !other._ptrCoef)
      throw std::runtime_error(
          "FFLin::copy constructor ** Null pointer to coefficient array\n");
  ...
  // Define operation
  FFVar&
  operator()(std::vector<FFVar> const& Var, std::vector<double> const& Coef,
  ...
  FFVar&
  operator()(size_t const nVar, FFVar const* pVar, double const* pCoef,
             double const& Bias = 0., int policy = COPY)
  {
#ifdef MC__FFLIN_CHECK
  ...
  FFVar&
  operator()(size_t const nVar, FFVar const* const* ppVar, double const* pCoef,
             double const& Bias = 0., int policy = COPY)
  {
#ifdef MC__FFLIN_CHECK
  ...
  FFVar&
  operator()(std::vector<FFVar> const& Var, double const& Coef,
             double const& Bias = 0.)  //, int policy=COPY )
  {
#ifdef MC__FFLIN_CHECK
    assert(Var.size());
  ...
  FFVar&
  operator()(size_t const nVar, FFVar const* pVar, double const& Coef,
             double const& Bias = 0.)  //, int policy=COPY )
  {
#ifdef MC__FFLIN_CHECK
    assert(nVar && pVar);
  ...
  FFVar&
  operator()(size_t const nVar, FFVar const* const* ppVar, double const& Coef,
             double const& Bias = 0.)  //, int policy=COPY )
  {
#ifdef MC__FFLIN_CHECK
    assert(nVar && ppVar);
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error("FFLin::feval ** No evaluation method for type" +
                             std::string(idU.name()) + "\n");
  }
        ...
        unsigned const nVar, void* vVar) const
  {
    ...
    throw std::runtime_error("FFLin::reval ** No evaluation method for type" +
                             std::string(idU.name()) + "\n");
  }
...
template <typename U>
inline void
FFLin<T>::_eval(size_t const nRes, U* vRes, size_t const nVar, U const* vVar,
                unsigned const* mVar) const
{
  vRes[0] = _valBias;
...
template <typename T>
inline void
FFLin<T>::eval(size_t const nRes, FFVar* vRes, size_t const nVar,
               FFVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFLIN_TRACE
...
template <typename T>
inline void
FFLin<T>::deriv(unsigned const nRes, FFVar const* vRes, unsigned const nVar,
                FFVar const* vVar, FFVar** vDer) const
{
#ifdef MC__FFLIN_TRACE
...
template <typename T>
inline void
FFLin<T>::eval(size_t const nRes, SLiftVar* vRes, size_t const nVar,
               SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFLIN_TRACE
...
template <typename T>
inline void
FFLin<T>::eval(size_t const nRes, PolVar<T>* vRes, size_t const nVar,
               PolVar<T> const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFLIN_TRACE
  ...
  for (unsigned i = 0; i < nVar; ++i) vTVar[i] = vVar[i].range();
  T TRes;
...
template <typename T>
inline bool
FFLin<T>::reval(size_t const nRes, PolVar<T>* vRes, size_t const nVar,
                PolVar<T>* vVar) const
{
#ifdef MC__FFLIN_TRACE
  ...
  // if( _nCoef < nVar )
  //   img->add_cut( pop, PolCut<T>::EQ, -_valBias, nVar, vVar, _ptrCoef[0],
  //   vRes[0], -1. );
  // else
  //   img->add_cut( pop, PolCut<T>::EQ, -_valBias, nVar, vVar, _ptrCoef,
  //   vRes[0], -1. );
...
template <typename T>
inline bool
FFLin<T>::reval(size_t const nRes, T* vRes, size_t const nVar, T* vVar) const
{
#ifdef MC__FFLIN_TRACE
...
FFLin<T>::lt(FFOp const* other) const
{
  ...
  {
    if (_ptrCoef[i < _nCoef ? i : 0] <
        oprecast->_ptrCoef[i < oprecast->_nCoef ? i : 0])
      return true;
    if (_ptrCoef[i < _nCoef ? i : 0] >
        oprecast->_ptrCoef[i < oprecast->_nCoef ? i : 0])
      return false;
