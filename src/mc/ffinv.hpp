...
mc::FFInv is a C++ class that determines the structure of mathematical
expressions with a view to inverting them, that is, express one of the
participating variables as a function of the rest. It relies on the operator
overloading and function overloading mechanisms of C++. The overloaded operators
are: `+', `-', `*', and `/'; the overloaded functions include: `exp', `log',
`sqr', `pow', `cheb', `sqrt', `fabs', `xlog', `min', `max', `cos', `sin', `tan',
`acos', `asin', `atan', `cosh', `sinh', `tanh'.

This class may be used in combination with mc::FFDep for detecting the type of
dependencies in factorable functions.


\section sec_FFInvEval How Do I Determine the Invertibility of a Factorable
Function?

Suppose you are given 4 variables \f$x_0,\ldots,x_3\f$ and want to analyze the
structure of the vector function: \f{eqnarray*}
...
The first line declares <tt>X</tt> as an array of mc::FFInv class objects, then
the second line defines X[0] ... X[NX-1] as independent variables with indices 0
... NX-1, respectively.

Once the independent variables \f${\bf x}\f$ have been defined, the invertible
structure of \f${\bf f}({\bf x})\f$ can be detected using the mc::FFInv
...
Retrieve information about the invertible structure of \f$f_1\f$ and \f$f_2\f$
using
...
These results indicate that X[0], X[2] and X[3] participate in F[0], but not
X[1]: X[0] and X[3] are separably linear (S), but the invertibility of X[2] is
undetermined (U) due to multiple occurences.

Likewise, all four variables X[0], X[1], X[2] and X[3] participate in F[1]: the
invertibility of X[0] is undetermined &ndash; since IPOW is not allowed as
invertible in the option set FFInv::Options::INVOP by default); X[1]
participates linearly and is thus invertible; X[2] is nonlinearly invertible (N)
&ndash; insofar as EXP is indeed allowed as invertible in the option set
FFInv::Options::INVOP; and X[3] is separably linear (S).


\section sec_FFInvOpt Available Options for Invertible Structure Detection

The class mc::FFInv has a static public member called mc::FFInv::options that
can be used to set/modify the options. For instance:
...
<TABLE border="1">
<CAPTION><EM>Options in mc::FFInv::Options: name, type and
description</EM></CAPTION> <TR><TH><b>Name</b> <TD><b>Type</b><TD><b>Default</b>
         <TD><b>Description</b>
     <TR><TH><tt>INVOPT</tt> <TD><tt>std::set<NLINV></tt> <TD> {
FFInv::Options::INV, FFInv::Options::SQRT, FFInv::Options::EXP,
FFInv::Options::LOG, FFInv::Options::RPOW } <TD>Defines the set of nonlinear
invertible operations - FFInv::Options::IPOW not included by default.
</TABLE>
...
FFInv::Exceptions is thrown, which contains the type of error. It is the user's
...
////////////////////////////////////////////////////////////////////////
{
 ...
 public:
  ...
  FFInv(double const& c = 0.) {}
  //! @brief Copy constructor
  FFInv(FFInv const& S) : _inv(S._inv) {}
  //! @brief Destructor
  ~FFInv() {}
  ...
  //! @brief Determine if current expression is invertible in the variable of
  //! index <a>ind</a>
  std::pair<bool, TYPE>
  inv(int const ind) const
  {
  ...
  FFInv& combine(FFInv const& S, TYPE const& invmin);
  //! @brief Combine the invertibility sets of two variables
  static FFInv combine(FFInv const& S1, FFInv const& S2, TYPE const& invmin);
  //! @brief Update invertibility type of variables
  FFInv& update(TYPE const& invmin);
  //! @brief Copy variables and update invertibility type
  static FFInv copy(FFInv const& S, TYPE const& invmin);
 ...
 public:
  ...
  FFInv&
  operator=(FFInv const& S)
  {
    if (this != &S) _inv = S._inv;
    return *this;
  }
  FFInv&
  operator+=(double const& c)
...
inline FFInv
FFInv::copy(FFInv const& S, TYPE const& invmin)
{
  FFInv S2(S);
...
inline FFInv
FFInv::combine(FFInv const& S1, FFInv const& S2, TYPE const& invmin)
{
  FFInv S3(S1);
...
inline std::ostream&
operator<<(std::ostream& out, FFInv const& S)
{
  out << "{ ";
...
inline FFInv
operator+(double const& c, FFInv const& S)
{
  return S;
}

inline FFInv
operator+(FFInv const& S, double const& c)
{
  return S;
}

inline FFInv
operator+(FFInv const& S1, FFInv const& S2)
{
...
inline FFInv
sum(const unsigned int n, const FFInv* S)
{
  switch (n)
  {
    case 0:
...
FFInv::operator*=(FFInv const& S)
{
  ...
  if (_inv.empty()) return combine(S, TYPE::L);
...
inline FFInv
prod(const unsigned int n, const FFInv* S)
{
  switch (n)
  {
    case 0:
...
inline FFInv
monom(const unsigned int n, const FFInv* S, const unsigned* k)
{
  switch (n)
  {
    case 0:
      return 0.;
    case 1:
      return pow(S[0], (int)k[0]);
    default:
      return pow(S[0], (int)k[0]) * monom(n - 1, S + 1, k + 1);
  }
}
...
inline FFInv
arh(FFInv const& S, double const& a)
{
...
inline FFInv
lmtd(FFInv const& S1, FFInv const& S2)
{
  return FFInv::combine(S1, S2, FFInv::TYPE::U);
}

inline FFInv
rlmtd(FFInv const& S1, FFInv const& S2)
{
  return FFInv::combine(S1, S2, FFInv::TYPE::U);
}
...
inline FFInv
cheb(FFInv const& S, const unsigned n)
{
  if (n == 0) return FFInv();
  if (n == 1) return S;
  if (n == 2) return sqr(S);
...
inline FFInv
pow(FFInv const& S, double const& a)
{
  if (a == 0.) return FFInv();
  if (a == 1.) return S;
  if (a == 2.) return sqr(S);
...
inline FFInv
pow(FFInv const& S1, FFInv const& S2)
{
  return FFInv::combine(S1, S2, FFInv::TYPE::U);
}

inline FFInv
min(FFInv const& S1, FFInv const& S2)
{
  return FFInv::combine(S1, S2, FFInv::TYPE::U);
}

inline FFInv
max(FFInv const& S1, FFInv const& S2)
{
  return FFInv::combine(S1, S2, FFInv::TYPE::U);
}

inline FFInv
min(const unsigned int n, const FFInv* S)
{
  switch (n)
  {
    case 0:
...
inline FFInv
max(const unsigned int n, const FFInv* S)
{
  switch (n)
  {
    case 0:
...
inline FFInv
inter(FFInv const& S1, FFInv const& S2)
{
  return FFInv::combine(S1, S2, FFInv::TYPE::U);
}
...
struct Op<mc::FFInv>
{
  ...
  zeroone()
  {
    throw typename FFInv::Exceptions(FFInv::Exceptions::UNDEF);
  }
  static void
  I(FV& x, const FV& y)
  {
