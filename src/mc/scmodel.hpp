...
/*!
\page page_CHEBYSHEV_SPARSE Sparse Chebyshev Model Arithmetic for Factorable
Functions

The classes mc::SCModel and mc::SCVar provide an implementation of sparse
Chebyshev model arithmetic. The polynomial part is propagated in floating-point
arithmetic, without preallocation to enable sparse storage. In that respect, The
remainder term is propagated in the arithmetic of the templated type, typically
a verified interval arithmetic. Rounding errors are not accounted for during the
polynomial propagation, so mc::SCModel and mc::SCVar do not provide a fully
verified implementation.

\section sec_CHEBYSHEV_SPARSE_use How do I compute a sparse Chebyshev model of a
factorable function?

Suppose we want to compute a 4th-order sparse Chebyshev model for the
real-valued function \f$f(x,y)=x\exp(y^2)-y^2\f$ with \f$(x,y)\in
[1,2]\times[0,1]\f$. For simplicity, bounds on the remainder terms are computed
...
First, an mc::SCModel object is initialized by passing the order of the
polynomial inclusion (4th order here):
...
mc::SCVar, participating in the Chebyshev model <tt>mod</tt>, belonging to the
interval \f$[1,2]\f$, and having index 0 (C-style indexing in use). The same
holds for the variable <tt>Y</tt>, participating in the model <tt>mod</tt>,
belonging to the interval \f$[0,1]\f$, and having index 1.

Having defined the variables, a sparse Chebyshev model of
\f$f(x,y)=x\exp(x+y^2)-y^2\f$ on \f$[1,2]\times[0,1]\f$ at the mid-point
\f$(\frac{3}{2},\frac{1}{2})\f$ is computed as:
...
The coefficients and corresponding monomials in the Chebyshev model are
displayed first, with the middle integer being the total monomial order. The
remainder term as well as the Chebyshev model range estimator are reported next.

Other operations involve retreiving the remainder bound, or computing the value
of its polynomial part at a given point:
...
See the documentations of mc::SCModel and mc::SCVar for a complete list of
member functions.


\section sec_CHEBYSHEV_SPARSE_fct Which functions are overloaded for sparse
Chebyshev model arithmetic?

mc::SCVar overloads the usual functions <tt>exp</tt>, <tt>log</tt>,
<tt>sqr</tt>, <tt>sqrt</tt>, <tt>pow</tt>, <tt>inv</tt>, <tt>cos</tt>,
<tt>sin</tt>, <tt>tan</tt>, <tt>acos</tt>, <tt>asin</tt>, <tt>atan</tt>,
<tt>cosh</tt>, <tt>sinh</tt>, <tt>tanh</tt>, <tt>erf</tt>, <tt>erfc</tt>,
<tt>fabs</tt>. The functions <tt>min</tt>, <tt>max</tt>, <tt>fstep</tt>, and
<tt>bstep</tt> are not currently overloaded in mc::SCVar. Moreover, mc::SCVar
defines the following functions:
- <tt>inter(x,y,z)</tt>, computing a Chebyshev model of the intersection \f$x =
y\cap z\f$ of two Chebyshev models and returning true/false if the intersection
is nonempty/empty. With Chebyshev models \f$\mathcal P_y\oplus\mathcal R_y\f$
and \f$\mathcal P_z\oplus\mathcal R_z\f$, this intersection is computed as
follows: \f{align*}
  \mathcal P_{x} =\ & (1-\eta) \mathcal P_y^{\rm C} + \eta \mathcal P_z^{\rm
C}\\ \mathcal R_{x} =\ & [\mathcal R_y^{\rm C}\oplus\eta\mathcal{B}(\mathcal
P_y^{\rm C}-\mathcal P_z^{\rm C})] \cap [\mathcal R_z^{\rm C}\oplus
(1-\eta)\mathcal{B}(\mathcal P_z^{\rm C}-\mathcal P_y^{\rm C})]\,. \f} with
\f$\mathcal{B}(\cdot)\f$ the Chebyshev model range bounder, and \f$\eta\f$ a
real scalar in \f$[0,1]\f$. Choosing \f$\eta=1\f$ amounts to setting the
polynomial part \f$\mathcal P_{x}\f$ as \f$\mathcal P_y\f$, whereas \f$\eta=0\f$
sets \f$\mathcal P_{x}\f$ as \f$\mathcal P_z\f$. The parameter \f$\eta\f$ can be
defined in mc::CModel::Options::REF_POLY.
- <tt>hull(x,y)</tt>, computing a Chebyshev model of the union \f$x = y\cup z\f$
of two Chebyshev models. With Chebyshev models \f$\mathcal P_y\oplus\mathcal
R_y\f$ and \f$\mathcal P_z\oplus\mathcal R_z\f$, this union is computed as
follows: \f{align*}
  \mathcal P_{x} =\ & (1-\eta) \mathcal P_y^{\rm C} + \eta \mathcal P_z^{\rm
C}\\ \mathcal R_{x} =\ & {\rm hull}\{\mathcal R_y^{\rm
C}\oplus\eta\mathcal{B}(\mathcal P_y^{\rm C}-\mathcal P_z^{\rm C}), \mathcal
R_z^{\rm C}\oplus (1-\eta)\mathcal{B}(\mathcal P_z^{\rm C}-\mathcal P_y^{\rm
C})\}\,. \f} with \f$\mathcal{B}(\cdot)\f$ and \f$\eta\f$ as previously.
.

\section sec_CHEBYSHEV_SPARSE_opt How are the options set for the computation of
a sparse Chebyshev model?

The class mc::SCModel has a public member called mc::SCModel::options that can
be used to set/modify the options. For instance:
...
<TABLE border="1">
<CAPTION><EM>Options in mc::SCModel::Options: name, type and
description</EM></CAPTION> <TR><TH><b>Name</b> <TD><b>Type</b><TD><b>Default</b>
         <TD><b>Description</b>
     <TR><TH><tt>BASIS</tt> <TD><tt>unsigned int</tt> <TD>1
         <TD>Basis representation of the monomials: 0-Monomial basis;
1-Chebyshev basis. <TR><TH><tt>REMEZ_USE</tt> <TD><tt>bool</tt> <TD>false
         <TD>Whether to use the Remez algorithm for computing a minimax
approximation for univariate terms. <TR><TH><tt>REMEZ_MAXIT</tt>
<TD><tt>unsigned int</tt> <TD>10 <TD>Maximal number of iterations in Remez
algorithm for computing a minimax approximation for univariate terms.
     <TR><TH><tt>REMEZ_TOL</tt> <TD><tt>double</tt> <TD>1e-5
         <TD>Stopping tolerance in Remez algorithm for computing a minimax
approximation for univariate terms. <TR><TH><tt>REMEZ_MIG</tt>
<TD><tt>double</tt> <TD>1e-10 <TD>Threshold for interval width below which Remez
algorithm is not useds. <TR><TH><tt>INTERP_EXTRA</tt> <TD><tt>unsigned int</tt>
<TD>0 <TD>Extra terms in Chebyshev interpolation of univariates: 0-Chebyshev
interpolation of order NORD; extra terms allow approximation of Chebyshev
truncated series. <TR><TH><tt>INTERP_THRES</tt> <TD><tt>double</tt>
<TD>1e2*DBL_EPSILON <TD>Threshold for coefficient values in Chebyshev expansion
for bounding of transcendental univariates. <TR><TH><tt>BOUNDER_TYPE</tt>
<TD><tt>mc::SCModel::Options::BOUNDER</tt> <TD>mc::SCModel::Options::LSB
         <TD>Chebyshev model range bounder.
     <TR><TH><tt>MIXED_IA</tt> <TD><tt>bool</tt> <TD>false
         <TD>Whether to intersect internal bounds with underlying bounds in the
templated arithmetics. <TR><TH><tt>MIN_FACTOR</tt> <TD><tt>double</tt> <TD>0e0
         <TD>Threshold for monomial coefficients below which the term is removed
and appended to the remainder term. <TR><TH><tt>REF_POLY</tt>
<TD><tt>double</tt> <TD>0. <TD>Scalar in \f$[0,1]\f$ related to the choice of
the polynomial part in the overloaded functions mc::inter and mc::hull (see \ref
sec_CHEBYSHEV_SPARSE_fct). A value of 0. amounts to selecting the polynomial
part of the left operand, whereas a value of 1. selects the right operand.
...
\section sec_CHEBYSHEV_SPARSE_err Errors What errors can I encounter during
computation of a sparse Chebyshev model?
...
mc::SCModel::Exceptions is thrown, which contains the type of error. Possible
errors encountered during the computation of a sparse Chebyshev model are:

<TABLE border="1">
<CAPTION><EM>Errors from class mc::SCModel::Exceptions during the Computation of
a Sparse Chebyshev Model</EM></CAPTION> <TR><TH><b>Number</b>
<TD><b>Description</b> <TR><TH><tt>1</tt> <TD>Division by zero scalar
     ...
     <TR><TH><tt>-2</tt> <TD>Inconsistent bounds with template parameter
arithmetic <TR><TH><tt>-3</tt> <TD>Operation between Chebyshev variables linked
to different Chebyshev models <TR><TH><tt>-4</tt> <TD>Internal error
...
//! @brief C++ class for the computation of sparse Chebyshev models for
//! factorable function: environment
////////////////////////////////////////////////////////////////////////
...
template <typename T, typename KEY = unsigned,
          typename COMP = std::less<unsigned>>
class SCModel
////////////////////////////////////////////////////////////////////////
{
  friend class SCVar<T, KEY, COMP>;
  template <typename U, typename K, typename C>
  friend class SCModel;
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> inv(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> sqr(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> sqrt(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> exp(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C>&& exp(SCVar<U, K, C>&&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> log(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> xlog(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> pow(const SCVar<U, K, C>&, const int);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> pow(const SCVar<U, K, C>&, double const&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> pow(double const, const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> pow(const SCVar<U, K, C>&, const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> prod(const unsigned int, const SCVar<U, K, C>*);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> monom(const unsigned int, const SCVar<U, K, C>*,
                              const unsigned*);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> cheb(const SCVar<U, K, C>&, const unsigned);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> cos(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> sin(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> tan(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> acos(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> asin(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> atan(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> cosh(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> sinh(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> tanh(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> erf(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> erfc(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> fabs(const SCVar<U, K, C>&);
 ...
 protected:
  ...
  //! @brief Maps Variable ID -> Degree -> Vector of Bernstein Coefficients;
  //! Stores basis functions (T_k or x^k) in their natural degree as Armadillo
  //! vectors
  mutable std::map<KEY, std::vector<arma::vec>, COMP> _bernstein_cache;

  //! @brief Resize the model variable data containers
  void _set(KEY const& id, T const& X);

  //! @brief Overloads returning a pointer of a new KEY
  template <typename K>
  static K*
  _newaux(K* dum, int ndx)
  {
    return new K(ndx);
  }
  //{ K* aux = new K( ndx ); std::cout << "AUX: " << aux << std::endl; return
  // aux; }
  template <typename K>
  static K
  _newaux(K& dum, int ndx)
 ...
 public:
  /** @addtogroup SCHEBYSHEV Sparse Chebyshev Model Arithmetic for Factorable
   * Functions
   *  @{
   */
  //! @brief Constructor of Sparse Chebyshev model environment for maximal order
  //! <tt>maxord</tt>
  SCModel(const unsigned maxord = 3) : _maxord(maxord)
  {
  ...
  //! @brief Reset new auxiliary variables
  void reset_aux();
  ...
  //! @brief Get Chebyshev basis functions in U arithmetic for variable bounds
  //! <a>bndvar</a>
  template <typename U>
  ...
  //! @brief Get Chebyshev monomial bounds in U arithmetic for variable bounds
  //! <a>bndvar</a>
  template <typename U>
  void get_bndmon(std::map<t_mon, U, lt_mon>& bndmon,
                  std::map<KEY, U, COMP> const& bndvar,
                  bool const scaled = false, bool const useprod = false) const;

  //! @brief Transform monomial-coefficient map 'coefmon' to Berstein basis
  //! representation
  template <typename C>
  std::pair<std::map<t_mon, C, lt_mon>, std::map<KEY, unsigned, COMP>>
  to_bernstein(std::map<t_mon, C, lt_mon> const& coefmon,
               unsigned const ORDER = 0) const;
   ...
   public:
    ...
    what()
    {
      switch (_ierr)
      {
        ...
        case LOG:
          return "mc::SCModel\t Log operation with non-positive numbers in "
                 "range";
        case SQRT:
          return "mc::SCModel\t Square-root operation with negative numbers in "
                 "range";
        case DPOW:
          return "mc::SCModel\t Real power operation with negative numbers in "
                 "range";
        case TAN:
          return "mc::SCModel\t Tangent operation with (k+1/2)·PI in range";
        case ACOS:
          return "mc::SCModel\t Cosine inverse operation with range outside "
                 "[-1,1]";
        case ASIN:
          return "mc::SCModel\t Sine inverse operation with range outside "
                 "[-1,1]";
        case COMPOSE:
        ...
        case INCON:
          return "mc::SCModel\t Inconsistent bounds with template parameter "
                 "arithmetic";
        case MODEL:
          return "mc::SCModel\t Operation between variables in different model "
                 "environments";
        case UNDEF:
  ...
  struct Options
  {
    ...
    unsigned BASIS;
    //! @brief Whether to lift the remainder term in nonlinear operations by
    //! introducing auxiliary variables in the model
    bool LIFT_USE;
    //! @brief Absolute tolerance for lifting the reminder term - only if
    //! LIFT_USE == true
    double LIFT_ATOL;
    //! @brief relative tolerance for lifting the reminder term - only if
    //! LIFT_USE == true
    double LIFT_RTOL;
    //! @brief Whether to use the Remez algorithm for computing a minimax
    ...
    //! approximation for univariate terms
    double REMEZ_TOL;
    //! @brief Threshold for interval width below which Remez algorithm is not
    //! used
    double REMEZ_MIG;
    //! @brief Extra terms in chebyshev interpolation of univariates:
    //! 0-Chebyshev interpolation of order MAXORD; extra terms allow
    //! approximation of Chebyshev truncated series
    unsigned INTERP_EXTRA;
    //! @brief Threshold for coefficient values in Chebyshev expansion for
    //! bounding of transcendental univariates
    double INTERP_THRES;
    //! @brief Chebyshev model range bounder - See \ref sec_CHEBYSHEV_SPARSE_opt
    BOUNDER BOUNDER_TYPE;
    //! @brief Degree of the Bernstein basis when
    //! mc::CModel::options::BOUNDER_TYPE is set to
    //! mc::CModel::options::BERNSTEIN
    ...
    //! @brief Whether to simplify the monomial terms with small magnitude in
    //! the model
    bool MIG_USE;
    //! @brief Absolute tolerance for simplifying monomial terms - only if
    //! MIG_USE == true
    double MIG_ATOL;
    //! @brief relative tolerance for simplifying monomial terms - only if
    //! MIG_USE == true
    double MIG_RTOL;
    //! @brief Whether to intersect internal bounds with underlying bounds in T
    //! arithmetics
    bool MIXED_IA;
    //! @brief Scalar in \f$[0,1]\f$ related to the choice of the polynomial
    //! part in the overloaded functions mc::inter and mc::hull (see \ref
    //! sec_CHEBYSHEV_SPARSE_fct). A value of 0. amounts to selecting the
    //! polynomial part of the left operand, whereas a value of 1. selects the
    //! right operand.
    double REF_POLY;
    //! @brief Number of digits in output stream for Chebyshev model
    //! coefficients.
 ...
 private:
  ...
  //! @brief Get Chebyshev basis functions in U arithmetic for variable
  //! <a>bndvar</a>
  template <typename U>
  std::vector<U>& _get_bndpow(unsigned const maxord, U const& bndvar,
                              double const& refvar, double const& scalvar,
                              std::vector<U>& bndpow) const;

  //! @brief Get Chebyshev basis functions in U arithmetic for [-1,1] scaled
  //! variable <a>bndvar</a>
  template <typename U>
  std::vector<U>& _get_bndpow(unsigned const maxord, U const& bndvar,
                              std::vector<U>& bndpow) const;
  ...
  VEC _to_chebyshev(VEC const& veccoef) const;

  //! @brief Construct minimax polynomial coefficient for univariate <a>f</a> in
  //! <a>_coefuniv</a> and return corresponding error for univariate <a>f</a>
  //! using Remez algorithm
  template <typename PUNIV>
  double _minimax(PUNIV const& f);

  //! @brief Compose minimax polynomial for univariate <a>f</a> with Chebyshev
  //! variable <a>CV</a> and return resulting Chebyshev variable in <a>CV2</a>
  template <typename PUNIV>
  bool _minimax(PUNIV const& f, SCVar<T, KEY, COMP> const& CV,
                SCVar<T, KEY, COMP>& CV2);

  //! @brief Return error at bound between univariate <a>f</a> and corresponding
  //! interpolating coefficients in <a>_coefuniv</a>
  template <typename PUNIV>
  double _rematbound(PUNIV const& f);

  //! @brief Construct interpolating polynomial coefficient for univariate
  //! <a>f</a> in <a>_coefuniv</a> up to order <a>maxord</a>
  template <typename PUNIV>
  void _chebinterp(PUNIV const& f, unsigned const maxord);

  //! @brief Construct interpolating polynomial coefficient for univariate
  //! <a>f</a> in <a>_coefuniv</a> until coefficient magnitude is less than
  //! <a>tol</a>
  template <typename PUNIV>
  void _chebinterp(PUNIV const& f, double const& tol, unsigned& maxord);

  //! @brief Apply the Clenshaw algorithm
  SCVar<T, KEY, COMP> _clenshaw(SCVar<T, KEY, COMP>&& CVinner,
                                unsigned const maxord);

  //! @brief Apply the Horner algorithm
  SCVar<T, KEY, COMP> _horner(SCVar<T, KEY, COMP>&& CVinner,
                              unsigned const maxord);

  //! @brief Compose interpolating polynomial for univariate <a>f</a> with
  //! Chebyshev variable <a>CV</a> and return resulting Chebyshev variable in
  //! <a>CV2</a>
  template <typename PUNIV>
  bool _chebinterp(PUNIV const& f, SCVar<T, KEY, COMP> const& CV,
                   SCVar<T, KEY, COMP>& CV2, bool const rematbound);

  //! @brief Apply Chebyshev composition to variable <a>CVI</a> using the
  //! coefficients <a>coefmon</a> of the outer function
  template <typename U>
  static SCVar<T, KEY, COMP> _composition(std::vector<U> const& coefouter,
                                          unsigned const maxord,
                                          SCVar<T, KEY, COMP> const& CVinner);

  //! @brief Recursive calculation of nonnegative integer powers
  SCVar<T, KEY, COMP> _intpow(SCVar<T, KEY, COMP> const& CV, int const n) const;

  //! @brief Polynomial range bounder - Naive approach
  template <typename C, typename U>
  U _polybound_naive(std::map<t_mon, C, lt_mon> const& coefmon,
  ...
  template <typename C, typename U>
  U _polybound_LSB(std::map<t_mon, C, lt_mon> const& coefmon,
  ...
  template <typename C, typename U>
  U _polybound_bernstein(
      std::map<t_mon, C, lt_mon> const& coefmon,
      std::map<KEY, std::vector<U>, COMP> const& bndbasis) const;

  //! @brief Polynomial range bounder using specified bounder <a>type</a> with
  //! basis functions <a>bndbasis</a> in U arithmetic and monomial coefficients
  //! <a>coefmon</a> in C arithmetic
  template <typename C, typename U>
  U _polybound(std::map<t_mon, C, lt_mon> const& coefmon,
               std::map<KEY, std::vector<U>, COMP> const& bndbasis,
               int const type) const;

  //! @brief Monomial range bounder for <a>mon</a>
  T _monbound(t_mon const& mon) const;

  //! @brief Monomial value of <a>mon</a> at point <a>x</a>
  double _monval(t_mon const& mon, std::map<KEY, double, COMP> const& x) const;

  //! @brief Recursive product of univariate Chebyshev polynomials
  void _sprod1D(std::map<unsigned, t_poly> const& sp1map,
                std::map<unsigned, t_poly> const& sp2map, t_poly& coefmon,
                T& rem, t_var const& ndxvar,
                typename t_var::const_iterator itvar) const;

  //! @brief Scaling of Chebyshev coefficient maps
  void _sscal1D(const t_poly& coefmon0, double const& coefscal,
                t_poly& coefmon) const;

  //! @brief Lifting of Chebyshev coefficient maps
  void _slift1D(const t_poly& coefmon0, double const& dscal,
                t_poly& coefmon) const;

  //! @brief Lifting of Chebyshev coefficient maps
  void _slift1D(t_poly const& coefmon0, double const& dscal, t_poly& coefmon,
                T& rem, typename t_var::const_iterator itvar,
                unsigned const ndxord) const;

  //! @brief Lifting of Chebyshev coefficient maps
  void _slift1D(const t_poly& coefmon0, double const& dscal, T& rem,
                unsigned const ndxor) const;

  //! @brief Build 1D vector of Chebyshev coefficients
  void _svec1D(typename t_var::const_iterator itvar,
               std::pair<t_mon, double> const& mon,
               std::map<unsigned, t_poly>& mapspoly) const;

  //! @brief Display of recursive univariate Chebyshev polynomials
  void _sdisp1D(std::map<unsigned, t_poly> const& coefmon,
                typename t_var::const_iterator itvar,
                const std::string& name = "",
                std::ostream& os        = std::cout) const;

  //! @brief Display of recursive univariate Chebyshev polynomials
  void _sdisp1D(const t_poly& coefmon, const std::string& name = "",
                std::ostream& os = std::cout) const;

  //! @brief Build 1D vector of Chebyshev coefficients
  void _svec1Dfull(typename t_var::const_iterator itvar,
                   std::pair<t_mon, double> const& mon,
                   std::vector<SCVar<T, KEY, COMP>>& vec) const;

  //! @brief Display of recursive univariate Chebyshev polynomials
  void _sdisp1Dfull(std::vector<SCVar<T, KEY, COMP>> const& vec,
                    typename t_var::const_iterator itvar,
                    std::string const& name = "",
                    std::ostream& os        = std::cout) const;

  //! @brief Product of multivariate Chebyshev polynomials in sparse format
  void _sprod(SCVar<T, KEY, COMP> const& CV1, SCVar<T, KEY, COMP> const& CV2,
              t_poly& coefmon, double& coefrem) const;
};

template <typename T, typename KEY, typename COMP>
inline const std::string SCModel<T, KEY, COMP>::Options::BOUNDER_NAME[3] = {
    "NAIVE", "LSB", "BERNSTEIN"};

template <typename T, typename KEY, typename COMP>
inline T SCModel<T, KEY, COMP>::TZeroOne = Op<T>::zeroone();

template <typename T, typename KEY, typename COMP>
inline T SCModel<T, KEY, COMP>::TOne = 2. * Op<T>::zeroone() - 1.;

//! @brief C++ class for the computation of sparse Chebyshev models for
//! factorable function: propagation
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
{
  ...
  template <typename U, typename K, typename C>
  friend std::ostream& operator<<(std::ostream&, const SCVar<U, K, C>&);

  template <typename U, typename K, typename C>
  friend U funcptr(const U, const unsigned);
  template <typename U, typename K, typename C>
  friend void interpolation(double*, const SCVar<U, K, C>&, const unsigned);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> composition(double const*, const SCVar<U, K, C>&);
  ...
  friend SCVar<U, K, C> fabs(const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SCVar<U, K, C> hull(const SCVar<U, K, C>&, const SCVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend bool inter(SCVar<U, K, C>&, const SCVar<U, K, C>&,
                    const SCVar<U, K, C>&);
 ...
 private:
  ...
  //! @brief Pointer to variable bound in underlying T arithmetic (possibly NULL
  //! if not computed)
  mutable T* _bndT;

  //! @brief Pointer to polynomial bound of variable (possibly NULL if not
  //! available)
  mutable T* _bndpol;
  ...
  void _set_bndpol(T const& bndpol);
  ...
  double const&
  _scalvar(KEY const& id) const
  {
    return _CM->_scalvar[id];
  };

  //! @brief Set Chebyshev variable equal to <a>CV</a>
  SCVar<T, KEY, COMP>& _set(SCVar<T, KEY, COMP> const& CV);

  //! @brief Set Chebyshev variable equal to <a>CV</a>
  SCVar<T, KEY, COMP>& _set(SCVar<T, KEY, COMP>&& CV);

  //! @brief Set Chebyshev variable <a>id</a> with domain <a>dom</a>
  SCVar<T, KEY, COMP>& _set(KEY const& id, T const& dom,
                            const bool updt = true);

  //! @brief Product of multivariate Chebyshev polynomials in sparse format
  void
  _sprod(const SCVar<T, KEY, COMP>& CV1, const SCVar<T, KEY, COMP>& CV2,
         t_poly& coefmon, double& coefrem) const
  {
 ...
 public:
  ...
  static std::string display(t_poly coefmon, int const& BASIS = 1,
                             int const& IDISP = 5);
  ...
  SCVar(SCModel<T, KEY, COMP>* CM);

  //! @brief Constructor of Chebyshev variable for a real scalar
  SCVar(double const& d = 0., SCModel<T, KEY, COMP>* CM = nullptr);

  //! @brief Constructor of Chebyshev variable for a remainder bound
  SCVar(T const& B, SCModel<T, KEY, COMP>* CM = nullptr);

  //! @brief Constructor of Chebyshev variable <a>id</a> with domain <a>dom</a>
  SCVar(SCModel<T, KEY, COMP>* CM, KEY const& id, T const& dom);

  //! @brief Copy constructor of Chebyshev variable
  SCVar(SCVar<T, KEY, COMP> const& CV);

  //! @brief Move constructor of Chebyshev variable
  SCVar(SCVar<T, KEY, COMP>&& CV);
  ...
  //! @brief Set Chebyshev variable <a>id</a> with domain <a>dom</a>
  SCVar<T, KEY, COMP>&
  set(SCModel<T, KEY, COMP>* CM, KEY const& id, T const& dom)
  {
    set(CM);
    _set(id, dom);
    return *this;
  ...
  SCVar<T, KEY, COMP>&
  set(SCModel<T, KEY, COMP>* CM)
  {
    _CM = CM;
    return *this;
  }

  //! @brief Set multivariate polynomial coefficients in variable as
  //! <tt>coefmon</tt>
  SCVar<T, KEY, COMP>&
  set(t_poly const& coefmon, T const& bndrem = 0.)
  {
    _coefmon = coefmon;
    _unset_bndT();
    _unset_bndpol();
    _bndrem = bndrem;
    return *this;
  }  // this is assuming the same order and number of variables
  ...
  //! @brief Takes the negative of variable
  SCVar<T, KEY, COMP>& neg();
  ...
  //     else{ T bndT; return Op<T>::inter( bndT, _polybound(type) + _bndrem,
  //     *_bndT )?
  ...
  {
    if (!_bndT)
      return bndpol() + _bndrem;
    else
    {
      T bndT;
      return Op<T>::inter(bndT, bndpol() + _bndrem, *_bndT)
                 ? bndT
                 : bndpol() + _bndrem;
    }
  }
  ...
  template <typename U>
  U
  bound(std::map<KEY, std::vector<U>, COMP> const& bndbasis,
        U const& bndrem) const
  {
    return _polybound(bndbasis) + bndrem;
  }

  //! @brief Retreive bound on variable using bounder <a>type</a> in U
  //! arithmetic
  template <typename U>
  U
  bound(std::map<KEY, std::vector<U>, COMP> const& bndbasis, U const& bndrem,
  ...
  //! @brief Retreive bound on all terms with (total) order <tt>minord</tt> or
  //! higher in polynomial part
  T
  bndord(const unsigned minord) const
  {
    if (!_CM)
      return !minord && !_coefmon.empty() && !_coefmon.begin()->first.tord
                 ? _coefmon.begin()->second
                 : 0.;
    return _CM->_polybound_naive(_coefmon,
                                 std::map<KEY, std::vector<T>, COMP>(), minord);
  }
  ...
  double P(std::map<KEY, double, COMP> const& x) const;

  //! @brief Return new Chebyshev variable with same multivariate polynomial
  //! part but zero remainder
  SCVar<T, KEY, COMP>
  P() const
  {
    SCVar<T, KEY, COMP> var(*this);
    var._bndrem = 0.;
    return var;
  }
  ...
  //! @brief Get coefficient of constant term in Chebyshev variable. The value
  //! of this coefficient is reset to 0 if <tt>reset=true</tt>, otherwise it is
  //! left unmodified (default).
  double constant(bool const reset = false);

  //! @brief Get coefficients of linear term for variable <tt>id</tt> in
  //! Chebyshev variable. The value of this coefficient is reset to 0 if
  //! <tt>reset=true</tt>, otherwise it is left unmodified (default).
  double linear(KEY const& id, bool const reset = false);

  //! @brief Lift the model by appending new vauxiliary ariable corresponding to
  //! the remainder term if magnitude greater than TOL
  SCVar<T, KEY, COMP>& lift(SCModel<T, KEY, COMP>* SCM, double const& ATOL,
                            double const& RTOL);

  //! @brief Project the model by removing monomials with auxiliary variables
  //! participating
  SCVar<T, KEY, COMP>& project(bool const reset = false);

  //! @brief Project the model by removing monomials with variable id
  //! participating
  SCVar<T, KEY, COMP>& project(KEY const& id);

  //! @brief Simplify the model by appending coefficient less than TOL as well
  //! as terms with order greater than or equal to TORD to the remainder term
  SCVar<T, KEY, COMP>& simplify(double const& ATOL = 0e0,
                                double const& RTOL = 0e0, int const TORD = -1);

  //! @brief Scale coefficients in Chebyshev variable for the modified domain
  //! <a>dom</a> of variable id
  SCVar<T, KEY, COMP>& scale(KEY const& id, T const& dom);

  //! @brief Scale coefficients in Chebyshev variable <a>id</a> for the modified
  //! variables domain <a>dom</a>
  SCVar<T, KEY, COMP>& scale(std::map<KEY, T, COMP> const& dom);

  //! @brief Unscale coefficients in Chebyshev variable for their original
  //! variable ranges
  t_poly unscale();

  //! @brief Return new coefficient map in monomial basis representation
  t_poly to_monomial(bool const scaled = false) const;

  //! @brief Return new coefficient map in monomial basis representation after
  //! removing terms with coefficient less than TOL or order greater than or
  //! equal to ORD, and also return a bound on the removed terms
  std::pair<t_poly, T> to_monomial(bool const scaled, double const& ATOL,
                                   double const& RTOL,
                                   int const TORD = -1) const;
  ...
  T bound_bernstein(t_poly const& coefbern) const;
  /** @} */

  SCVar<T, KEY, COMP>& operator=(SCVar<T, KEY, COMP> const&);
  SCVar<T, KEY, COMP>& operator=(SCVar<T, KEY, COMP>&&);
  SCVar<T, KEY, COMP>& operator=(double const&);
  SCVar<T, KEY, COMP>& operator=(T const&);
  template <typename U>
  SCVar<T, KEY, COMP>& operator+=(SCVar<U, KEY, COMP> const&);
  template <typename U>
  SCVar<T, KEY, COMP>& operator+=(U const&);
  SCVar<T, KEY, COMP>& operator+=(double const&);
  template <typename U>
  SCVar<T, KEY, COMP>& operator-=(const SCVar<U, KEY, COMP>&);
  template <typename U>
  SCVar<T, KEY, COMP>& operator-=(U const&);
  SCVar<T, KEY, COMP>& operator-=(double const&);
  SCVar<T, KEY, COMP>& operator*=(const SCVar<T, KEY, COMP>&);
  SCVar<T, KEY, COMP>& operator*=(double const&);
  SCVar<T, KEY, COMP>& operator*=(T const&);
  SCVar<T, KEY, COMP>& operator/=(SCVar<T, KEY, COMP> const&);
  SCVar<T, KEY, COMP>& operator/=(double const&);
 ...
 private:
  //! @brief Polynomial range bounder using specified bounder <a>type</a> with
  //! basis functions <a>bndbasis</a> in U arithmetic
  template <typename U>
  U
  _polybound(std::map<KEY, std::vector<U>, COMP> const& bndbasis,
             int const type) const
  {
    if (!_CM)
      return !_coefmon.empty() && !_coefmon.begin()->first.tord
                 ? _coefmon.begin()->second
                 : 0.;
  ...
  template <typename U>
  U
  _polybound(std::map<KEY, std::vector<U>, COMP> const& bndbasis) const
  {
  ...
  T
  _polybound(int const type) const
  {
  ...
  //! @brief Scale current variable in order for its range to be within [-1,1],
  //! with <a>c</a> and <a>w</a> respectively the center and width,
  //! respectively, of the orginal variable range
  SCVar<T, KEY, COMP>
  _rescale(double const& w, double const& c) const
  {
#ifdef MC__SCMODEL_TRACE
    std::cerr
        << "-- SCVar<T,KEY,COMP>& _rescale( double const&, double const& )\n";
#endif
  ...
  template <typename U>
  static U
  _rescale(U& var, double const& w, double const& c)
  {
    return (!isequal(w, 0.) ? (var - c) / w : c);
  }

  //! @brief Cancel zero entries in coefficient map <a>coefmon</a>
  void _simplify(t_poly& coefmon) const;

  //! @brief Simplify the monomial itmon by appending coefficient less than TOL
  //! as well as terms with order greater than or equal to TORD to the remainder
  //! term
  SCVar<T, KEY, COMP>& _simplify(
      typename SCVar<T, KEY, COMP>::t_poly::iterator itmon, double const& ATOL,
      double const& RTOL, int const TORD = -1);

  //! @brief Simplify the coefficient map <a>coefmon</a> by removing entries
  //! with coefficient less than TOL or order greater than or equal to ORD, and
  //! return a bound on the removed entries
  T _simplify_monomial(t_poly& coefmon, bool const scaled, double const& ATOL,
                       double const& RTOL, int const TORD = -1) const;

  //! @brief Scale coefficient map <a>coefmon</a> for the modified range
  //! <a>bnd</a> of variable id
  void _scale(typename t_var::const_iterator itvar, T const& bndvar,
              t_poly& coefmon) const;

  //! @brief Convert Chebyshev basis into monomial basis for variable
  //! <a>ivar</a> in coefficient map <a>coefmon</a>
  void _to_monomial(KEY const& id, t_poly& coefmon) const;
};
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_set(KEY const& id, T const& X)
{
  _setvar.insert(id);
  _bndvar[id]  = X;
...
template <typename U>
inline std::vector<U>&
SCModel<T, KEY, COMP>::_get_bndpow(unsigned const maxord, U const& bndvar,
                                   double const& refvar, double const& scalvar,
                                   std::vector<U>& bndpow) const
{
  ...
  for (unsigned i = 2; i <= maxord; i++) switch (options.BASIS)
    {
      case Options::MONOM:
        bndpow[i] = Op<U>::pow(bndpow[1], i);
        break;
      case Options::CHEB:
        bndpow[i] = Op<U>::cheb(bndpow[1], i);
        break;
    }
  return bndpow;
}
...
template <typename T, typename KEY, typename COMP>
template <typename U>
inline std::map<KEY, std::vector<U>, COMP>&
SCModel<T, KEY, COMP>::get_basis(unsigned const maxord,
                                 std::map<KEY, U, COMP> const& bndvar,
                                 std::map<KEY, std::vector<U>, COMP>& bndbasis,
                                 bool const scaled) const
{
  bndbasis.clear();
  for (auto const& id : _setvar)
  {
    bndbasis[id].clear();
    if (scaled)
      _get_bndpow(maxord, bndvar[id], bndbasis[id]);
    else
      _get_bndpow(maxord, bndvar[id], _refvar[id], _scalvar[id]);
  }
  return bndbasis;
}
...
template <typename U>
inline void
SCModel<T, KEY, COMP>::get_bndmon(std::map<t_mon, U, lt_mon>& bndmon,
                                  std::map<KEY, U, COMP> const& bndvar,
                                  bool const scaled, bool const useprod) const
{
  ...
  if (useprod)
  {
    ...
    for (; it != bndmon.end(); ++it)
    {
      ...
      if (nmon == 1)
        it->second = Umon.front();
      else
        it->second = Op<U>::prod(nmon, Umon.data());
    }
  }
...
template <typename PUNIV>
inline double
SCModel<T, KEY, COMP>::_minimax(PUNIV const& f)
{
  // 1. Resize Armadillo Matrices and Vector
  unsigned const n_points = _maxord + 2;
  ...
  _coefuniv.assign(_maxord + 1, 0.0);
  double cur_err     = 0.0;
  ...
  {
    if (_maxord == 0) return _coefuniv[0];
    if (_maxord == 1) return _coefuniv[0] + _coefuniv[1] * x;

    double b_k, b_kp1 = 0.0, b_kp2 = 0.0;
    double x2 = 2.0 * x;

    for (int k = _maxord; k >= 1; --k)
    {
      b_k   = _coefuniv[k] + x2 * b_kp1 - b_kp2;
      b_kp2 = b_kp1;
      b_kp1 = b_k;
    }
    return _coefuniv[0] + x * b_kp1 - b_kp2;
  };
  ...
  for (unsigned iter = 0; iter < options.REMEZ_MAXIT; ++iter)
  {
    ...
    for (unsigned i = 0; i < n_points; ++i)
    {
      ...
      if (_maxord >= 1) _minimax_mat(i, 1) = x;
      for (unsigned j = 2; j <= _maxord; ++j)
        _minimax_mat(i, j) =
            2.0 * x * _minimax_mat(i, j - 1) - _minimax_mat(i, j - 2);
    ...
    try
    {
      ...
      for (unsigned j = 0; j <= _maxord; ++j) _coefuniv[j] = _minimax_sol(j);
      cur_err = _minimax_sol(n_points - 1);
    }
    catch (...)
    {
      // Solver failed (singularity). Fallback to standard Chebyshev
      // Interpolation. _chebinterp populates the member _coefuniv directly with
      // valid coefficients.
      ...
      for (unsigned i = _maxord + 1; i <= nord; i++)
        max_abs_err += std::fabs(_coefuniv[i]);
      // Exit the Remez loop
      break;
    }

    // 5. Grid Search for Global Max Error
    unsigned const n_grid  = 20 * _maxord + 200;
    ...
    // 6. Convergence Check
    if (std::abs(max_abs_err - std::abs(cur_err)) < options.REMEZ_TOL) break;
    ...
    if (remove_idx == -1)
      remove_idx =
          (std::abs(errs[0]) < std::abs(errs[n_points])) ? 0 : n_points;
  ...
  if (options.BASIS != Options::CHEB)
  {
    ...
    arma::vec y_vals(_maxord + 1);
    arma::mat V(_maxord + 1, _maxord + 1);

    for (unsigned i = 0; i <= _maxord; ++i)
    {
      double const x = std::cos(PI * double(i) / double(_maxord));
      y_vals(i)      = eval_chebyshev(x);

      double xp = 1.0;
      for (unsigned j = 0; j <= _maxord; ++j)
      {
        V(i, j) = xp;
        xp *= x;
      }
    ...
    try
    {
      ...
      for (unsigned j = 0; j <= _maxord; ++j) _coefuniv[j] = c_mon(j);
    }
    catch (...)
    {
      _coefuniv.assign(_maxord + 1, 0.0);
...
template <typename PUNIV>
inline double
SCModel<T, KEY, COMP>::_minimax(PUNIV const& f)  // range of f assumed as [-1,1]
{
  boost::math::tools::remez_minimax<double> problem(f, _maxord, 0, -1., 1.,
                                                    false, false, 0, 64);
  ...
  for (unsigned iter = 0; iter < options.REMEZ_MAXIT; ++iter)
  {
    ...
    std::cout << iter << ": [ " << std::right << std::scientific
              << std::setprecision(15);
...
#ifdef MC__SCMODEL_DEBUG_MINIMAX
  std::cout << arma::vec(problem.numerator().data()).t();
#endif
...
template <typename VEC>
inline VEC
SCModel<T, KEY, COMP>::_to_chebyshev(VEC const& veccoef) const
{
  ...
  for (int J = N - 2; J >= 0; --J)
  {
    ...
    for (int I = J; I < N - 1; ++I) vecnew[I] += vecnew[I + 2];
  }
  return vecnew;
}
...
template <typename PUNIV>
inline bool
SCModel<T, KEY, COMP>::_minimax(PUNIV const& f, SCVar<T, KEY, COMP> const& CV,
                                SCVar<T, KEY, COMP>& CV2)
{
  ...
  try
  {
    double m(Op<SCVar<T, KEY, COMP>>::mid(CV)),
        r(0.5 * Op<SCVar<T, KEY, COMP>>::diam(CV));
    ...
    default:
      for (unsigned i = 0; i <= maxord; i++)
      {
        double mulconst2(std::cos(mulconst * double(i))),
            mulconst3(4 * mulconst2 * mulconst2 - 2), b0(0), b1(0);
        b0 = _finterp[maxord];
...
inline void
SCModel<T, KEY, COMP>::_chebinterp(
    PUNIV const& f, double const& tol,
    unsigned& maxord)  // range of f assumed as [-1,1]
{
  _chebinterp(f, maxord);
  for (; std::fabs(_coefuniv[maxord]) > tol ||
         (maxord && std::fabs(_coefuniv[maxord - 1]) > tol);)
  {
    maxord *= 2;
    _chebinterp(f, maxord);
...
SCModel<T, KEY, COMP>::_horner(SCVar<T, KEY, COMP>&& CVinner,
                               unsigned const maxord)
{
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
SCModel<T, KEY, COMP>::_clenshaw(SCVar<T, KEY, COMP>&& CVinner,
                                 unsigned const maxord)
{
  assert(maxord < _coefuniv.size());
  ...
  if (!maxord)
    ...
    // http://en.wikipedia.org/wiki/Clenshaw_algorithm#Special_case_for_Chebyshev_series
#ifdef MC__SCMODEL_DEBUG_COMPOSITION
  ...
  for (unsigned i = maxord - 2; i > 1; i -= 2)
  {
    ...
    std::cout << "_coefuniv[i] + CVinnerx2 * CV2"
              << _coefuniv[i] + CVinnerx2 * CV2;
#endif
...
template <typename PUNIV>
inline bool
SCModel<T, KEY, COMP>::_chebinterp(PUNIV const& f,
                                   ...
                                   bool const rematbound)
{
  ...
  {
    double m(Op<SCVar<T, KEY, COMP>>::mid(CV)),
        r(0.5 * Op<SCVar<T, KEY, COMP>>::diam(CV)), rem(0.);
...
template <typename U>
inline SCVar<T, KEY, COMP>
SCModel<T, KEY, COMP>::_composition(std::vector<U> const& coefouter,
                                    unsigned const maxord,
                                    SCVar<T, KEY, COMP> const& CVinner)
{
  // composition based on
  ...
  for (unsigned i = maxord - 2; i > 1; i -= 2)
  {
    ...
    std::cout << "coefouter[i] + CVinnerx2 * CV2"
              << coefouter[i] + CVinnerx2 * CV2;
#endif
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
SCModel<T, KEY, COMP>::_intpow(SCVar<T, KEY, COMP> const& CV, int const n) const
{
  if (n == 0)
    return 1.;
  else if (n == 1)
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_sscal1D(t_poly const& coefmon0, double const& dscal,
                                t_poly& coefmon) const
{
  if (isequal(dscal, 0.)) return;
  coefmon = coefmon0;
  if (isequal(dscal, 1.)) return;
  for (auto& [mon, coef] : coefmon) coef *= dscal;
  return;
}

template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_slift1D(t_poly const& coefmon0, double const& dscal,
                                t_poly& coefmon) const
{
  if (isequal(dscal, 0.)) return;
                                ...
                                unsigned const ndxord) const
{
  for (auto const& [mon0, coef0] : coefmon0)
  {
    ...
    {  // append to remainder coefficient if total order too large
      switch (options.BASIS)
      {
        case Options::CHEB:
          rem += (coef0 * dscal) * TOne;
          continue;
        case Options::MONOM:
          rem += (coef0 * dscal) *
                 (ndxord % 2 || mon0.gcexp() % 2 ? TOne : TZeroOne);
          continue;
      }
    }
    auto mon = mon0;  // local copy for modification
#ifdef MC__SCMODEL_DEBUG_SPROD
    std::cout << "mon: " << mon.display(options.BASIS) << "  (" << *itvar << ","
              << ndxord << ")" << std::endl;
#endif
    mon.tord += ndxord;
...
inline void
SCModel<T, KEY, COMP>::_slift1D(t_poly const& coefmon0, double const& dscal,
                                T& rem, unsigned const ndxord) const
{
  for (auto const& [mon0, coef0] : coefmon0) switch (options.BASIS)
    {
      case Options::CHEB:
        rem += (coef0 * dscal) * TOne;
        continue;
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_sdisp1D(t_poly const& coefmon, std::string const& name,
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_sdisp1D(std::map<unsigned, t_poly> const& coefmap,
                                typename t_var::const_iterator itvar,
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_sdisp1Dfull(std::vector<SCVar<T, KEY, COMP>> const& vec,
                                    typename t_var::const_iterator itvar,
                                    std::string const& name,
                                    std::ostream& os) const
{
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_sprod1D(std::map<unsigned, t_poly> const& sp1map,
                                ...
                                typename t_var::const_iterator itvar) const
{
  ...
  for (auto& [ndx1, coefmon1] : sp1map)
  {
    ...
    if (coefmon1.size() == 1 && !coefmon1.begin()->first.tord)
    {
      ...
      {
        auto [it12, ins] = sp12map.insert(
            std::make_pair(std::make_pair(ndx1, ndx2), t_poly()));
    ...
    for (auto& [ndx2, coefmon2] : sp2map)
    {
      ...
      for (auto& mon1 : coefmon1) _svec1D(itvarnext, mon1, sp11map);
#ifdef MC__SCMODEL_DEBUG_SPROD
      _sdisp1D(sp11map, itvarnext, "Poly #1: ");
#endif
      for (auto& mon2 : coefmon2) _svec1D(itvarnext, mon2, sp22map);
#ifdef MC__SCMODEL_DEBUG_SPROD
      _sdisp1D(sp22map, itvarnext, "Poly #2: ");
#endif
      auto [it12, ins] =
          sp12map.insert(std::make_pair(std::make_pair(ndx1, ndx2), t_poly()));
      assert(ins);  // map is initially empty
  ...
  for (auto const& [ndx12, coefmon12] : sp12map)
  {
    ...
    if (!ndx1 && !ndx2) _slift1D(coefmon12, 1., coefmon);
...
template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_svec1D(typename t_var::const_iterator itvar,
                               std::pair<t_mon, double> const& coefmon,
                               std::map<unsigned, t_poly>& mapspoly) const
{
  auto const& [ivar, iord] = *coefmon.first.expr.begin();
  if (!coefmon.first.tord ||
      ivar != *itvar)  // no dependence on variable *itvar
    mapspoly[0].insert(coefmon);
  else  // dependence on variable *itvar of order iord
    mapspoly[iord].insert(std::make_pair(
        t_mon(coefmon.first.tord - iord,
              std::map<KEY, unsigned, COMP>(++coefmon.first.expr.begin(),
                                            coefmon.first.expr.end())),
        coefmon.second));
}

template <typename T, typename KEY, typename COMP>
inline void
SCModel<T, KEY, COMP>::_svec1Dfull(typename t_var::const_iterator itvar,
                                   std::pair<t_mon, double> const& coefmon,
                                   std::vector<SCVar<T, KEY, COMP>>& vec) const
{
  ...
  {  // dependence on variable *itvar of order iord
    auto& [ivar, iord] = *ie;
...
template <typename T, typename KEY, typename COMP>
template <typename C>
inline std::pair<std::map<typename SCModel<T, KEY, COMP>::t_mon, C,
                          typename SCModel<T, KEY, COMP>::lt_mon>,
                 std::map<KEY, unsigned, COMP>>
SCModel<T, KEY, COMP>::to_bernstein(std::map<t_mon, C, lt_mon> const& coefmon,
                                    unsigned const ORDER) const
{
  ...
  auto elevate_to_target = [&](arma::vec const& src,
                               unsigned target_N) -> arma::vec
  {
    arma::vec dest = src;
    while (dest.n_elem < target_N + 1) dest = elevate_one_degree(dest);
    return dest;
  };
  ...
  for (auto& [var, max_deg] : degreemax)
  {
    ...
    // Elevate cached natural basis vectors to the specific target degree
  ...
  for (auto const& [var_id, max_deg] : degreemax)
  {
    ...
    for (auto const& [mon, val] : coefbern)
    {
      ...
      for (unsigned j = 0; j < b_poly.n_elem; ++j)
      {
        ...
        t_mon new_mon = mon;
        new_mon.expr[var_id] =
            j;  // This now represents Bernstein index, not degree
        new_mon.tord = mon.tord - k + j;  // Update total order heuristic

        // Efficient map insertion/update
        auto [it, ins] =
            coefnext.insert(std::make_pair(new_mon, val * b_poly[j]));
...
template <typename C, typename U>
inline U
SCModel<T, KEY, COMP>::_polybound_bernstein(
    std::map<t_mon, C, lt_mon> const& coefmon,
    std::map<KEY, std::vector<U>, COMP> const& bndbasis) const
{
  ...
  // Transfom to Bernstein basis
  auto const& [coefbern, degreemax] =
      to_bernstein(coefmon, options.BERNSTEIN_ORDER);

  // Compute Berstein coefficient hull
  bool first = true;
  U bound_hull(0.);

  for (auto const& term : coefbern)
  {
    // term.second is type C (double or Interval), implicitly convertible to U
    if (first)
    {
    ...
    std::map<KEY, std::vector<U>, COMP> const& bndbasis) const
{
  ...
  std::map<t_mon, C, lt_mon> coeflin;
  coeflin.insert(it1, it2);
  ...
  for (; it2 != it3; ++it2)
  {
    ...
    {  // off-diagonal quadratic terms
      bndpol +=
          it2->second * (!bndbasis.empty() ? bndbasis.at(ie2->first)[1] *
                                                 bndbasis.at((++ie2)->first)[1]
                                           : TOne);
      continue;
    }
    ...
    switch (options.BASIS)
    {
      case Options::CHEB:
        if (it1 != coeflin.end() && std::fabs(it2->second) > TOL)
        {
          ...
          bndpol += (2. * aii) * Op<U>::sqr((!bndbasis.empty()
                                                 ? bndbasis.at(ie2->first)[1]
                                                 : TOne) +
                                            ai / (aii * 4.)) -
                    aii - ai * ai / 8. / aii;
        ...
        else if (it1 != coeflin.end())
        {
          bndpol +=
              it2->second *
                  (!bndbasis.empty() ? bndbasis.at(ie2->first)[2] : TOne) +
              it1->second *
                  (!bndbasis.empty() ? bndbasis.at(ie2->first)[1] : TOne);
          coeflin.erase(it1);
        }
        else
          bndpol += it2->second *
      ...
      case Options::MONOM:
        if (it1 != coeflin.end() && std::fabs(it2->second) > TOL)
        {
          ...
          double const& aii = it2->second;
          bndpol +=
              aii * Op<U>::sqr((!bndbasis.empty() ? bndbasis.at(ie2->first)[1]
                                                  : TOne) +
                               ai / (2. * aii)) -
              ai * ai / (aii * 4.);
          coeflin.erase(it1);
        ...
        else if (it1 != coeflin.end())
        {
          bndpol +=
              it2->second *
                  (!bndbasis.empty() ? bndbasis.at(ie2->first)[2] : TZeroOne) +
              it1->second *
                  (!bndbasis.empty() ? bndbasis.at(ie2->first)[1] : TOne);
          coeflin.erase(it1);
        }
  ...
  for (it1 = coeflin.begin(); it1 != coeflin.end(); ++it1)
  {
    ...
    bndpol +=
        it1->second * (!bndbasis.empty() ? bndbasis.at(ie1->first)[1] : TOne);
  }
  ...
  if (bndbasis.empty())
  {
    switch (options.BASIS)
    {
      case Options::CHEB:
      {
        ...
        for (; it != coefmon.end(); ++it) bndcoef += std::fabs(it->second);
...
SCModel<T, KEY, COMP>::_polybound(
    std::map<t_mon, C, lt_mon> const& coefmon,
    std::map<KEY, std::vector<U>, COMP> const& bndbasis, int const type) const
{
  switch (type)
  {
    case Options::LSB:
...
template <typename T, typename KEY, typename COMP>
inline T
SCModel<T, KEY, COMP>::_monbound(t_mon const& mon) const
{
...
template <typename T, typename KEY, typename COMP>
inline double
SCModel<T, KEY, COMP>::_monval(t_mon const& mon,
                               std::map<KEY, double, COMP> const& x) const
{
#if 0
...
#endif
    if (_setaux.find(id) != _setaux.end())
      return 0e0;  // case an auxiliary is encoutered
    switch (options.BASIS)
    {
      case Options::CHEB:
        monval *=
            isequal(_scalvar.at(id), 0.)
                ? _refvar.at(id)
                : mc::cheb((x.at(id) - _refvar.at(id)) / _scalvar.at(id), ord);
        break;
      case Options::MONOM:
        monval *= isequal(_scalvar.at(id), 0.)
                      ? _refvar.at(id)
                      : std::pow((x.at(id) - _refvar.at(id)) / _scalvar.at(id),
                                 (int)ord);
        break;
    }
  }
...
template <typename T, typename KEY, typename COMP>
inline void
SCVar<T, KEY, COMP>::_set_bndpol(T const* bndpol)
{
  if (!bndpol)
  {
...
template <typename T, typename KEY, typename COMP>
inline void
SCVar<T, KEY, COMP>::_set_bndT(T const* bndT)
{
  if (!bndT)
  {
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>::SCVar(SCVar<T, KEY, COMP> const& CV) : _CM(nullptr)
{
#ifdef MC__SCMODEL_TRACE
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator=(SCVar<T, KEY, COMP> const& CV)
{
#ifdef MC__SCMODEL_TRACE
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>::SCVar(SCVar<T, KEY, COMP>&& CV) : _CM(nullptr)
{
#ifdef MC__SCMODEL_TRACE
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator=(SCVar<T, KEY, COMP>&& CV)
{
#ifdef MC__SCMODEL_TRACE
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>::SCVar(SCModel<T, KEY, COMP>* CM) : _CM(CM)
{
  _init();
  _set_bndpol(0.);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>::SCVar(double const& d, SCModel<T, KEY, COMP>* CM)
    : _CM(CM)
{
  _init();
  if (d == 0.) return;
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator=(double const& d)
{
  _reinit();
  _CM = nullptr;
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>::SCVar(T const& B, SCModel<T, KEY, COMP>* CM)
    : _CM(CM)
{
  _init();
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator=(T const& B)
{
  _reinit();
  _CM               = nullptr;
  double const midB = Op<T>::mid(B);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>::SCVar(SCModel<T, KEY, COMP>* CM, KEY const& id,
                                  T const& dom)
    : _CM(CM)
{
  _init();
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::_set(KEY const& id, T const& dom, bool const updt)
{
  if (!_CM)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::INIT);
  ...
  if (_CM->options.MIG_USE)
    simplify(_CM->options.MIG_ATOL, _CM->options.MIG_RTOL);
#if 0
  std::cout << "coefmon size: " << _coefmon.size() << std::endl;
#endif

  // Interval bounds
  if (_CM->options.MIXED_IA)
    _set_bndT(_bndvar(id));
  else
    _unset_bndT();
  return *this;
}
...
template <typename T, typename KEY, typename COMP>
inline double
SCVar<T, KEY, COMP>::P(std::map<KEY, double, COMP> const& x) const
{
...
template <typename T, typename KEY, typename COMP>
inline double
SCVar<T, KEY, COMP>::constant(bool const reset)
{
  auto it_0 =
      (_coefmon.empty() || _coefmon.begin()->first.tord ? _coefmon.end()
                                                        : _coefmon.begin());
...
template <typename T, typename KEY, typename COMP>
inline double
SCVar<T, KEY, COMP>::linear(KEY const& id, bool const reset)
{
  auto it_i =
      (!nord() || _coefmon.empty() ? _coefmon.end() : _coefmon.find(t_mon(id)));
  double coeflin = (it_i == _coefmon.end() || isequal(_scalvar(id), 0.)
                        ? 0.
                        : it_i->second / _scalvar(id));
...
template <typename T, typename KEY, typename COMP>
inline void
SCVar<T, KEY, COMP>::_scale(typename t_var::const_iterator itvar,
                            T const& bndvar, t_poly& coefmon) const
{
  // Nothing to do if model _CM is nullptr, or variable range X did not change
  if (!_CM || (isequal(Op<T>::l(bndvar), Op<T>::l(_bndvar(*itvar))) &&
               isequal(Op<T>::u(bndvar), Op<T>::u(_bndvar(*itvar)))))
    return;

  // Get coefficients in univariate polynomial representation w.r.t variable
  // *itvar
  std::vector<SCVar<T, KEY, COMP>> veccoef(nord() + 1,
                                           SCVar<T, KEY, COMP>(_CM));
  ...
  SCVar<T, KEY, COMP> cvvar(_CM);
  cvvar._set(*itvar, bndvar, false);
#ifdef MC__SCMODEL_DEBUG_SCALE
...
SCVar<T, KEY, COMP>::scale(std::map<KEY, T, COMP> const& dom)
{
  ...
  if (_CM && _CM->options.MIG_USE)
    simplify(_CM->options.MIG_ATOL, _CM->options.MIG_RTOL);
  return *this;
}

template <typename T, typename KEY, typename COMP>
inline typename SCVar<T, KEY, COMP>::t_poly
                          ...
                          double const& RTOL)
{
  ...
  std::cout << remrad << " < " << 0.5 * RTOL * Op<T>::diam(_polybound()) + ATOL
            << " : " << (remrad < 0.5 * RTOL * Op<T>::diam(_polybound()) + ATOL)
            << std::endl;
#endif
  if (!_CM->_maxord ||
      (remrad < 0.5 * RTOL * Op<T>::diam(_polybound()) + ATOL + DBL_EPSILON))
    return *this;
  _center();
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::project(bool const reset)
{
  // Nothing to do
...
SCVar<T, KEY, COMP>::project(KEY const& id)
{
  ...
  for (auto it = _coefmon.begin(); it != _coefmon.end();)
  {
    ...
    if (itaux == mon.expr.end())
    {
      ++it;
      continue;
    }
    t_mon const monaux(itaux->first, itaux->second);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::simplify(double const& ATOL, double const& RTOL,
                              int const TORD)
{
  ...
  for (auto it = _coefmon.begin(); it != _coefmon.end();)
  {
    ...
    // Eliminate any non-constant terms with small enough coefficient or large
    // enough total order
    double const THRES =
        (RTOL > 0 ? 0.5 * RTOL * Op<T>::diam(_polybound()) : 0e0) + ATOL;
    if ((mon.tord && std::fabs(coef) <= THRES) ||
...
SCVar<T, KEY, COMP>::_simplify_monomial(
    ...
    double const& ATOL, double const& RTOL, int const TORD) const
{
  T bndrem(0e0);
  ...
  for (auto it = coefmon.begin(); it != coefmon.end();)
  {
    ...
    double const THRES =
        (RTOL > 0 ? 0.5 * RTOL * Op<T>::diam(_polybound()) : 0e0) + ATOL;
    if ((mon.tord && THRES > 0e0 && std::fabs(coef) <= THRES) ||
        (TORD >= 0 && (int)mon.tord > TORD))
    {
      // compute monomial bound
      if (scaled)
        bndrem += coef * _CM->_monbound(mon);
      else
      {
        T bndmon(1e0);
...
template <typename T, typename KEY, typename COMP>
inline void
SCVar<T, KEY, COMP>::_simplify(t_poly& coefmon) const
{
...
SCVar<T, KEY, COMP>::_to_monomial(
    KEY const& id, typename SCVar<T, KEY, COMP>::t_poly& coefmon) const
{
...
template <typename T, typename KEY, typename COMP>
inline typename SCVar<T, KEY, COMP>::t_poly
SCVar<T, KEY, COMP>::to_monomial(bool const scaled) const
{
...
template <typename T, typename KEY, typename COMP>
inline std::pair<typename SCVar<T, KEY, COMP>::t_poly, T>
SCVar<T, KEY, COMP>::to_monomial(bool const scaled, double const& ATOL,
                                 double const& RTOL, int const TORD) const
{
...
template <typename T, typename KEY, typename COMP>
inline std::pair<typename SCVar<T, KEY, COMP>::t_poly,
                 std::map<KEY, unsigned, COMP>>
SCVar<T, KEY, COMP>::to_bernstein(unsigned const ORDER) const
{
...
template <typename T, typename KEY, typename COMP>
inline T
SCVar<T, KEY, COMP>::bound_bernstein(t_poly const& coefbern) const
{
  // Compute Berstein coefficient hull
  bool first = true;
  T bound_hull(0.);
  ...
  return (bound_hull + _bndrem);
}

template <typename T, typename KEY, typename COMP>
inline std::string
SCVar<T, KEY, COMP>::display(t_poly coefmon, int const& BASIS, int const& IDISP)
{
  ...
  for (auto const& [mon, coef] : coefmon)
    out << std::right << std::setw(IDISP + 7) << coef << "  " << std::setw(2)
        << mon.tord << "  " << mon.display(BASIS) << std::endl;
...
template <typename T, typename KEY, typename COMP>
inline std::ostream&
operator<<(std::ostream& out, SCVar<T, KEY, COMP> const& CV)
{
  unsigned IDISP = CV._CM ? CV._CM->options.DISPLAY_DIGITS : 7;
  int BASIS =
      CV._CM ? CV._CM->options.BASIS : SCModel<T, KEY, COMP>::Options::CHEB;
  out << CV.display(CV._coefmon, BASIS, IDISP) << std::scientific
      << std::setprecision(IDISP) << std::right;

  // Remainder term
  out << std::right << "   R     =  "
      << "[" << std::setw(IDISP + 7) << Op<T>::l(CV._bndrem) << ","
      << std::setw(IDISP + 7) << Op<T>::u(CV._bndrem) << "]" << std::endl;

  // Range bounder
  out << std::right << "   B     =  "
      << "[" << std::setw(IDISP + 7) << Op<T>::l(CV.B()) << ","
      << std::setw(IDISP + 7) << Op<T>::u(CV.B()) << "]" << std::endl;
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator+(SCVar<T, KEY, COMP> const& CV)
{
  return CV;
}
...
template <typename U>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator+=(SCVar<U, KEY, COMP> const& CV)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr
      << "-- SCVar<T,KEY,COMP>& operator+= ( SCVar<T,KEY,COMP> const& )\n";
  ...
  if (_CM && CV._CM && _CM != CV._CM)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::MODEL);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator+(SCVar<T, KEY, COMP> const& CV1, SCVar<T, KEY, COMP> const& CV2)
{
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(SCVar<T, KEY, COMP>&& CV1, SCVar<T, KEY, COMP> const& CV2)
{
  return (std::move(CV1 += CV2));
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(SCVar<T, KEY, COMP> const& CV1, SCVar<T, KEY, COMP>&& CV2)
{
  return (std::move(CV2 += CV1));
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(SCVar<T, KEY, COMP>&& CV1, SCVar<T, KEY, COMP>&& CV2)
{
  return (CV1.nmon() >= CV2.nmon() ? std::move(CV1 += CV2)
                                   : std::move(CV2 += CV1));
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator+=(double const& c)
{
#ifdef MC__SCMODEL_TRACE
  ...
  if (!ins)
  {
    itcst->second += c;
    if (_CM && _CM->options.MIG_USE)
      _simplify(itcst, _CM->options.MIG_ATOL, _CM->options.MIG_RTOL);
  }
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator+(SCVar<T, KEY, COMP> const& CV1, double const& c)
{
  SCVar<T, KEY, COMP> CV3(CV1);
  return (CV3 += c);
}
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator+(double const& c, SCVar<T, KEY, COMP> const& CV2)
{
  SCVar<T, KEY, COMP> CV3(CV2);
  return (CV3 += c);
}
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(SCVar<T, KEY, COMP>&& CV1, double const& c)
{
  return (std::move(CV1 += c));
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(double const& c, SCVar<T, KEY, COMP>&& CV2)
{
  return (std::move(CV2 += c));
}

template <typename T, typename KEY, typename COMP>
template <typename U>
inline SCVar<T, KEY, COMP>&
SCVar<T, KEY, COMP>::operator+=(U const& B)
{
#ifdef MC__SCMODEL_TRACE
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator+(SCVar<T, KEY, COMP> const& CV1, T const& B)
{
  SCVar<T, KEY, COMP> CV3(CV1);
  return (CV3 += B);
}
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator+(T const& B, SCVar<T, KEY, COMP> const& CV2)
{
  SCVar<T, KEY, COMP> CV3(CV2);
  return (CV3 += B);
}
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(SCVar<T, KEY, COMP>&& CV1, T const& B)
{
  return (std::move(CV1 += B));
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
operator+(T const& B, SCVar<T, KEY, COMP>&& CV2)
{
  return (std::move(CV2 += B));
}
...
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> neg()\n";
#endif

  for (auto& [mon, coef] : _coefmon) coef *= -1;
  _bndrem *= -1;
...
SCVar<T, KEY, COMP>::operator*=(SCVar<T, KEY, COMP> const& CV)
{
  ...
  if (_CM && _CM->options.MIG_USE)
    simplify(_CM->options.MIG_ATOL, _CM->options.MIG_RTOL);
  if (_CM && _CM->options.LIFT_USE)
    lift(_CM, _CM->options.LIFT_ATOL, _CM->options.LIFT_RTOL);
...
sqr(SCVar<T, KEY, COMP> const& CV)
{
  ...
  std::cout << "Product of uncertainty parts:"
            << 2. * CV._polybound() * CV._bndrem + Op<T>::sqr(CV._bndrem)
            << std::endl;
#endif
  ...
  if (CV._CM->options.MIG_USE)
    CVSQR.simplify(CVSQR._CM->options.MIG_ATOL, CVSQR._CM->options.MIG_RTOL);
  if (CV._CM->options.LIFT_USE)
    CVSQR.lift(CVSQR._CM, CVSQR._CM->options.LIFT_ATOL,
               CVSQR._CM->options.LIFT_RTOL);
...
SCVar<T, KEY, COMP>::operator/=(double const& c)
{
  if (isequal(c, 0.))
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::DIV);
  if (c == 1.) return *this;
  *this *= (1. / c);
  return *this;
}
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
operator/(double const& c, SCVar<T, KEY, COMP> const& CV)
{
  if (c == 0.) return 0.;
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
inv(SCVar<T, KEY, COMP> const& CV)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> inv( SCVar<T,KEY,COMP> const& )\n";
#endif
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::inv(CV.B()));
  if (Op<T>::l(CV.B()) <= 0. && Op<T>::u(CV.B()) >= 0.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::INV);
  ...
  if ((!CV._CM->options.REMEZ_USE ||
    ...
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::COMPOSE);

  if (CV._CM->options.MIXED_IA) CV2._set_bndT(Op<T>::inv(CV.B()));
  if (CV._CM->options.MIG_USE)
    CV2.simplify(CV._CM->options.MIG_ATOL, CV._CM->options.MIG_RTOL);
  if (CV._CM->options.LIFT_USE)
    CV2.lift(CV._CM, CV._CM->options.LIFT_ATOL, CV._CM->options.LIFT_RTOL);
  return CV2;
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
sqrt(SCVar<T, KEY, COMP> const& CV)
{
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::sqrt(CV.B()));
  if (Op<T>::l(CV.B()) < 0.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::SQRT);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
exp(SCVar<T, KEY, COMP> const& CV)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> exp( SCVar<T,KEY,COMP> const& )\n";
#endif
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::exp(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>&&
exp(SCVar<T, KEY, COMP>&& CV)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> && exp( SCVar<T,KEY,COMP> && )\n";
#endif
  if (!CV._CM)
  {
    CV = Op<T>::exp(CV.B());
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
log(SCVar<T, KEY, COMP> const& CV)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> log( SCVar<T,KEY,COMP> const& )\n";
#endif
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::log(CV.B()));
  if (Op<T>::l(CV.B()) <= 0.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::LOG);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
xlog(SCVar<T, KEY, COMP> const& CV)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> xlog( SCVar<T,KEY,COMP> const& )\n";
#endif
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::xlog(CV.B()));
  if (Op<T>::l(CV.B()) <= 0.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::LOG);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
pow(SCVar<T, KEY, COMP> const& CV, const int n)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr
      << "-- SCVar<T,KEY,COMP> pow( SCVar<T,KEY,COMP> const&, const int )\n";
#endif
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::pow(CV.B(), n));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
pow(const SCVar<T, KEY, COMP>& CV, double const& a)
{
#ifdef MC__SCMODEL_TRACE
  std::cerr << "-- SCVar<T,KEY,COMP> pow( SCVar<T,KEY,COMP> const&, double "
               "const& )\n";
#endif
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::pow(CV.B(), a));
  if (Op<T>::l(CV.B()) <= 0.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::DPOW);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
pow(const SCVar<T, KEY, COMP>& CV1, const SCVar<T, KEY, COMP>& CV2)
{
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
pow(double const& a, const SCVar<T, KEY, COMP>& CV)
{
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
prod(const unsigned n, const SCVar<T, KEY, COMP>* CV)
{
  switch (n)
  {
    case 0:
    ...
    case 1:
      return CV[0];
    default:
      return CV[0] * prod(n - 1, CV + 1);
  }
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
monom(const unsigned n, const SCVar<T, KEY, COMP>* CV, const unsigned* k)
{
  switch (n)
  {
    case 0:
      return 1.;
    case 1:
      return pow(CV[0], (int)k[0]);
    default:
      return pow(CV[0], (int)k[0]) * monom(n - 1, CV + 1, k + 1);
  }
}

template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
cheb(const SCVar<T, KEY, COMP>& CV, const unsigned n)
{
  switch (n)
  {
    case 0:
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
cos(const SCVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::cos(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
sin(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return cos( CV - PI/2. );
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::sin(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
tan(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return sin( CV ) / cos( CV );
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::tan(CV.B()));
  if (Op<T>::l(Op<T>::cos(CV.B())) <= 0. && Op<T>::u(Op<T>::cos(CV.B())) >= 0.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::TAN);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
acos(const SCVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::acos(CV.B()));
  if (Op<T>::l(CV.B()) < -1. || Op<T>::u(CV.B()) > 1.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::ACOS);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
asin(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return PI/2. - acos( CV );
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::asin(CV.B()));
  if (Op<T>::l(CV.B()) < -1. || Op<T>::u(CV.B()) > 1.)
    throw typename SCModel<T, KEY, COMP>::Exceptions(
        SCModel<T, KEY, COMP>::Exceptions::ASIN);
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
atan(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return asin( CV / sqrt( sqr( CV ) + 1. ) );
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::atan(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
cosh(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return 0.5*(mc::exp(CV)+mc::exp(-CV));
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::cosh(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
sinh(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return 0.5*(mc::exp(CV)-mc::exp(-CV));
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::sinh(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
tanh(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return (mc::exp(2*CV)-1)/(mc::exp(2*CV)+1);
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::tanh(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
erf(const SCVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::erf(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
erfc(const SCVar<T, KEY, COMP>& CV)
{
#if 0
  return( 1 - mc::erf(CV) );
#endif

  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::erfc(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
fabs(const SCVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SCVar<T, KEY, COMP>(Op<T>::fabs(CV.B()));
  if (Op<T>::l(CV.B()) >= 0.) return CV;
  if (Op<T>::u(CV.B()) <= 0.) return -CV;
...
template <typename T, typename KEY, typename COMP>
inline SCVar<T, KEY, COMP>
hull(SCVar<T, KEY, COMP> const& CV1, SCVar<T, KEY, COMP> const& CV2)
{
  ...
  else if (!CV2._CM)
  {
    ...
    return CVR +
           Op<T>::hull(CV1.R(), CV2._coefmon.at(0) + CV2._bndrem - CVR.B());
  }
  ...
  CVR    = (1. - eta) * CV1C + eta * CV2C +
        Op<T>::hull(R1C + eta * BCVD, R2C + (eta - 1.) * BCVD);

  if (CV1._CM->options.MIXED_IA) CVR._set_bndT(Op<T>::hull(CV1.B(), CV2.B()));
  if (CV1._CM->options.MIG_USE)
    CVR.simplify(CV1._CM->options.MIG_ATOL, CV1._CM->options.MIG_RTOL);
  if (CV1._CM->options.LIFT_USE)
    CVR.lift(CV1._CM, CV1._CM->options.LIFT_ATOL, CV1._CM->options.LIFT_RTOL);
  return CVR;
}

template <typename T, typename KEY, typename COMP>
inline bool
inter(SCVar<T, KEY, COMP>& CVR, SCVar<T, KEY, COMP> const& CV1,
      SCVar<T, KEY, COMP> const& CV2)
{
  // Neither operands associated to SCModel -- Make intersection in T type
  if (!CV1._CM && !CV2._CM)
  {
  ...
  else if (!CV2._CM)
  {
    ...
    T R1 = CV1.R();
    CVR  = CV1.P();
    if (!Op<T>::inter(CVR._bndrem, R1, B2 - CVR.B())) return false;
    CVR._center();

    if (CVR._CM->options.MIXED_IA)
      CVR._set_bndT(BR);
    else
      CVR._unset_bndT();
    return true;
...
//! for use of mc::SCVar in DAG evaluation and as template parameter in other
//! MC++ types
template <typename T, typename KEY, typename COMP>
struct Op<mc::SCVar<T, KEY, COMP>>
{
  ...
  l(const CV& x)
  {
    return mc::Op<T>::l(x.B());
  }
  static double
  u(const CV& x)
  {
    return mc::Op<T>::u(x.B());
  }
  static double
  abs(const CV& x)
  {
    return mc::Op<T>::abs(x.B());
  }
  static double
  mid(const CV& x)
  {
    return mc::Op<T>::mid(x.B());
  }
  static double
  diam(const CV& x)
  {
    return mc::Op<T>::diam(x.B());
  }
  static CV
  inv(const CV& x)
  {
  ...
  static CV
  lmtd(const CV& x, const CV& y)
  {
    return (x - y) / (mc::log(x) - mc::log(y));
  }
  static CV
  rlmtd(const CV& x, const CV& y)
  {
  ...
  fstep(const CV& x)
  {
    return CV(mc::Op<T>::fstep(x.B()));
  }
  static CV
  bstep(const CV& x)
  {
    return CV(mc::Op<T>::bstep(x.B()));
  }
  static CV
  hull(const CV& x, const CV& y)
  {
    return mc::hull(x, y);
  }
  static CV
  min(const CV& x, const CV& y)
  {
    return mc::Op<T>::min(x.B(), y.B());
  }
  static CV
  max(const CV& x, const CV& y)
  {
    return mc::Op<T>::max(x.B(), y.B());
  }
  static CV
  arh(const CV& x, double const& k)
  {
    return mc::exp(-k / x);
  }
  ...
  static CV
  cheb(const CV& x, const unsigned n)
  {
    return mc::cheb(x, n);
  }
  static CV
  prod(const unsigned n, const CV* x)
  {
    return mc::prod(n, x);
  }
  static CV
  monom(const unsigned n, const CV* x, const unsigned* k)
  {
    return mc::monom(n, x, k);
  }
  static bool
  inter(CV& xIy, const CV& x, const CV& y)
  {
    return mc::inter(xIy, x, y);
  }
  static bool
  eq(const CV& x, const CV& y)
  {
    return mc::Op<T>::eq(x.B(), y.B());
  }
  static bool
  ne(const CV& x, const CV& y)
  {
    return mc::Op<T>::ne(x.B(), y.B());
  }
  static bool
  lt(const CV& x, const CV& y)
  {
    return mc::Op<T>::lt(x.B(), y.B());
  }
  static bool
  le(const CV& x, const CV& y)
  {
    return mc::Op<T>::le(x.B(), y.B());
  }
  static bool
  gt(const CV& x, const CV& y)
  {
    return mc::Op<T>::gt(x.B(), y.B());
  }
  static bool
  ge(const CV& x, const CV& y)
  {
    return mc::Op<T>::ge(x.B(), y.B());
  }
};
...
//! mc::SCVar of MC++ as a template parameter of the classes fadbad::F,
...
struct Op<mc::SCVar<T, KEY, COMP>>
{
  ...
  template <typename U>
  static CV&
  myCadd(CV& x, U const& y)
  {
    return x += y;
  }
  template <typename U>
  static CV&
  myCsub(CV& x, U const& y)
  {
    return x -= y;
  }
  template <typename U>
  static CV&
  myCmul(CV& x, U const& y)
  {
    return x *= y;
  }
  template <typename U>
  static CV&
  myCdiv(CV& x, U const& y)
  {
    return x /= y;
  }
  static CV
  ...
  static bool
  myEq(const CV& x, const CV& y)
  {
    return mc::Op<T>::eq(const_cast<CV*>(&x)->bound(),
                         const_cast<CV*>(&y)->bound());
  }
  static bool
  myNe(const CV& x, const CV& y)
  {
    return mc::Op<T>::ne(const_cast<CV*>(&x)->bound(),
                         const_cast<CV*>(&y)->bound());
  }
  static bool
  myLt(const CV& x, const CV& y)
  {
    return mc::Op<T>::lt(const_cast<CV*>(&x)->bound(),
                         const_cast<CV*>(&y)->bound());
  }
  static bool
  myLe(const CV& x, const CV& y)
  {
    return mc::Op<T>::le(const_cast<CV*>(&x)->bound(),
                         const_cast<CV*>(&y)->bound());
  }
  static bool
  myGt(const CV& x, const CV& y)
  {
    return mc::Op<T>::gt(const_cast<CV*>(&x)->bound(),
                         const_cast<CV*>(&y)->bound());
  }
  static bool
  myGe(const CV& x, const CV& y)
  {
    return mc::Op<T>::ge(const_cast<CV*>(&x)->bound(),
                         const_cast<CV*>(&y)->bound());
  }
};
