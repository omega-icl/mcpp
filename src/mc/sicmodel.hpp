...
/*!
\page page_CHEBYSHEV_SPARSEINT Sparse Interval Chebyshev Model Arithmetic for
Factorable Functions

The classes mc::SICModel and mc::SICVar provide an implementation of sparse
interval Chebyshev model arithmetic [Zha & Chachuat, 2021]. By contrast to
sparse Chebyshev model arithmetic, the distinction between the polynomial part
and the remainder part is blurred in a sparse interval Chebyshev model by
considering a multivariate polynomial with interval coefficients, \f$\mathcal
P:\mathbb{R}^n\to\mathbb{IR}\f$, so that: \f{align*} f({x}) \in \mathcal P({x}),
\quad \forall {x}\in D. \f} These interval coefficients are propagated in the
arithmetic of the templated type, typically a verified interval arithmetic.
Sparse interval Chebyshev models yield a superset of sparse Chebyhev models. In
general, they enable tighter inclusions by taking advantage of the distribution
of the remainder term over the participating monomials.

\section sec_CHEBYSHEV_SPARSEINT_use How do I compute a sparse interval
Chebyshev model of a factorable function?

Suppose we want to compute a 4th-order sparse interval Chebyshev model for the
...
First, an mc::SICModel object is initialized by passing the order of the
...
Next, the variables \f$x\f$ and \f$y\f$ are added to the sparse interval
Chebyshev model:
...
mc::SICVar, participating in the Chebyshev model <tt>mod</tt>, belonging to the
...
The interval coefficients and corresponding monomials in the Chebyshev model are
displayed first, with the middle integer being the total monomial order. The
Chebyshev model range estimator is reported last.

Other operations involve retreiving the model range, or computing the range of
the interval-valued polynomial at a given point:
...
See the documentations of mc::SICModel and mc::SICVar for a complete list of
member functions.


\section sec_CHEBYSHEV_SPARSEINT_fct Which functions are overloaded for sparse
interval Chebyshev model arithmetic?

mc::SICVar overloads the usual functions <tt>exp</tt>, <tt>log</tt>,
...
<tt>bstep</tt> are not currently overloaded in mc::SICVar. The functions
<tt>inter</tt> and <tt>hull</tt> are overloaded, using the midpoint polynomial
as the reference (centred) polynomial part and the interval-coefficient radii as
the remainder (see \ref sec_CHEBYSHEV_SPARSEINT_opt for the role of REF_POLY).

\section sec_CHEBYSHEV_SPARSEINT_opt How are the options set for the computation
of a sparse interval Chebyshev model?

The class mc::SICModel has a public member called mc::SICModel::options that can
...
<TABLE border="1">
<CAPTION><EM>Options in mc::SICModel::Options: name, type and
...
1-Chebyshev basis. <TR><TH><tt>HOT_SPLIT</tt>
<TD><tt>mc::SICModel::Options::ALLOCATION</tt> <TD>mc::SICModel::Options::FULL
         <TD>Strategy for allocating higher-order terms to monomials in sparse
product terms. <TR><TH><tt>PRODBND_SPLIT</tt> <TD><tt>bool</tt> <TD>false
...
<TD>1e2*machprec() <TD>Threshold for coefficient values in Chebyshev expansion
for bounding of transcendental univariates. <TR><TH><tt>BOUNDER_TYPE</tt>
<TD><tt>mc::SICModel::Options::BOUNDER</tt> <TD>mc::SICModel::Options::LSB
         <TD>Chebyshev model range bounder.
     <TR><TH><tt>BERNSTEIN_ORDER</tt> <TD><tt>unsigned int</tt> <TD>0
         <TD>Degree of the Bernstein basis when BOUNDER_TYPE is set to
mc::SICModel::Options::BERNSTEIN. <TR><TH><tt>MIXED_IA</tt> <TD><tt>bool</tt>
...
\section sec_CHEBYSHEV_SPARSEINT_err Errors What errors can I encounter during
computation of a sparse interval Chebyshev model?
...
mc::SICModel::Exceptions is thrown, which contains the type of error. Possible
errors encountered during the computation of a sparse interval Chebyshev model
are:
...
//! @brief C++ class for the computation of sparse interval Chebyshev models for
...
////////////////////////////////////////////////////////////////////////
{
  ...
  friend class SICModel;
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> inv(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> sqr(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> sqrt(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> exp(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> log(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> xlog(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> pow(const SICVar<U, K, C>&, const int);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> pow(const SICVar<U, K, C>&, double const&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> pow(double const, const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> pow(const SICVar<U, K, C>&, const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> prod(const unsigned int, const SICVar<U, K, C>*);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> monom(const unsigned int, const SICVar<U, K, C>*,
                               const unsigned*);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> cheb(const SICVar<U, K, C>&, const unsigned);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> cos(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> sin(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> tan(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> acos(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> asin(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> atan(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> cosh(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> sinh(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> tanh(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> erf(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> erfc(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> fabs(const SICVar<U, K, C>&);
 ...
 protected:
  ...
  //! @brief Interval coefficients in Chebyshev interpolant of univariate
  //! functions
  ...
  //  //! @brief Resize and return a pointer to the Chebyshev coefficient
  //  interpolant array std::vector<double>& _resize_coefuniv
  //    ()
 ...
 public:
  /** @addtogroup SICHEBYSHEV Sparse Interval Chebyshev Model Arithmetic for
   * Factorable Functions
   *  @{
   */
  //! @brief Constructor of Sparse Chebyshev model environment for maximal order
  //! <tt>maxord</tt>
  SICModel(const unsigned maxord = 3) : _maxord(maxord)
  {
    _coefuniv.reserve(_maxord + options.INTERP_EXTRA + 1);
  }

  //! @brief Copy constructor of Sparse Interval Chebyshev model environment
  SICModel(SICModel<T, KEY, COMP> const& mod)
      : _maxord(mod._maxord),
        _setvar(mod._setvar),
        _bndvar(mod._bndvar),
        _refvar(mod._refvar),
        _scalvar(mod._scalvar)
  {
    _coefuniv.reserve(_maxord + options.INTERP_EXTRA + 1);
  }

#ifdef MC__SCMODEL_H
  //! @brief Conversion constructor from a (non-interval) sparse Chebyshev model
  //! environment.
  //! @note Only available when scmodel.hpp has been included beforehand, so
  //! that
  //!       mc::SCModel is a complete type. This keeps sicmodel.hpp standalone
  //!       and avoids re-introducing the armadillo dependency that scmodel.hpp
  //!       pulls in.
  SICModel(SCModel<T, KEY, COMP> const& mod)
      : _maxord(mod.maxord()),
        _setvar(mod.setvar()),
        _bndvar(mod.bndvar()),
        _refvar(mod.refvar()),
        _scalvar(mod.scalvar())
  {
    _coefuniv.reserve(_maxord + options.INTERP_EXTRA + 1);
  }
#endif
  ...
  //! @brief Transform monomial-coefficient map 'coefmon' to Bernstein basis
  //! representation
  template <typename C>
  ...
  class Exceptions
  {
   public:
    ...
    enum TYPE
    {
      ...
      MODEL = -3,  //!< Operation between Chebyshev variables linked to
    ...
    what()
    {
      switch (_ierr)
      {
        ...
        case LOG:
          return "mc::SICModel\t Log operation with non-positive numbers in "
                 "range";
        case SQRT:
          return "mc::SICModel\t Square-root operation with negative numbers "
                 "in range";
        case DPOW:
          return "mc::SICModel\t Real power operation with negative numbers in "
                 "range";
        case TAN:
          return "mc::SICModel\t Tangent operation with (k+1/2)·PI in range";
        case ACOS:
          return "mc::SICModel\t Cosine inverse operation with range outside "
                 "[-1,1]";
        case ASIN:
          return "mc::SICModel\t Sine inverse operation with range outside "
                 "[-1,1]";
        case COMPOSE:
        ...
        case INCON:
          return "mc::SICModel\t Inconsistent bounds with template parameter "
                 "arithmetic";
        case MODEL:
          return "mc::SICModel\t Operation between variables in different "
                 "model environments";
        case UNDEF:
  ...
  struct Options
  {
    ...
    //! @brief Strategies for allocating higher-order terms to monomials in
    //! sparse product terms
    enum ALLOCATION
    {
    ...
    unsigned BASIS;
    //! @brief Strategy for allocating higher-order terms to monomials in sparse
    //! product terms
    ...
    //! @brief Threshold for coefficient values in Chebyshev interpolation of
    //! univariates
    double INTERP_THRES;
    //! @brief Chebyshev model range bounder
    BOUNDER BOUNDER_TYPE;
    //! @brief Degree of the Bernstein basis when
    //! mc::SICModel::Options::BOUNDER_TYPE is set to
    //! mc::SICModel::Options::BERNSTEIN
    ...
    //! part in the overloaded functions mc::inter and mc::hull. A value of 0
    //! amounts to selecting the polynomial part of the left operand, whereas a
    //! value of 1 selects the right operand.
    double REF_POLY;
 ...
 private:
  ...
  double _minimax(PUNIV const& f, unsigned const maxord);
  ...
  template <typename PUNIV>
  bool _minimax(PUNIV const& f, SICVar<T, KEY, COMP> const& CV,
  ...
  SICVar<T, KEY, COMP> _clenshaw(SICVar<T, KEY, COMP> const& CVinner,
                                 unsigned const maxord);

  //! @brief Apply the Horner algorithm
  SICVar<T, KEY, COMP> _horner(SICVar<T, KEY, COMP> const& CVinner,
                               unsigned const maxord);
  ...
  //! <a>CV2</a>
  template <typename PUNIV>
  bool _chebinterp(PUNIV const& f, SICVar<T, KEY, COMP> const& CV,
                   SICVar<T, KEY, COMP>& CV2, bool const rematbound);

  //! @brief Apply Chebyshev composition to variable <a>CVI</a> using the
  //! coefficients <a>coefmon</a> of the outer function
  template <typename U>
  static SICVar<T, KEY, COMP> _composition(std::vector<U> const& coefouter,
                                           unsigned const maxord,
                                           SICVar<T, KEY, COMP> const& CVinner);

  //! @brief Recursive calculation of nonnegative integer powers
  SICVar<T, KEY, COMP> _intpow(SICVar<T, KEY, COMP> const& CV,
                               int const n) const;
  ...
  void _sscal1D(const t_poly& coefmon0, T const& coefscal,
  ...
  void _svec1Dfull(typename t_var::const_iterator itvar,
                   std::pair<t_mon, T> const& mon,
                   std::vector<SICVar<T, KEY, COMP>>& vec) const;

  //! @brief Display of recursive univariate Chebyshev polynomials
  void _sdisp1Dfull(std::vector<SICVar<T, KEY, COMP>> const& vec,
                    typename t_var::const_iterator itvar,
                    std::string const& name = "",
                    std::ostream& os        = std::cout) const;

  //! @brief Product of multivariate Chebyshev polynomials in sparse format
  void _sprod(SICVar<T, KEY, COMP> const& CV1, SICVar<T, KEY, COMP> const& CV2,
              t_poly& coefmon, double& coefrem) const;
};

template <typename T, typename KEY, typename COMP>
inline const std::string SICModel<T, KEY, COMP>::Options::BOUNDER_NAME[3] = {
    "NAIVE", "LSB", "BERNSTEIN"};

template <typename T, typename KEY, typename COMP>
inline T SICModel<T, KEY, COMP>::TZeroOne = Op<T>::zeroone();

template <typename T, typename KEY, typename COMP>
inline T SICModel<T, KEY, COMP>::TOne = 2. * Op<T>::zeroone() - 1.;
...
////////////////////////////////////////////////////////////////////////
{
  ...
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> operator-(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> operator*(const SICVar<U, K, C>&,
                                   const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend std::ostream& operator<<(std::ostream&, const SICVar<U, K, C>&);

  template <typename U, typename K, typename C>
  friend U funcptr(const U, const unsigned);
  template <typename U, typename K, typename C>
  friend void interpolation(double*, const SICVar<U, K, C>&, const unsigned);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> composition(double const*, const SICVar<U, K, C>&);
  ...
  friend SICVar<U, K, C> fabs(const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend SICVar<U, K, C> hull(const SICVar<U, K, C>&, const SICVar<U, K, C>&);
  template <typename U, typename K, typename C>
  friend bool inter(SICVar<U, K, C>&, const SICVar<U, K, C>&,
                    const SICVar<U, K, C>&);
 ...
 private:
  ...
  SICVar<T, KEY, COMP>& _set(SICVar<T, KEY, COMP> const& CV);

  //! @brief Set Chebyshev variable equal to <a>CV</a>
  SICVar<T, KEY, COMP>& _set(SICVar<T, KEY, COMP>&& CV);

  //! @brief Set Chebyshev variable <a>id</a> with domain <a>dom</a>
  SICVar<T, KEY, COMP>& _set(KEY const& id, T const& dom,
 ...
 public:
  ...
  std::string display(t_poly coefmon, int const& BASIS = 1,
                      int const& IDISP = 5) const;
  ...
  SICVar(SICModel<T, KEY, COMP>* CM);

  //! @brief Constructor of Chebyshev variable for a real scalar
  SICVar(double const& d = 0., SICModel<T, KEY, COMP>* CM = nullptr);

  //! @brief Constructor of Chebyshev variable for a remainder bound
  SICVar(T const& B, SICModel<T, KEY, COMP>* CM = nullptr);

  //! @brief Constructor of Chebyshev variable <a>id</a> with domain <a>dom</a>
  SICVar(SICModel<T, KEY, COMP>* CM, KEY const& id, T const& dom);

  //! @brief Copy constructor of Chebyshev variable
  SICVar(SICVar<T, KEY, COMP> const& CV);

  //! @brief Copy constructor of Chebyshev variable
  SICVar(SICVar<T, KEY, COMP>&& CV);
  ...
  //! @brief Set Chebyshev variable <a>id</a> with domain <a>dom</a>
  SICVar<T, KEY, COMP>&
  set(SICModel<T, KEY, COMP>* CM, KEY const& id, T const& dom)
  {
    set(CM);
    _set(id, dom);
    return *this;
  ...
  //! <tt>coefmon</tt>
  SICVar<T, KEY, COMP>&
  set(t_poly& coefmon)  //, t_var const& ndxvar )
  {
    _coefmon = coefmon;
    //_ndxvar  = ndxvar;
    _unset_bndT();
  ...
  //! <tt>coefmon</tt>
  SICVar<T, KEY, COMP>&
  set(std::map<t_mon, double, lt_mon> const& coefmon, t_var const& ndxvar,
  ...
  bound(int const type) const
  {
    ...
    {
      T bndT;
      return Op<T>::inter(bndT, _polybound(type), *_bndT) ? bndT
                                                          : _polybound(type);
    }
  }
  ...
  bound() const
  {
    ...
    {
      T bndT;
      return Op<T>::inter(bndT, bndpol(), *_bndT) ? bndT : bndpol();
    }
  }
  ...
  template <typename U>
  U
  bound(std::map<KEY, std::vector<U>, COMP> const& bndbasis) const
  {
  ...
  T constant(bool const reset = false);
  ...
  T linear(KEY const& id, bool const reset = false);

  //! @brief Lift the model by appending new auxiliary variable corresponding to
  //! the sum of uncertainties if magnitude greater than TOL
  SICVar<T, KEY, COMP>& lift(SICModel<T, KEY, COMP>* SCM, double const& ATOL,
                             double const& RTOL);

  //! @brief Project the model by removing auxiliary variables participating in
  //! monomials
  SICVar<T, KEY, COMP>& project(bool const reset = false);

  //! @brief Project the model by removing variable id participating in
  //! monomials
  SICVar<T, KEY, COMP>& project(KEY const& id);

  //! @brief Simplify the model by appending coefficient less than TOL as well
  //! as terms with order greater than or equal to TORD to the constant term
  SICVar<T, KEY, COMP>& simplify(double const& ATOL = 0e0,
  ...
  SICVar<T, KEY, COMP>& scale(KEY const& id, T const& dom);

  //! @brief Scale coefficients in Chebyshev variable <a>id</a> for the modified
  //! variables domain <a>dom</a>
  SICVar<T, KEY, COMP>& scale(std::map<KEY, T, COMP> const& dom);
  ...
  SICVar<T, KEY, COMP>& operator=(SICVar<T, KEY, COMP> const&);
  SICVar<T, KEY, COMP>& operator=(SICVar<T, KEY, COMP>&&);
  SICVar<T, KEY, COMP>& operator=(double const&);
  SICVar<T, KEY, COMP>& operator=(T const&);
  template <typename U>
  SICVar<T, KEY, COMP>& operator+=(SICVar<U, KEY, COMP> const&);
  template <typename U>
  SICVar<T, KEY, COMP>& operator+=(U const&);
  SICVar<T, KEY, COMP>& operator+=(double const&);
  template <typename U>
  SICVar<T, KEY, COMP>& operator-=(const SICVar<U, KEY, COMP>&);
  template <typename U>
  SICVar<T, KEY, COMP>& operator-=(U const&);
  SICVar<T, KEY, COMP>& operator-=(double const&);
  SICVar<T, KEY, COMP>& operator*=(const SICVar<T, KEY, COMP>&);
  SICVar<T, KEY, COMP>& operator*=(double const&);
  SICVar<T, KEY, COMP>& operator*=(T const&);
  SICVar<T, KEY, COMP>& operator/=(SICVar<T, KEY, COMP> const&);
  SICVar<T, KEY, COMP>& operator/=(double const&);
 ...
 private:
  ...
  template <typename U>
  static U
  _rescale(U& X, double const w, double const c)
  {
  ...
  SICVar<T, KEY, COMP>& _simplify(
      typename SICVar<T, KEY, COMP>::t_poly::iterator itmon, double const& ATOL,
...
template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_set(KEY const& id, T const& X)
{
  _setvar.insert(id);
  _bndvar[id]  = X;
...
template <typename U>
inline std::vector<U>&
SICModel<T, KEY, COMP>::_get_bndpow(unsigned const maxord, U const& bndvar,
                                    double const& refvar, double const& scalvar,
                                    std::vector<U>& bndpow) const
{
...
SICModel<T, KEY, COMP>::get_basis(unsigned const maxord,
                                  std::map<KEY, U, COMP> const& bndvar,
                                  std::map<KEY, std::vector<U>, COMP>& bndbasis,
...
template <typename U>
inline void
SICModel<T, KEY, COMP>::get_bndmon(std::map<t_mon, U, lt_mon>& bndmon,
                                   std::map<KEY, U, COMP> const& bndvar,
                                   bool const scaled) const
{
    ...
    PUNIV const& f, unsigned const maxord)  // range of f assumed as [-1,1]
{
  ...
  for (unsigned iter = 0; iter < options.REMEZ_MAXIT; ++iter)
  {
    ...
    catch (...)
    {
      ...
      // Interpolation. _chebinterp populates the member _ainterp directly with
      // valid coefficients.
...
SICModel<T, KEY, COMP>::_minimax(
    PUNIV const& f, unsigned const maxord)  // range of f assumed as [-1,1]
{
  boost::math::tools::remez_minimax<double> problem(f, maxord, 0, -1., 1.,
                                                    false, false, 0, 64);
...
template <typename VEC>
inline VEC
SICModel<T, KEY, COMP>::_to_chebyshev(VEC const& veccoef) const
{
...
template <typename PUNIV>
inline bool
SICModel<T, KEY, COMP>::_minimax(PUNIV const& f, SICVar<T, KEY, COMP> const& CV,
                                 SICVar<T, KEY, COMP>& CV2)
{
  ...
  try
  {
    double m(Op<SICVar<T, KEY, COMP>>::mid(CV)),
        r(0.5 * Op<SICVar<T, KEY, COMP>>::diam(CV));
    ...
    for (unsigned i = 0; i <= _maxord; i++) _coefuniv[i] = _ainterp[i];
    ...
    unsigned& maxord)  // range of f assumed as [-1,1]
{
  ...
  for (; std::fabs(_ainterp[maxord]) > tol ||
         (maxord && std::fabs(_ainterp[maxord - 1]) > tol);)
  {
    maxord *= 2;
    _chebinterp(f, maxord);
...
SICModel<T, KEY, COMP>::_horner(SICVar<T, KEY, COMP> const& CVinner,
                                unsigned const maxord)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
SICModel<T, KEY, COMP>::_clenshaw(SICVar<T, KEY, COMP> const& CVinner,
                                  unsigned const maxord)
{
  assert(maxord < _coefuniv.size());
...
template <typename PUNIV>
inline bool
SICModel<T, KEY, COMP>::_chebinterp(PUNIV const& f,
                                    ...
                                    bool const rematbound)
{
  ...
  {
    double m(Op<SICVar<T, KEY, COMP>>::mid(CV)),
        r(0.5 * Op<SICVar<T, KEY, COMP>>::diam(CV)), rem(0.);
    auto const& fscaled = [=](const double& x) { return f(r * x + m); };
    _coefuniv.resize(_maxord + 1);
    if (rematbound)
    {
...
template <typename U>
inline SICVar<T, KEY, COMP>
SICModel<T, KEY, COMP>::_composition(std::vector<U> const& coefouter,
                                     unsigned const maxord,
                                     SICVar<T, KEY, COMP> const& CVinner)
{
  // composition based on
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
SICModel<T, KEY, COMP>::_intpow(SICVar<T, KEY, COMP> const& CV,
                                int const n) const
{
  if (n == 0)
    return 1.;
...
template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_sscal1D(t_poly const& coefmon0, T const& Iscal,
                                 t_poly& coefmon) const
{
  if (Op<T>::abs(Iscal) == 0.) return;
  coefmon = coefmon0;
  for (auto& [mon, coef] : coefmon) coef *= Iscal;
  return;
}

template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_slift1D(t_poly const& coefmon0, double const& dscal,
                                 t_poly& coefmon) const
{
      ...
      switch (options.BASIS)
      {
        case Options::CHEB:
          switch (options.HOT_SPLIT)
          {
            case Options::FULL:
            {
              ...
              {
                auto [itmon1, ins1] =
                    coefmon.insert(std::make_pair(t_mon(ivar, iord), add2mon));
                if (!ins1) itmon1->second += add2mon;
#ifdef MC__SICMODEL_DEBUG_SLIFT
                std::cout << "_slift1D: "
                          << itmon1->first.display(options.BASIS) << "  "
                          << add2mon << std::endl;
#endif
              }
              if (ndxord <= _maxord)
              {
                auto [itmon2, ins2] = coefmon.insert(
                    std::make_pair(t_mon(*itvar, ndxord), add2mon));
                if (!ins2) itmon2->second += add2mon;
#ifdef MC__SICMODEL_DEBUG_SLIFT
                std::cout << "_slift1D: "
                          << itmon2->first.display(options.BASIS) << "  "
                          << add2mon << std::endl;
#endif
              }
              else
              {
                auto [itmon2, ins2] =
                    coefmon.insert(std::make_pair(t_mon(), add2mon));
                if (!ins2) itmon2->second += add2mon;
#ifdef MC__SICMODEL_DEBUG_SLIFT
                std::cout << "_slift1D: "
                          << itmon2->first.display(options.BASIS) << "  "
            ...
            case Options::SIMPLE:
            {
              ...
              auto [itmon1, ins1] =
                  coefmon.insert(std::make_pair(mon0, add2mon));
              if (!ins1) itmon1->second += add2mon;
#ifdef MC__SICMODEL_DEBUG_SLIFT
              std::cout << "_slift1D: " << itmon1->first.display(options.BASIS)
                        << "  " << itmon1->second << std::endl;
#endif
              if (ndxord <= _maxord)
              {
                auto [itmon2, ins2] = coefmon.insert(
...
#ifdef MC__SICMODEL_DEBUG_SLIFT
              std::cout << "_slift1D: " << itmon->first.display(options.BASIS)
                        << "  " << itmon->second << std::endl;
#endif
              continue;
            }
          }
        ...
        case Options::MONOM:
          switch (options.HOT_SPLIT)
          {
            ...
            {
              T add2mon1 =
                  (0.5 * dscal) * coef0 * (ndxord % 2 ? TOne : TZeroOne);
              for (auto const& [ivar, iord] : mon0.expr)
              {
                auto [itmon1, ins1] =
                    coefmon.insert(std::make_pair(t_mon(ivar, iord), add2mon1));
                if (!ins1) itmon1->second += add2mon1;
              }
              T add2mon2 =
                  (0.5 * dscal) * coef0 * (mon0.gcexp() % 2 ? TOne : TZeroOne);
              if (ndxord <= _maxord)
              {
                auto [itmon2, ins2] = coefmon.insert(
                    std::make_pair(t_mon(*itvar, ndxord), add2mon2));
                if (!ins2) itmon2->second += add2mon2;
              }
              else
              {
                auto [itmon2, ins2] =
                    coefmon.insert(std::make_pair(t_mon(), add2mon2));
                if (!ins2) itmon2->second += add2mon2;
              }
              continue;
            }
            case Options::SIMPLE:
            {
              T add2mon1 =
                  (0.5 * dscal) * coef0 * (ndxord % 2 ? TOne : TZeroOne);
              auto [itmon1, ins1] =
                  coefmon.insert(std::make_pair(mon0, add2mon1));
              if (!ins1) itmon1->second += add2mon1;
              T add2mon2 =
                  (0.5 * dscal) * coef0 * (mon0.gcexp() % 2 ? TOne : TZeroOne);
              if (ndxord <= _maxord)
...
template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_sdisp1D(t_poly const& coefmon, std::string const& name,
...
template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_sdisp1D(std::map<unsigned, t_poly> const& coefmap,
                                 typename t_var::const_iterator itvar,
                                 std::string const& name,
                                 std::ostream& os) const
{
...
SICModel<T, KEY, COMP>::_sdisp1Dfull(
    std::vector<SICVar<T, KEY, COMP>> const& vec,
    typename t_var::const_iterator itvar, std::string const& name,
    std::ostream& os) const
{
  os << name;
  bool first = true;
...
template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_sprod1D(std::map<unsigned, t_poly> const& sp1map,
...
template <typename T, typename KEY, typename COMP>
inline void
SICModel<T, KEY, COMP>::_svec1D(typename t_var::const_iterator itvar,
                                std::pair<t_mon, T> const& mon,
                                std::map<unsigned, t_poly>& mapspoly) const
{
  ...
  if (!mon.first.tord || ivar != *itvar)  // no dependence on variable *itvar
    ...
    mapspoly[iord].insert(std::make_pair(
        t_mon(mon.first.tord - iord,
              std::map<KEY, unsigned, COMP>(++mon.first.expr.begin(),
                                            mon.first.expr.end())),
        mon.second));
}
  ...
  if (bndbasis.empty())
  {
    switch (options.BASIS)
    {
      case Options::CHEB:
      {
        ...
        for (; it != coefmon.end(); ++it) bndpol += it->second * TOne;
        return bndpol;
      }
      case Options::MONOM:
      {
...
template <typename T, typename KEY, typename COMP>
template <typename C>
inline std::pair<std::map<typename SICModel<T, KEY, COMP>::t_mon, C,
                          typename SICModel<T, KEY, COMP>::lt_mon>,
                 std::map<KEY, unsigned, COMP>>
SICModel<T, KEY, COMP>::to_bernstein(std::map<t_mon, C, lt_mon> const& coefmon,
...
template <typename C, typename U>
inline U
SICModel<T, KEY, COMP>::_polybound_bernstein(
    std::map<t_mon, C, lt_mon> const& coefmon,
    std::map<KEY, std::vector<U>, COMP> const& bndbasis) const
{
  // Constant or linear model
...
template <typename T, typename KEY, typename COMP>
inline T
SICModel<T, KEY, COMP>::_monbound(t_mon const& mon) const
{
...
template <typename T, typename KEY, typename COMP>
inline double
SICModel<T, KEY, COMP>::_monval(t_mon const& mon,
...
template <typename T, typename KEY, typename COMP>
inline void
SICVar<T, KEY, COMP>::_set_bndpol(const T* bndpol)
{
  if (!bndpol)
  {
...
template <typename T, typename KEY, typename COMP>
inline void
SICVar<T, KEY, COMP>::_set_bndT(const T* bndT)
{
  if (!bndT)
  {
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>::SICVar(SICVar<T, KEY, COMP> const& CV)
{
  _init();
  _set(CV);
}
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator=(SICVar<T, KEY, COMP> const& CV)
{
  _set(CV);
  return *this;
}
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>::SICVar(SICVar<T, KEY, COMP>&& CV)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator=(SICVar<T, KEY, COMP>&& CV)
{
#ifdef MC__SICMODEL_TRACE
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>::SICVar(SICModel<T, KEY, COMP>* CM) : _CM(CM)
{
  _init();
  _set_bndpol(0.);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>::SICVar(double const& d, SICModel<T, KEY, COMP>* CM)
    : _CM(CM)
{
  _init();
  if (d == 0.) return;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator=(double const& d)
{
  _reinit();
  _CM = nullptr;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>::SICVar(T const& B, SICModel<T, KEY, COMP>* CM)
    : _CM(CM)
{
  _init();
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator=(T const& B)
{
  _reinit();
  _CM = nullptr;
  _coefmon.insert(std::make_pair(t_mon(), B));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>::SICVar(SICModel<T, KEY, COMP>* CM, KEY const& id,
                                    T const& dom)
    : _CM(CM)
{
  _init();
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::_set(KEY const& id, T const& dom, bool const updt)
{
  if (!_CM)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::INIT);
...
template <typename T, typename KEY, typename COMP>
inline T
SICVar<T, KEY, COMP>::IP(std::map<KEY, double, COMP> const& x) const
{
...
template <typename T, typename KEY, typename COMP>
inline double
SICVar<T, KEY, COMP>::P(std::map<KEY, double, COMP> const& x) const
{
  assert(_CM || _coefmon.size() <= 1);
  double Pval = 0.;
  for (auto const& [mon, coef] : _coefmon)
  {
    if (!mon.tord)
      ...
      Pval += Op<T>::mid(coef) * _CM->_monval(mon, x);
  }
  return Pval;
}
...
SICVar<T, KEY, COMP>::P() const
{
  ...
  if (!var._coefmon.begin()->first.tord)
    var._coefmon.insert(std::make_pair(t_mon(), 0.));
...
template <typename T, typename KEY, typename COMP>
inline T
SICVar<T, KEY, COMP>::constant(const bool reset)
{
  auto it_0 =
...
template <typename T, typename KEY, typename COMP>
inline T
SICVar<T, KEY, COMP>::linear(KEY const& id, bool const reset)
{
  auto it_i =
      (!nord() || _coefmon.empty() ? _coefmon.end() : _coefmon.find(t_mon(id)));
  T coeflin = (it_i == _coefmon.end() || isequal(_scalvar(id), 0.)
                   ? 0.
...
template <typename T, typename KEY, typename COMP>
inline void
SICVar<T, KEY, COMP>::_scale(typename t_var::const_iterator itvar,
                             T const& bndvar, t_poly& coefmon) const
{
  ...
  // *itvar
  std::vector<SICVar<T, KEY, COMP>> veccoef(nord() + 1,
                                            SICVar<T, KEY, COMP>(_CM));
...
template <typename T, typename KEY, typename COMP>
inline typename SICVar<T, KEY, COMP>::t_poly
                           ...
                           double const& RTOL)
{
  ...
  std::cout << remrad << " < "
            << 0.5 * RTOL * Op<T>::diam(_polybound()) + ATOL + machprec()
            << " : "
            << (remrad <
                0.5 * RTOL * Op<T>::diam(_polybound()) + ATOL + machprec())
            << std::endl;
#endif
  if (!_CM->_maxord ||
      (remrad < 0.5 * RTOL * Op<T>::diam(_polybound()) + ATOL + machprec()))
    return *this;

  // Allocate range to constant coefficient
  if (!_coefmon.begin()->first.tord)
    _coefmon.insert(std::make_pair(t_mon(), 0.));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::project(bool const reset)
{
  // Nothing to do
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::simplify(double const& ATOL, double const& RTOL,
                               int const TORD)
{
  ...
  for (auto it = _coefmon.begin(); it != _coefmon.end();)
  {
    ...
    if ((mon.tord && Op<T>::abs(coef) <= THRES) ||
    ...
    double const& ATOL, double const& RTOL, int const TORD) const
{
  ...
  for (auto it = coefmon.begin(); it != coefmon.end();)
  {
    ...
    double const THRES =
        (RTOL > 0 ? 0.5 * RTOL * Op<T>::diam(_polybound()) : 0e0) + ATOL;
    if ((mon.tord && THRES > 0e0 && Op<T>::abs(coef) <= THRES) ||
        (TORD >= 0 && (int)mon.tord > TORD))
    {
      ...
      for (auto const& [var, ord] : mon.expr)
        bndmon *= Op<T>::pow(
            scaled ? SICModel<T, KEY, COMP>::TOne : _bndvar(var), (int)ord);
      bndrem += bndmon * coef;
      it = coefmon.erase(it);
      continue;
    }
...
template <typename T, typename KEY, typename COMP>
inline void
SICVar<T, KEY, COMP>::_simplify(t_poly& coefmon) const
{
...
SICVar<T, KEY, COMP>::_to_monomial(
    KEY const& id, typename SICVar<T, KEY, COMP>::t_poly& coefmon) const
{
...
template <typename T, typename KEY, typename COMP>
inline typename SICVar<T, KEY, COMP>::t_poly
SICVar<T, KEY, COMP>::to_monomial(bool const scaled) const
{
...
template <typename T, typename KEY, typename COMP>
inline std::pair<typename SICVar<T, KEY, COMP>::t_poly, T>
SICVar<T, KEY, COMP>::to_monomial(bool const scaled, double const& ATOL,
...
template <typename T, typename KEY, typename COMP>
inline std::pair<typename SICVar<T, KEY, COMP>::t_poly,
                 std::map<KEY, unsigned, COMP>>
SICVar<T, KEY, COMP>::to_bernstein(unsigned const ORDER) const
{
...
template <typename T, typename KEY, typename COMP>
inline T
SICVar<T, KEY, COMP>::bound_bernstein(t_poly const& coefbern) const
{
  // Compute Bernstein coefficient hull
  bool first = true;
  T bound_hull(0.);
...
template <typename T, typename KEY, typename COMP>
inline std::string
SICVar<T, KEY, COMP>::display(typename SICVar::t_poly coefmon, int const& BASIS,
                              int const& IDISP) const
{
  ...
  for (auto const& [mon, coef] : coefmon)
    out << "[" << std::setw(IDISP + 7) << Op<T>::l(coef) << ","
        << std::setw(IDISP + 7) << Op<T>::u(coef) << "]  " << std::setw(2)
...
template <typename T, typename KEY, typename COMP>
inline std::ostream&
operator<<(std::ostream& out, SICVar<T, KEY, COMP> const& CV)
{
  unsigned IDISP = CV._CM ? CV._CM->options.DISPLAY_DIGITS : 7;
  int BASIS =
      CV._CM ? CV._CM->options.BASIS : SICModel<T, KEY, COMP>::Options::CHEB;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator+(SICVar<T, KEY, COMP> const& CV)
{
  return CV;
}
...
template <typename U>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator+=(SICVar<U, KEY, COMP> const& CV)
{
  if (_CM && CV._CM && _CM != CV._CM)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::MODEL);
...
inline SICVar<T, KEY, COMP>
operator+(SICVar<T, KEY, COMP> const& CV1, SICVar<U, KEY, COMP> const& CV2)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator+=(double const& c)
{
  if (c == 0.) return *this;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator+(SICVar<T, KEY, COMP> const& CV1, double const& c)
{
  SICVar<T, KEY, COMP> CV3(CV1);
  CV3 += c;
  return CV3;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator+(double const& c, SICVar<T, KEY, COMP> const& CV2)
{
  SICVar<T, KEY, COMP> CV3(CV2);
  CV3 += c;
  return CV3;
...
template <typename U>
inline SICVar<T, KEY, COMP>&
SICVar<T, KEY, COMP>::operator+=(U const& B)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator+(SICVar<T, KEY, COMP> const& CV1, T const& B)
{
  SICVar<T, KEY, COMP> CV3(CV1);
  CV3 += B;
  return CV3;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator+(T const& B, SICVar<T, KEY, COMP> const& CV2)
{
  SICVar<T, KEY, COMP> CV3(CV2);
  CV3 += B;
  return CV3;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator*(SICVar<T, KEY, COMP> const& CV1, SICVar<T, KEY, COMP> const& CV2)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
sqr(SICVar<T, KEY, COMP> const& CV)
{
  ...
  if (CV._CM && CV._CM->options.MIG_USE)
    CVSQR.simplify(CV._CM->options.MIG_ATOL, CV._CM->options.MIG_RTOL);
  CVSQR._unset_bndpol();
#ifdef MC__SICMODEL_DEBUG_SPROD
  CVSQR._unset_bndT();
...
SICVar<T, KEY, COMP>::operator/=(double const& c)
{
  if (isequal(c, 0.))
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::DIV);
  if (c == 1.) return *this;
  *this *= (1. / c);
  return *this;
}
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator/(SICVar<T, KEY, COMP> const& CV, double const& c)
{
  if (isequal(c, 0.))
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::DIV);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
operator/(double const& c, SICVar<T, KEY, COMP> const& CV)
{
  if (c == 0.) return 0.;
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
inv(SICVar<T, KEY, COMP> const& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::inv(CV.B()));
  if (Op<T>::l(CV.B()) <= 0. && Op<T>::u(CV.B()) >= 0.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::INV);
  ...
  if ((!CV._CM->options.REMEZ_USE ||
    ...
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::COMPOSE);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
sqrt(SICVar<T, KEY, COMP> const& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::sqrt(CV.B()));
  if (Op<T>::l(CV.B()) < 0.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::SQRT);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
exp(SICVar<T, KEY, COMP> const& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::exp(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
log(SICVar<T, KEY, COMP> const& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::log(CV.B()));
  if (Op<T>::l(CV.B()) <= 0.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::LOG);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
xlog(SICVar<T, KEY, COMP> const& CV)
{
#if 0
  return CV * log( CV );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::xlog(CV.B()));
  if (Op<T>::l(CV.B()) <= 0.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::LOG);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
pow(SICVar<T, KEY, COMP> const& CV, const int n)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::pow(CV.B(), n));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
pow(const SICVar<T, KEY, COMP>& CV, double const& a)
{
#if 0
  return exp( a * log( CV ) );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::pow(CV.B(), a));
  if (Op<T>::l(CV.B()) <= 0.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::DPOW);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
pow(const SICVar<T, KEY, COMP>& CV1, const SICVar<T, KEY, COMP>& CV2)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
pow(double const a, const SICVar<T, KEY, COMP>& CV)
{
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
prod(const unsigned n, const SICVar<T, KEY, COMP>* CV)
{
  switch (n)
  {
    case 0:
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
monom(const unsigned n, const SICVar<T, KEY, COMP>* CV, const unsigned* k)
{
  switch (n)
  {
    case 0:
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
cheb(const SICVar<T, KEY, COMP>& CV, const unsigned n)
{
  switch (n)
  {
    case 0:
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
cos(const SICVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::cos(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
sin(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return cos( CV - PI/2. );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::sin(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
tan(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return sin( CV ) / cos( CV );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::tan(CV.B()));
  if (Op<T>::l(Op<T>::cos(CV.B())) <= 0. && Op<T>::u(Op<T>::cos(CV.B())) >= 0.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::TAN);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
acos(const SICVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::acos(CV.B()));
  if (Op<T>::l(CV.B()) < -1. || Op<T>::u(CV.B()) > 1.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::ACOS);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
asin(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return PI/2. - acos( CV );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::asin(CV.B()));
  if (Op<T>::l(CV.B()) < -1. || Op<T>::u(CV.B()) > 1.)
    throw typename SICModel<T, KEY, COMP>::Exceptions(
        SICModel<T, KEY, COMP>::Exceptions::ASIN);
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
atan(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return asin( CV / sqrt( sqr( CV ) + 1. ) );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::atan(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
cosh(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return 0.5*(mc::exp(CV)+mc::exp(-CV));
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::cosh(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
sinh(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return 0.5*(mc::exp(CV)-mc::exp(-CV));
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::sinh(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
tanh(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return (mc::exp(2*CV)-1)/(mc::exp(2*CV)+1);
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::tanh(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
erf(const SICVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::erf(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
erfc(const SICVar<T, KEY, COMP>& CV)
{
#if 0
  return( 1 - mc::erf(CV) );
#endif

  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::erfc(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
fabs(const SICVar<T, KEY, COMP>& CV)
{
  if (!CV._CM) return SICVar<T, KEY, COMP>(Op<T>::fabs(CV.B()));
...
template <typename T, typename KEY, typename COMP>
inline SICVar<T, KEY, COMP>
hull(SICVar<T, KEY, COMP> const& CV1, SICVar<T, KEY, COMP> const& CV2)
{
...
template <typename T, typename KEY, typename COMP>
inline bool
inter(SICVar<T, KEY, COMP>& CVR, SICVar<T, KEY, COMP> const& CV1,
      SICVar<T, KEY, COMP> const& CV2)
{
  // Inspired by mc::inter for mc::SCVar (see mc::hull above for the treatment
  // of interval coefficients). Returns false if the intersection is found to be
  ...
  else if (!CV2._CM)
  {
    ...
    T Rint;
    if (!Op<T>::inter(Rint, R1, B2 - CVmid.B())) return false;
    CVR = CVmid;
    CVR += Rint;
...
//! for use of mc::SICVar in DAG evaluation and as template parameter in other
//! MC++ types
template <typename T, typename KEY, typename COMP>
...
//! mc::SICVar of MC++ as a template parameter of the classes fadbad::F,
