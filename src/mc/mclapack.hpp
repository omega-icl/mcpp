...
template <typename U>
inline void
display(const unsigned int m, const unsigned int n, const U* a,
        const unsigned int lda, const std::string& stra, std::ostream& os)
{
  os << stra << " =" << std::endl << std::scientific << std::setprecision(5);
...
//! @brief Wrapper to LAPACK function <tt>_dsyev<tt> doing eigenvalue
//! decomposition of symmetric <tt>n</tt>-by-<tt>n</tt> matrix <tt>A</tt>. A
//! pointer to an array containing the eignenvalues is returned - NULL pointer
//! if the eigenvalue decomposition was unsuccessful. If <tt>eigv</tt> is set to
//! <tt>true</tt> eigenvector too are computed and returned in <tt>A</tt>, which
//! is therefore alterred. Note that <tt>A</tt> is altered even if eigenvector
//! are not computed.
inline double*
dsyev_wrapper(const unsigned int n, double* A, const bool eigv = false)
{
  const int N(n);
  int info(1);
...
*/
inline int
dsysv(const dsymatrix& mat, dsymatrix& mat_inv)
{
  VERBOSE_REPORT;
  ...
  mat_inv.identity();
  char UPLO('l');
  int NRHS(mat.n), LDA(mat.n), *IPIV(new int[mat.n]), LDB(mat.n), LWORK(-1),
      INFO(1);
  double* WORK(new double[1]);
  dsysv_(&UPLO, &mat.n, &NRHS, mat_cp.array, &LDA, IPIV, mat_inv.array, &LDB,
         WORK, &LWORK, &INFO);
  ...
  {
    WARNING_REPORT;
    std::cerr << "DSYSV: Serious trouble happend. INFO = " << INFO << "."
              << std::endl;
  }
  return INFO;
}
...
*/
inline int
dsytrf(const dsymatrix& Amat, dgematrix& Lmat, std::vector<int>& IPIV)
{
  VERBOSE_REPORT;
  ...
  {
    WARNING_REPORT;
    std::cerr << "DSYTRF: Serious trouble happend. INFO = " << INFO << "."
              << std::endl;
  }
  return INFO;
}
...
*/
inline int
dsytrs(dgematrix const& Lmat, std::vector<int> const& IPIV,
       dgematrix const& Bmat, dgematrix& Xmat)
{
  VERBOSE_REPORT;
  Xmat = Bmat;
  char UPLO('l');
  int N(Lmat.n), NRHS(Bmat.n), LDA(Lmat.m), LDB(Bmat.m), INFO(1);
  dsytrs_(&UPLO, &N, &NRHS, Lmat.array, &LDA, IPIV.data(), Xmat.array, &LDB,
          &INFO);
  ...
  {
    WARNING_REPORT;
    std::cerr << "DSYTRS: Serious trouble happend. INFO = " << INFO << "."
              << std::endl;
  }
  return INFO;
}
...
*/
inline int
dgetrf(const dgematrix& Amat, dgematrix& Umat, dgematrix& Lmat,
       std::vector<int>& Pvec)
{
  VERBOSE_REPORT;
  dgematrix A(Amat);
  ...
  {
    WARNING_REPORT;
    std::cerr << "DGETRF: Argument #" << -INFO << " has an illegal value."
              << std::endl;
    return INFO;
  }
...
#endif
  Pvec.resize(M);
  for (int ir = 0; ir < M; ir++) Pvec[ir] = ir;
...
#ifdef MC__DEBUG_DGETRF
  std::cout << "Vector P:\n";
  for (int ir = 0; ir < M; ir++) std::cout << Pvec[ir] << std::endl;
#endif
  Lmat.resize(M, NM);
  Lmat.zero();
  for (int ir = 0; ir < NM; ir++) Lmat(ir, ir) = 1;
  for (int ir = 0; ir < M; ir++)
    for (int ic = 0; ic < ir && ic < NM; ic++) Lmat(ir, ic) = A(ir, ic);
#ifdef MC__DEBUG_DGETRF
  std::cout << "Matrix L:\n" << Lmat << std::endl;
#endif
  Umat.resize(NM, N);
  Umat.zero();
  for (int ir = 0; ir < NM; ir++)
    for (int ic = ir; ic < N; ic++) Umat(ir, ic) = A(ir, ic);
#ifdef MC__DEBUG_DGETRF
  std::cout << "Matrix U:\n" << Umat << std::endl;
#endif
  return INFO;
...
inline int
dgetrf(const dgematrix& Amat, dgematrix& Umat, dgematrix& Lmat, dgematrix& Pmat)
{
  const int M(Amat.m);
  std::vector<int> Pvec;
  ...
  Pmat.resize(M, M);
  Pmat.zero();
  for (int ir = 0; ir < M; ir++) Pmat(ir, Pvec[ir]) = 1;
#ifdef MC__DEBUG_DGETRF
  std::cout << "Matrix P:\n" << Pmat << std::endl;
#endif
...
*/
inline int
dgeqrf(const dgematrix& Amat, dgematrix& Qmat, dgematrix& Rmat)
{
  VERBOSE_REPORT;
  dgematrix A(Amat);
  const int M(Amat.m), N(Amat.n), NM = (M < N ? M : N);
  ...
  {
    WARNING_REPORT;
    std::cerr << "DGEQRF: Serious trouble happend. INFO = " << INFO << "."
              << std::endl;
    return INFO;
  }
  ...
  Rmat.zero();
  for (int ir = 0; ir < NM; ir++)
    for (int ic = ir; ic < N; ic++) Rmat(ir, ic) = A(ir, ic);
...
inline int
dgeqrf(const dgematrix& Amat, double& Adet)
{
  VERBOSE_REPORT;
  if (Amat.m != Amat.n)
  {
    ERROR_REPORT;
    std::cerr << "This matrix is not square and has no determinant."
              << std::endl
              << "Your input was (" << Amat.m << "x" << Amat.n << ")."
              << std::endl;
    return -1;
  }
  ...
  Adet = -1.;
  for (int ir = 0; ir < NM; ir++) Adet *= -A(ir, ir);
#ifdef MC__DEBUG_DGEQRF
  std::cout << "A = " << Amat << std::endl;
...
inline int
dgesv(const dgematrix& mat, dgematrix& mat_inv)
{
  VERBOSE_REPORT;
  if (mat.m != mat.n)
  {
    ERROR_REPORT;
    std::cerr << "This matrix is not square and has no inverse matrix."
              << std::endl
...
inline int
dgbsv(const dgbmatrix& mat, dgematrix& mat_inv)
{
  VERBOSE_REPORT;
  if (mat.m != mat.n)
  {
