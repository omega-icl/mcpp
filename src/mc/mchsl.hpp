...
extern "C" void mc13d_(const int*, const int*, const int*, const int*,
                       const int*, int*, int*, int*, int*);
extern "C" void mc21a_(const int*, const int*, const int*, const int*,
                       const int*, int*, int*, int*);
extern "C" void mc33ad_(const int*, const int*, const int*, int*, const int*,
...
////////////////////////////////////////////////////////////////////////
{
 public:
  ...
  FFHSL(FFGraph* dag = nullptr) : _dag(dag) {}
  ...
  //! the output arguments are the same as in the <a
  //! href="http://www.hsl.rl.ac.uk/catalogue/mc13.html">documentation of
  //! MC13</a>
  bool MC13(const unsigned nDep, const FFVar* pDep, const FFVar* pIndep,
            int* IPERM, int* IOR, int* IB, int& NB, const bool disp = false,
            std::ostream& os = std::cout);
  //! @brief Perform bordered-block triangular reaarangement of a possibly
  //! non-square system - the output arguments are the same as in the <a
  //! href="http://www.hsl.rl.ac.uk/catalogue/mc33.html">documentation of
  //! MC33</a>
  bool MC33(const unsigned nDep, const FFVar* pDep, const unsigned nIndep,
            const FFVar* pIndep, int* IP, int* IQ, int* IPROF, int* IFLAG,
 ...
 private:
  ...
  FFHSL(FFHSL const&)            = delete;
  FFHSL& operator=(FFHSL const&) = delete;
};

////////////////////////////////// FFNum ///////////////////////////////////////

inline bool
FFHSL::MC13(unsigned const nDep, FFVar const* pDep, FFVar const* pIndep,
            int* IPERM, int* IOR, int* IB, int& NB, bool const disp,
            std::ostream& os)
{
  ...
  _dag->eval(sgDep, wkDep, nDep, pDep, vDep.data(), nVar, pVar.data(),
             vVar.data());
  ...
  for (unsigned i = 0; i < nDep; i++)
  {
    ...
    for (; cit != vDep[i].dep().end(); ++cit) ICN.push_back((*cit).first + 1);
  }
...
#else
  int NUMNZ;
  mc21a_(&N, ICN.data(), &LICN, IP.data(), LENR.data(), IPERM, &NUMNZ,
         IW.data());
  ...
  mc13d_(&N, ICN.data(), &LICN, IP.data(), LENR.data(), IOR, IB, &NB,
         IW.data());
#ifdef MC__MC13_DEBUG
...
inline bool
FFHSL::MC33(unsigned const nDep, FFVar const* pDep, unsigned const nIndep,
            FFVar const* pIndep, int* IP, int* IQ, int* IPROF, int* IFLAG,
            bool const disp, std::ostream& os)
{
  // Get list of operations
  ...
  mc33ad_(&M, &N, &NZI, &NZO, &ITYPE, A.data(), IRN.data(), JCN.data(), IP, IQ,
          IPROF, IFLAG, IW.data(), IW1.data(), &IERR);
  ...
  {
    os << std::endl
       << "Bordered-block Lower-triangular structure:" << std::endl
       << std::right << "   ";
    for (unsigned j = 0; j < nIndep; j++)
      os << (j + IFLAG[2] - IFLAG[1] ? " " : " | ") << std::setw(4)
         << pIndep[IQ[j] - 1];
    os << std::endl;
    for (unsigned i = 0; i < nDep; i++)
    {
