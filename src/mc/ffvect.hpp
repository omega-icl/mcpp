...
////////////////////////////////////////////////////////////////////////
{
 private:
  ...
  struct Worker
  {
    ...
    //! @brief set DAG copy
    bool
    set(FFGraph* _pDAG, std::vector<FFVar> const& _vDep,
        std::vector<FFVar> const& _vVar, std::vector<FFVar> const& _vCst)
    {
      try
      {
        if (pDAG) delete pDAG;
    ...
    template <typename U>
    void
    eval(std::vector<U>& wkU, U* uDep, U const* uVar, U const* uCst)
    {
#ifdef MC__FFVECT_DEBUG
      pDAG->output(sgDep);
#endif
      if (uCst)
        pDAG->eval(sgDep, wkU, vDep.size(), vDep.data(), uDep, vVar.size(),
                   vVar.data(), uVar, vCst.size(), vCst.data(), uCst);
      else
        pDAG->eval(sgDep, wkU, vDep.size(), vDep.data(), uDep, vVar.size(),
                   vVar.data(), uVar);
    }
#ifdef MC__FFVECT_DEBUG
    void
    eval(std::vector<double>& wkU, double* uDep, double const* uVar,
         double const* uCst)
    {
      pDAG->output(sgDep);
      for (size_t i = 0; i < vVar.size(); ++i)
        std::cout << "Var: " << vVar[i] << " = " << uVar[i] << std::endl;
      for (size_t i = 0; i < vCst.size(); ++i)
        std::cout << "Cst: " << vCst[i] << " = " << uCst[i] << std::endl;
      if (uCst)
        pDAG->eval(sgDep, wkU, vDep.size(), vDep.data(), uDep, vVar.size(),
                   vVar.data(), uVar, vCst.size(), vCst.data(), uCst);
      else
      ...
      for (size_t i = 0; i < vDep.size(); ++i)
        std::cout << "Dep:" << vDep[i] << " = " << uDep[i] << std::endl;
    }
#endif
  };
 ...
 public:
  //! @brief Default constructor
  Vect() : _pDAG(nullptr), _nVar(0), _nCst(0), _nFun(0) {}

  //! @brief Default constructor
  Vect(FFGraph* pDAG, std::vector<FFVar> const& vVar,
       std::vector<std::vector<FFVar>> const& vFun =
           std::vector<std::vector<FFVar>>())
  {
  ...
  bool
  set(FFGraph* pDAG, size_t nVar, FFVar const* pVar,
      std::vector<std::vector<FFVar>> const& vFun =
          std::vector<std::vector<FFVar>>())
  {
    return set(pDAG, std::vector<FFVar>(pVar, pVar + nVar),
               std::vector<FFVar>(), vFun);
  }

  bool
  set(FFGraph* pDAG, size_t nVar, FFVar const* pVar, size_t nCst,
      FFVar const* pCst,
      std::vector<std::vector<FFVar>> const& vFun =
          std::vector<std::vector<FFVar>>())
  {
    return set(pDAG, std::vector<FFVar>(pVar, pVar + nVar),
               std::vector<FFVar>(pCst, pCst + nCst), vFun);
  }

  bool
  set(FFGraph* pDAG, std::vector<FFVar> const& vVar,
      std::vector<std::vector<FFVar>> const& vFun =
          std::vector<std::vector<FFVar>>())
  {
          ...
          std::vector<std::vector<FFVar>>())
  {
    ...
    for (size_t i = 0; i < vFun.size(); ++i)
    {
      ...
      if (!_vWkr.back().set(pDAG, vFun[i], vVar, vCst)) return false;
    }
    return true;
  }
  ...
  std::vector<std::vector<FFVar>> const&
  vFun() const
  {
    return _vFun;
  }
  ...
  template <typename U>
  void eval_serial(U* y, U const* x, U const* c);

  //! @brief Evaluate function
  template <typename U>
  void eval_parallel(U* y, U const* x, U const* c);
};

template <typename U>
inline void
Vect::eval_serial(U* y, U const* x, U const* c)
{
  // static thread_local std::vector<U> wkU;
  std::vector<U> wkU;
  U* yi = y;
  for (size_t i = 0; i < _vFun.size();
       yi += _vFun[i].size(), ++i)  // increment i last
    _vWkr[i].eval(wkU, yi, x, c);
}

template <typename U>
inline void
Vect::eval_parallel(U* y, U const* x, U const* c)
{
  ...
  for (size_t i = 0; i < _vFun.size(); yi += _vFun[i].size(), ++i)
  {  // increment i last
...
#endif
      //_vWkr[i].eval( vwkU[i], yi, x, c );
      _vThd[i] = std::thread(&Vect::Worker::eval<U>, &_vWkr[i],
                             std::ref(vwkU[i]), yi, x, c);
    }
  }
#else
  for (size_t i = 0; i < _vFun.size(); yi += _vFun[i].size(), ++i)
  ...
  for (size_t i = 0; i < _vFun.size() - 1; ++i) _vThd[i].join();
}

//! @brief C++ class defining vectorized functions as external DAG operations in
//! MC++.
////////////////////////////////////////////////////////////////////////
    ...
    : public FFOp
{
 private:
  ...
  _set(Vect* pFun)
  {
    ...
    FFVar** ppRes =
        (_pFun->nCst()
             ? insert_external_operation(*this, _pFun->nFun(), _pFun->nVar(),
                                         _pFun->vVar().data(), _pFun->nCst(),
                                         _pFun->vCst().data())
             : insert_external_operation(*this, _pFun->nFun(), _pFun->nVar(),
                                         _pFun->vVar().data()));
 ...
 public:
  //! @brief Default Constructor
  FFVect() : FFOp(EXTERN), _pFun(nullptr), _ownFun(false) {}
  ...
  // Copy constructor
  FFVect(FFVect<T> const& Op) : FFOp(Op)
  {
#ifdef MC__FFVECT_TRACE
    ...
    if (!Op._pFun)
      throw std::runtime_error(
          "FFVect::copy constructor ** Undefined function\n");
  ...
  // Define operation
  FFVar&
  operator()(size_t const iFun, Vect* pFun)
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    //        static_cast<FADType<FFVar> const*>(vVar), mVar );
      ...
      return eval_serial(nRes, static_cast<FFInv*>(vRes), nVar,
      ...
      return eval_parallel(nRes, static_cast<McCormick<T>*>(vRes), nVar,
                           static_cast<McCormick<T> const*>(vVar), mVar);
    else if (idU == typeid(Specbnd<T>))
      return eval_parallel(nRes, static_cast<Specbnd<T>*>(vRes), nVar,
                           static_cast<Specbnd<T> const*>(vVar), mVar);
    else if (idU == typeid(TVar<T>))
      return eval_parallel(nRes, static_cast<TVar<T>*>(vRes), nVar,
                           static_cast<TVar<T> const*>(vVar), mVar);
    else if (idU == typeid(CVar<T>))
      return eval_parallel(nRes, static_cast<CVar<T>*>(vRes), nVar,
      ...
      return eval_parallel(nRes, static_cast<SupVar<PWLU>*>(vRes), nVar,
    ...
    //   static_cast<PolVar<T> const*>(vVar), mVar );
      ...
      return eval_serial(nRes, static_cast<FFExpr*>(vRes), nVar,
                         static_cast<FFExpr const*>(vVar), mVar);

    throw std::runtime_error(
        "FFVect::feval: **ERROR** No evaluation method with type" +
        std::string(idU.name()) + "\n");
  }

  template <typename U>
  void eval_parallel(unsigned const nRes, U* vRes, unsigned const nVar,
                     U const* vVar, unsigned const* mVar) const;

  template <typename U>
  void eval_serial(unsigned const nRes, U* vRes, unsigned const nVar,
                   U const* vVar, unsigned const* mVar) const;
        ...
        unsigned const nVar, void* vVar) const
  {
    ...
    throw std::runtime_error(
        "FFVect::reval: **ERROR** No evaluation method with type" +
        std::string(idU.name()) + "\n");
  }
...
template <typename U>
inline void
FFVect<T>::eval_parallel(unsigned const nRes, U* vRes, unsigned const nVar,
                         U const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFVECT_TRACE
  std::cout << "FFVect::eval_parallel: " << typeid(vRes[0]).name()
            << " (generic)\n";
#endif
#ifdef MC__FFVECT_CHECK
  assert(_pFun && nVar == _pFun->nVar() + _pFun->nCst() &&
         nRes == _pFun->nFun());
#endif
...
template <typename T>
inline void
FFVect<T>::eval_parallel(unsigned const nRes, double* vRes, unsigned const nVar,
                         double const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFVECT_TRACE
...
template <typename U>
inline void
FFVect<T>::eval_serial(unsigned const nRes, U* vRes, unsigned const nVar,
                       U const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFVECT_TRACE
  std::cout << "FFVect::eval_serial: " << typeid(vRes[0]).name()
            << " (generic)\n";
#endif
#ifdef MC__FFVECT_CHECK
  assert(_pFun && nVar == _pFun->nVar() + _pFun->nCst() &&
...
template <typename T>
inline void
FFVect<T>::eval(unsigned const nRes, FFVar* vRes, unsigned const nVar,
                FFVar const* vVar, unsigned const* mVar) const
{
...
_pFun->pDAG() ); #endif
  ...
  FFVar const*const* vResVal = insert_external_operation( *this, nRes, nVar,
vVarVal.data() );
...
template <typename T>
inline void
FFVect<T>::deriv(unsigned const nRes, FFVar const* vRes, unsigned const nVar,
                 FFVar const* pVar, FFVar** vDer) const
{
#ifdef MC__FFVECT_TRACE
...
#ifdef MC__FFVECT_CHECK
  assert(_pFun && nVar == _pFun->nVar() + _pFun->nCst() &&
         nRes == _pFun->nFun() && _pFun->pDAG());
#endif

  auto&& vVar = std::vector<FFVar>(pVar, pVar + _pFun->nVar());
  auto&& vCst =
      (_pFun->nCst() ? std::vector<FFVar>(pVar + _pFun->nVar(), pVar + nVar)
                     : std::vector<FFVar>());
  std::vector<std::vector<FFVar>> vFun;
  vFun.reserve(_pFun->vFun().size());
  //  Vect FunDer( _pFun->pDAG(), vVar, vCst );
  for (auto const& vFi : _pFun->vFun()) switch (_pFun->options.AUTODIFF)
    {
      default:
      case Vect::Options::AD::F:
  ...
  for (unsigned b = 0, bik = 0, boff = 0; b < _pFun->vFun().size();
       boff += _pFun->vFun()[b].size(), ++b)
...
template <typename T>
inline void
FFVect<T>::eval(unsigned const nRes, SLiftVar* vRes, unsigned const nVar,
                SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFVECT_TRACE
