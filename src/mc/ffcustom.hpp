...
//! @brief C++ class defining operations with custom functions as external DAG
//! operations in MC++.
////////////////////////////////////////////////////////////////////////
...
class FFCustom : public FFOp
{
 private:
  ...
  std::function<std::vector<McCormick<T>>(std::vector<McCormick<T>> const&)>
      _MCEval;

  // spectral bound evaluation
  std::function<std::vector<Specbnd<T>>(std::vector<Specbnd<T>> const&)>
      _SBEval;
  ...
  std::function<std::vector<SupVar<PWCU>>(std::vector<SupVar<PWCU>> const&)>
      _PWCSMEval;

  // piecewise-linear superposition model evaluation
  std::function<std::vector<SupVar<PWLU>>(std::vector<SupVar<PWLU>> const&)>
      _PWLSMEval;

  // default evaluation returning empty vector
  template <typename U>
  static std::vector<U>
  _Error(std::vector<U> const& vVar)
  {
    return std::vector<U>();
  }
 ...
 public:
  ...
  // Destructor
  virtual ~FFCustom() { delete _Deriv; }

  // Define operation
  FFVar**
  operator()(size_t const nDep, std::vector<FFVar> const& vVar, int const uid)
  {
#ifdef MC__FFCustom_CHECK
    assert(nDep);
#endif
    _uid = uid;  // unique identifier - used for comparison
    return this->insert_external_operation(*this, nDep, vVar.size(),
                                           vVar.data());
  }

  FFVar&
  operator()(size_t const iDep, size_t const nDep,
  ...
  FFVar&
  operator()(std::vector<FFVar> const& vVar, int const uid)
  {
    _uid = uid;  // unique identifier - used for comparison
    return *(
        this->insert_external_operation(*this, 1, vVar.size(), vVar.data())[0]);
  }

  // Set custom evaluation functions in double arithmetic
  void
  set_eval(std::function<std::vector<double>(std::vector<double> const&)> const&
               DEval)
  {
    _DEval = DEval;
  }
  ...
  void
  set_eval(std::function<std::vector<T>(std::vector<T> const&)> const& IEval)
  {
    _IEval = IEval;
  }
  ...
  void
  set_eval(std::function<std::vector<McCormick<T>>(
               std::vector<McCormick<T>> const&)> const& MCEval)
  {
    _MCEval = MCEval;
  }
  ...
  void
  set_eval(std::function<std::vector<Specbnd<T>>(
               std::vector<Specbnd<T>> const&)> const& SBEval)
  {
    _SBEval = SBEval;
  }
  ...
  void
  set_eval(
      std::function<std::vector<TVar<T>>(std::vector<TVar<T>> const&)> const&
          TMEval)
  {
    _TMEval = TMEval;
  }
  ...
  void
  set_eval(
      std::function<std::vector<CVar<T>>(std::vector<CVar<T>> const&)> const&
          CMEval)
  {
    _CMEval = CMEval;
  }
  ...
  void
  set_eval(
      std::function<std::vector<SCVar<T>>(std::vector<SCVar<T>> const&)> const&
          SCMEval)
  {
    _SCMEval = SCMEval;
  }

  // Set custom evaluation functions in piecewise-constant superposition model
  // arithmetic
  void
  set_eval(std::function<std::vector<SupVar<PWCU>>(
               std::vector<SupVar<PWCU>> const&)> const& PWCSMEval)
  {
    _PWCSMEval = PWCSMEval;
  }

  // Set custom evaluation functions in piecewise-linear superposition model
  // arithmetic
  void
  set_eval(std::function<std::vector<SupVar<PWLU>>(
               std::vector<SupVar<PWLU>> const&)> const& PWLSMEval)
  {
  ...
  void
  set_deriv(FFCustom<T> const& Deriv, int const uid)
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "FFCustom::feval ** No evaluation method for type" +
        std::string(idU.name()) + "\n");
  }
  ...
  void eval(size_t const nRes, double* vRes, size_t const nVar,
            double const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, T* vRes, size_t const nVar, T const* vVar,
            unsigned const* mVar) const;

  void eval(size_t const nRes, McCormick<T>* vRes, size_t const nVar,
            McCormick<T> const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, Specbnd<T>* vRes, size_t const nVar,
            Specbnd<T> const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, TVar<T>* vRes, size_t const nVar,
            TVar<T> const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, CVar<T>* vRes, size_t const nVar,
            CVar<T> const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, SCVar<T>* vRes, size_t const nVar,
            SCVar<T> const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, SupVar<PWCU>* vRes, size_t const nVar,
            SupVar<PWCU> const* vVar, unsigned const* mVar) const;

  void eval(size_t const nRes, SupVar<PWLU>* vRes, size_t const nVar,
  ...
  //  void eval
  //    ( size_t const nRes, PolVar<T>* vRes, size_t const nVar, PolVar<T>
  ...
  {
    //      if( idU == typeid( T ) )
    //        return reval( nRes, static_cast<T*>(vRes), nVar,
    //        static_cast<T*>(vVar) );
    //      else if( idU == typeid( PolVar<T> ) )
    ...
    throw std::runtime_error(
        "FFCustom::reval ** No evaluation method for type" +
        std::string(idU.name()) + "\n");
  }
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, double* pRes, size_t const nVar,
                  double const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: double\n";
#endif

  std::vector<double> const& vRes =
      _DEval(std::vector<double>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
        std::string(typeid(*pRes).name()) + "\n");
  for (unsigned j = 0; j < nRes; ++j) pRes[j] = vRes[j];
}

template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, T* pRes, size_t const nVar, T const* pVar,
                  unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, McCormick<T>* pRes, size_t const nVar,
                  McCormick<T> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: McCormick<T>\n";
#endif

  std::vector<McCormick<T>> const& vRes =
      _MCEval(std::vector<McCormick<T>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, Specbnd<T>* pRes, size_t const nVar,
                  Specbnd<T> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: Specbnd<T>\n";
#endif

  std::vector<Specbnd<T>> const& vRes =
      _SBEval(std::vector<Specbnd<T>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, TVar<T>* pRes, size_t const nVar,
                  TVar<T> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: TVar<T>\n";
#endif

  std::vector<TVar<T>> const& vRes =
      _TMEval(std::vector<TVar<T>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, CVar<T>* pRes, size_t const nVar,
                  CVar<T> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: CVar<T>\n";
#endif

  std::vector<CVar<T>> const& vRes =
      _CMEval(std::vector<CVar<T>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, SCVar<T>* pRes, size_t const nVar,
                  SCVar<T> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: SCVar<T>\n";
#endif

  std::vector<SCVar<T>> const& vRes =
      _SCMEval(std::vector<SCVar<T>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, SupVar<PWCU>* pRes, size_t const nVar,
                  SupVar<PWCU> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: SupVar<PWCU>\n";
#endif

  std::vector<SupVar<PWCU>> const& vRes =
      _PWCSMEval(std::vector<SupVar<PWCU>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, SupVar<PWLU>* pRes, size_t const nVar,
                  SupVar<PWLU> const* pVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  std::cout << "FFCustom::eval: SupVar<PWLU>\n";
#endif

  std::vector<SupVar<PWLU>> const& vRes =
      _PWLSMEval(std::vector<SupVar<PWLU>>(pVar, pVar + nVar));
  if (vRes.empty())  // throw typename FFBase::Exceptions(
                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error(
        "FFCustom::eval ** No evaluation method for type " +
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, FFDep* vRes, size_t const nVar,
                  FFDep const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, FFInv* vRes, size_t const nVar,
                  FFInv const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, FFExpr* vRes, size_t const nVar,
                  FFExpr const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, FFVar* vRes, size_t const nVar,
                  FFVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
  ...
  for (unsigned i = 0; i < nRes; ++i) vRes[i] = *(ppRes[i]);
}
/*
template< typename T >
inline void
...
fadbad::F<double> const* vVar, unsigned const* mVar ) const
{
  ...
  for( unsigned j=0; j<vRes[0].size(); ++j ){
    ...
    for( unsigned i=0; i<nVar; ++i ){
      ...
      vRes[0][j] += (_nCoef<nVar? _ptrCoef[0]*vVar[i][j]:
_ptrCoef[i]*vVar[i][j]);
    }
  }
}
...
template <typename T>
inline void
FFCustom<T>::deriv(unsigned const nRes, FFVar const* pRes, unsigned const nVar,
                   FFVar const* pVar, FFVar** pDer) const
{
#ifdef MC__FFCUSTOM_TRACE
  ...
  if (!_Deriv ||
      _Deriv->type != FFOp::EXTERN)  // throw typename FFBase::Exceptions(
                                     // FFBase::Exceptions::EXTERN );
    throw std::runtime_error("FFCustom::deriv ** No differentiation method\n");
  FFVar** ppDer = (*_Deriv)(nVar * nRes, std::vector<FFVar>(pVar, pVar + nVar),
                            _Deriv->_uid);
...
template <typename T>
inline void
FFCustom<T>::eval(size_t const nRes, SLiftVar* vRes, size_t const nVar,
                  SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFCUSTOM_TRACE
...
template <typename T>
inline bool
FFCustom<T>::lt(FFOp const* other) const
{
#ifdef MC__FFCUSTOM_TRACE
