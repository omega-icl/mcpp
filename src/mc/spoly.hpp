...
////////////////////////////////////////////////////////////////////////
{
 public:
  ...
  friend SPoly<K, C> sqr(SPoly<K, C> const&);
  template <typename K, typename C>
  friend SPoly<K, C> operator-(SPoly<K, C> const&);
  template <typename K, typename C>
  friend std::ostream& operator<<(std::ostream&, SPoly<K, C> const&);
 ...
 protected:
  ...
  SPoly<KEY, COMP>& _set(double const d);

  //! @brief Set polynomial expression equal to variable <a>x</a>
  SPoly<KEY, COMP>& _set(KEY const& x);

  //! @brief Set polynomial expression equal to monomial <a>mon</a>
  SPoly<KEY, COMP>& _set(std::pair<t_mon, double> const& mon);

  //! @brief Set polynomial expression equal to monomial map <a>mapmon</a>
  SPoly<KEY, COMP>& _set(t_poly const& mapmon);

  //! @brief Set polynomial expression equal to <a>spoly</a>
  SPoly<KEY, COMP>& _set(SPoly<KEY, COMP> const& spoly);

  //! @brief Clean sparse polynomial by removing zero entries
  void _clean(t_poly& spoly) const;

  //! @brief Recursive product of univariate sparse polynomials
  void _sprod1D(std::map<unsigned, t_poly> const& sp1map,
                std::map<unsigned, t_poly> const& sp2map, t_poly& mapmon,
                t_var const& setvar,
                typename t_var::const_iterator itvar) const;

  //! @brief Display of recursive univariate sparse polynomial
  void _sdisp1D(t_poly const& spoly, std::string const& name,
                std::ostream& os = std::cout) const;

  //! @brief Display of recursive univariate sparse polynomial
  void _sdisp1D(std::map<unsigned, t_poly> const& mapspoly,
                typename t_var::const_iterator itvar, std::string const& name,
                std::ostream& os = std::cout) const;

  //! @brief Display of recursive univariate sparse polynomial
  void _sdisp1D(std::map<unsigned, SPoly<KEY, COMP>> const& mapspoly,
                typename t_var::const_iterator itvar, std::string const& name,
                std::ostream& os = std::cout) const;

  //! @brief Build univariate sparse polynomial (with sparse polynomial
  //! coefficients)
  static void _svec1D(typename t_var::const_iterator itvar,
                      std::pair<t_mon, double> const& mon,
                      std::map<unsigned, t_poly>& mapspoly);

  //! @brief Scaling of univariate sparse polynomial
  static void _sscal1D(t_poly const& spoly, double const& dscal,
                       t_poly& spscal);

  //! @brief Lifting of sparse polynomial coefficient maps
  static void _slift1D(t_poly const& spoly, double const& dscal,
                       t_poly& splift);

  //! @brief Lifting of sparse polynomial coefficient maps
  static void _slift1D(t_poly const& spoly, double const& dscal, t_poly& splift,
                       typename t_var::const_iterator itvar,
                       unsigned const iord);

  //! @brief Convert univariate polynomial from Chebyshev to monomial basis
  void _convert(t_poly& mapmon, t_var const& setvar, int const BASIS) const;

  //! @brief Build univariate sparse polynomial (with sparse polynomial
  //! coefficients)
  static void _svec1Dfull(typename t_var::const_iterator itvar,
                          std::pair<t_mon, double> const& mon,
                          std::map<unsigned, t_poly>& mapspoly);
  ...
  static void _pow2cheb(std::map<unsigned, t_poly>& mapspoly);

  //! @brief Convert univariate polynomial from Chebyshev to monomial basis
  static void _cheb2pow(std::map<unsigned, t_poly>& mapspoly);
 ...
 public:
  ...
  SPoly(KEY const& x, double const& coef = 1.)
  {
    _init();
    _set(x);
    if (coef != 1.) operator*=(coef);
  }

  //! @brief Constructor of sparse polynomial expression as monomial
  SPoly(std::pair<t_mon, double> const& mon)
  {
    _init();
    _set(mon);
  }
  ...
  SPoly(SPoly<KEY, COMP> const& spoly)
  {
    _init();
    _set(spoly);
  }
  ...
  //! @brief Exchanges the contents of current sparse polynomial with those of
  //! <a>spoly</a>
  void swap(SPoly<KEY, COMP>& spoly);

  //! @brief Display sparse coefficient map
  static std::string display(
      t_poly spoly, int const& BASIS = SPoly<KEY, COMP>::options.BASIS,
      int const& DISPLEN = SPoly<KEY, COMP>::options.DISPLEN,
      bool const ONELINE = SPoly<KEY, COMP>::options.DISPLINE);

  //! @brief Clean sparse polynomial by removing entries below threshold
  //! <a>tol</a>
  void clean(double const& tol);

  //! @brief Convert coefficient map to desired <a>BASIS</a>
  SPoly<KEY, COMP>& convert(int const BASIS);

  //! @brief Check if polynomial is multiple of other polynomial <a>spoly</a>
  std::pair<bool, double> ismultiple(SPoly<KEY, COMP> const& spoly) const;
  ...
  double
  coef(t_mon const& mon = t_mon()) const
  {
  ...
  template <typename T>
  T eval(std::map<KEY, T, COMP> const& x) const;

  //! @brief Factor polynomial with respect to variable <a>ivar</a>
  std::map<unsigned, SPoly<KEY, COMP>> factor(KEY ivar) const;

  //! @brief Differentiate polynomial with respect to variable <a>ivar</a>
  SPoly<KEY, COMP> diff(KEY ivar) const;
  ...
  SPoly<KEY, COMP>&
  operator=(SPoly<KEY, COMP> const& spoly)
  {
    _set(spoly);
    return *this;
  }
  ...
  SPoly<KEY, COMP>&
  operator=(std::pair<t_mon, double> const& mon)
  {
    _set(mon);
    return *this;
  }
  ...
  //! @brief Overloaded operator '=' for monomial
  SPoly<KEY, COMP>&
  operator=(std::pair<KEY, double> const& mon)
  {
    _set(mon);
    return *this;
  }
  ...
  //! @brief Overloaded operator '+=' for constant
  SPoly<KEY, COMP>& operator+=(double const& d);

  //! @brief Overloaded operator '+=' for sparse polynomial
  SPoly<KEY, COMP>& operator+=(SPoly<KEY, COMP> const& spoly);

  //! @brief Overloaded operator '+=' for monomial
  SPoly<KEY, COMP>& operator+=(std::pair<t_mon, double> const& mon);
  ...
  class Exceptions
  {
   public:
    ...
    enum TYPE
    {
      ...
      // WRONGBASIS = 3, //!< Operation between two polynomials in different
      // bases
    ...
    what()
    {
      switch (_ierr)
      {
        ...
        // case WRONGBASIS:
        //   return "mc::SPoly\t Operation between two polynomials in different
        //   bases";
        case INTERNAL:
        default:
  ...
  {
    //! @brief Constructor
    Options() : BASIS(MONOM), REMZERO(true), DISPLEN(5), DISPLINE(true) {}
    //! @brief Copy of mc::SPoly::Options
    Options(Options const& opt)
    {
      BASIS    = opt.BASIS;
      REMZERO  = opt.REMZERO;
      DISPLEN  = opt.DISPLEN;
    ...
    bool REMZERO;
    //! @brief Number of digits in output stream for sparse polynomial
    //! coefficients
    unsigned DISPLEN;
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::swap(SPoly<KEY, COMP>& spoly)
{
...
{
  _reinit();
  if (!isequal(d, 0.)) _mapmon.insert(std::make_pair(t_mon(), d));
  return *this;
}
...
SPoly<KEY, COMP>::_set(std::pair<t_mon, double> const& mon)
{
  _cleanup();
  _mapmon.insert(mon);
  for (auto&& var : mon.first.expr) _setvar.insert(var.first);
  return *this;
}
...
SPoly<KEY, COMP>::_set(t_poly const& mapmon)
{
  ...
  for (auto const& [mon, coef] : mapmon)
    for (auto&& var : mon.expr) _setvar.insert(var.first);
  return *this;
}
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_clean(t_poly& spoly) const
{
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::clean(double const& tol)
{
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_pow2cheb(std::map<unsigned, t_poly>& spmap)
{
  // Converts COEFF in monomial basis to the Chebyshev basis.
  // Based on SCONMC function (http://www.netlib.org/math/MATH77/dconmc.f,
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_cheb2pow(std::map<unsigned, t_poly>& spmap)
{
  // Converts COEFF in Chebyshev basis to the monomial basis.
  // Based on SCONCM function (http://www.netlib.org/math/MATH77/dconcm.f,
  // http://www.netlib.org/math/docpdf/ch11-03.pdf)
  int const N = spmap.crbegin()->first;
  if (N <= 1) return;
  //    TP = 1.D0
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_svec1Dfull(typename t_var::const_iterator itvar,
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_convert(t_poly& mapmon, t_var const& setvar,
                           int const BASIS) const
{
  // Constant or linear polynomial - nothing to do
  if (mapmon.empty() || setvar.empty() || mapmon.crbegin()->first.tord <= 1)
    return;
...
//      // Add univariate term if a higher-order univariate term is present
//      already if( jt != dec.end() ){
//        jt->second += *it;
//        continue;
//      }
//      // Create new univariate expression if order is sufficiently large
//      else if( mon.tord >= ordmin ){
//        dec.insert( std::make_pair( pvar, *it ) ); // NEED CONSTRUCTOR FOR
//        MONOMIALS! continue;
//      }
//    }

//    // Otherwise, retain term in the multivariate polynomial
//    auto pt = dec.insert( std::make_pair( pvar, *it ) ); // NEED CONSTRUCTOR
//    FOR MONOMIALS! if( !pt.second )
//      pt.first->second += *it;
//  }
//
//  return dec;
...
template <typename KEY, typename COMP>
inline std::string
SPoly<KEY, COMP>::display(t_poly mapmon, int const& BASIS, int const& DISPLEN,
                          bool const DISPLINE)
{
  ...
  for (auto const& [mon, coef] : mapmon)
  {
    if (DISPLINE)
    {
      if (first)
      {
        ...
        out << " + " << coef;
      if (mon.tord) out << " " << mon.display(BASIS);
    }
    else
      out << std::right << std::setw(DISPLEN + 7) << coef << "  "
...
template <typename KEY, typename COMP>
inline std::ostream&
operator<<(std::ostream& out, SPoly<KEY, COMP> const& spoly)
{
  return out << spoly.display(spoly._mapmon, spoly.options.BASIS,
                              spoly.options.DISPLEN, spoly.options.DISPLINE);
}

template <typename KEY, typename COMP>
template <typename T>
inline T
SPoly<KEY, COMP>::eval(std::map<KEY, T, COMP> const& x) const
{
  T val(0.);
  for (auto const& [mon, coef] : _mapmon)
...
template <typename KEY, typename COMP>
inline std::map<unsigned, SPoly<KEY, COMP>>
SPoly<KEY, COMP>::factor(KEY ivar) const
{
  std::map<unsigned, SPoly<KEY, COMP>> spmap;
  auto itvar = _setvar.find(
      ivar);  // Do not use ivar below since could be a temporary variable
  if (itvar == _setvar.end()) return spmap;

  for (auto const& mon : _mapmon) _svec1Dfull(itvar, mon, spmap);
    ...
    case Options::MONOM:
      for (auto const& [iord, mapmon] : spmap)
      {
        if (!iord)
          ...
          spder += SPoly<KEY, COMP>(std::make_pair(
                       SMon<KEY, COMP>(*itvar, iord - 1), (double)iord)) *
    ...
    default:
      for (auto const& [iord, mapmon] : spmap)
      {
        if (!iord)
          ...
          spder +=
              SPoly<KEY, COMP>(std::make_pair(SMon<KEY, COMP>(*itvar, 1), 4.)) *
              SPoly<KEY, COMP>(mapmon);
        else
        {
          SPoly<KEY, COMP> monder(iord % 2 ? (double)iord : 0.);
          for (int jord = iord - 1; jord > 0; jord -= 2)
            monder += SPoly<KEY, COMP>(
                std::make_pair(SMon<KEY, COMP>(*itvar, jord), 2. * iord));
...
template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
operator+(SPoly<KEY, COMP> const& spoly1, double const d)
{
...
template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
operator+(double const d, SPoly<KEY, COMP> const& spoly1)
{
  SPoly<KEY, COMP> spoly2(spoly1);
  spoly2 += d;
  return spoly2;
...
template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
operator+(SPoly<KEY, COMP> const& spoly1, SPoly<KEY, COMP> const& spoly2)
{
...
{
  if (isequal(d, 0.))
    throw typename SPoly<KEY, COMP>::Exceptions(
        SPoly<KEY, COMP>::Exceptions::DIVZERO);
...
SPoly<KEY, COMP>::operator/=(SPoly<KEY, COMP> const& spoly)
{
  ...
  throw typename SPoly<KEY, COMP>::Exceptions(
      SPoly<KEY, COMP>::Exceptions::DIVPOLY);
}
...
template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
sqr(SPoly<KEY, COMP> const& spoly)
{
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_sprod1D(std::map<unsigned, t_poly> const& sp1map,
                           ...
                           typename t_var::const_iterator itvar) const
{
  ...
  for (auto& [ndx1, poly1] : sp1map)
  {
    ...
    for (auto& [ndx2, poly2] : sp2map)
    {
      ...
      for (auto& mon1 : poly1) _svec1D(itvarnext, mon1, sp11map);
#ifdef MC__SPOLYEXPR_DEBUG_SPROD
      _sdisp1D(sp11map, itvarnext, "Poly #1: ");
#endif
      for (auto& mon2 : poly2) _svec1D(itvarnext, mon2, sp22map);
#ifdef MC__SPOLYEXPR_DEBUG_SPROD
  ...
  for (auto const& [ndx12, poly12] : sp12map)
  {
    ...
    if (!ndx1 && !ndx2) _slift1D(poly12, 1., mapmon);
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_sscal1D(t_poly const& spoly, double const& dscal,
                           t_poly& spscal)

{
  ...
  for (auto& [mon, coef] : spscal) coef *= dscal;
  return;
}

template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_slift1D(t_poly const& spoly, double const& dscal,
                           t_poly& splift)
{
  if (isequal(dscal, 0.)) return;
...
inline void
SPoly<KEY, COMP>::_slift1D(t_poly const& spoly, double const& dscal,
                           t_poly& splift, typename t_var::const_iterator itvar,
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_svec1D(typename t_var::const_iterator itvar,
                          std::pair<t_mon, double> const& mon,
                          std::map<unsigned, t_poly>& mapspoly)
{
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_sdisp1D(t_poly const& spoly, std::string const& name,
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_sdisp1D(std::map<unsigned, t_poly> const& mapspoly,
                           typename t_var::const_iterator itvar,
                           std::string const& name, std::ostream& os) const
{
  ...
  {
    if (!first)
      os << " + " << t_mon(*itvar, iord).display(options.BASIS) << " ·";
    os << " { ";
    _sdisp1D(spoly, "", os);
    os << " }";
    first = false;
...
template <typename KEY, typename COMP>
inline void
SPoly<KEY, COMP>::_sdisp1D(std::map<unsigned, SPoly<KEY, COMP>> const& mapspoly,
                           typename t_var::const_iterator itvar,
                           std::string const& name, std::ostream& os) const
{
  os << name;
...
template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
pow(SPoly<KEY, COMP> const& spoly, unsigned const n)
{
  switch (n)
  {
    case 0:
    ...
    case 2:
      return sqr(spoly);
    default:
      return n % 2 ? sqr(pow(spoly, n / 2)) * spoly : sqr(pow(spoly, n / 2));
  }
}

template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
cheb(SPoly<KEY, COMP> const& spoly, unsigned const n)
{
  switch (n)
  {
    case 0:
    ...
    case 1:
      return spoly;
    case 2:
      return 2 * sqr(spoly) - 1;
    default:
      return 2 * spoly * cheb(spoly, n - 1) - cheb(spoly, n - 2);
  }
}

template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
prod(unsigned int const npoly, SPoly<KEY, COMP> const* ppoly)
{
  switch (npoly)
  {
    case 0:
    ...
    case 1:
      return ppoly[0];
    default:
      return ppoly[0] * prod(npoly - 1, ppoly + 1);
  }
}

template <typename KEY, typename COMP>
inline SPoly<KEY, COMP>
monom(unsigned int const npoly, SPoly<KEY, COMP> const* ppoly,
      unsigned const* k, bool const chebbasis = false)
{
  switch (npoly)
  {
    case 0:
      return 1.;
    case 1:
      return chebbasis ? cheb(ppoly[0], k[0]) : pow(ppoly[0], k[0]);
    default:
      return (chebbasis ? cheb(ppoly[0], k[0]) : pow(ppoly[0], k[0])) *
             monom(npoly - 1, ppoly + 1, k + 1);
  }
}

template <typename KEY, typename COMP>
inline std::pair<bool, double>
SPoly<KEY, COMP>::ismultiple(SPoly<KEY, COMP> const& spoly) const
{
  if (nmon() != spoly.nmon() || maxord() != spoly.maxord())
    return std::make_pair(false, 0.);

  auto it1 = _mapmon.cbegin();
  if (it1 == _mapmon.cend()) return std::make_pair(true, 1.);

  auto it2                  = spoly._mapmon.cbegin();
  auto const& [mon1, coef1] = *it1;
  auto const& [mon2, coef2] = *it2;
  if (mon1 != mon2) return std::make_pair(false, 0.);
...
operator==(SPoly<KEY, COMP> const& spoly1, SPoly<KEY, COMP> const& spoly2)
{
  ...
  for (; it1 != spoly1.mapmon().cend(); ++it1, ++it2)
  {
    auto const& [mon1, coef1] = *it1;
    auto const& [mon2, coef2] = *it2;
    if (!isequal(coef1, coef2) || mon1 != mon2) return false;
  }
  return true;
}
...
struct lt_SPoly
{
  ...
  bool
  operator()(SPoly<KEY, COMP> const& spoly1,
             SPoly<KEY, COMP> const& spoly2) const
  {
...
//! mc::SPoly for DAG evaluation or as a template parameter in other MC++
//! classes
template <typename KEY, typename COMP>
struct Op<mc::SPoly<KEY, COMP>>
{
