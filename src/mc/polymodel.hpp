...
//! @brief C++ base class for the computation of polynomial models for
//! factorable functions: Environment
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
{
 public:
   ...
   *  @{
   */
  //! @brief Constructor of polynomial model environment for <tt>nvar</tt>
  //! variables and order <tt>nord</tt>
  PolyModel(const unsigned nvar, const unsigned nord, const bool sparse = false)
  {
    _size(nvar, nord, sparse);
  }
  //! @brief Destructor of polynomial model environment

  virtual ~PolyModel() { _cleanup(); }
  ...
  //! @brief Const pointer to array of size <tt>nmon()*nvar()</tt> with variable
  //! exponents on each monomial term. The exponent for variable <tt>ivar</tt>
  //! in monomial term <tt>imon</tt> is at position <tt>imon*nvar()+ivar</tt>.
  const unsigned*
  expmon() const
  {
    return _expmon;
  ...
  //! @brief Const pointer to array of size <a>nvar</a> of variable exponents in
  //! monomial term <tt>imon</tt> of polynomial model
  const unsigned*
  expmon(const unsigned imon) const
  {
    return _expmon + imon * _nvar;
  }

  //! @brief Index of monomial term whose variable exponents are the same as
  //! those in array <tt>iexp</tt> (of size <tt>nvar()</tt>)
  unsigned
  loc_expmon(const unsigned* iexp) const
  {
    return _loc_expmon(iexp);
  }

  //! @brief Array of size <tt>_nord</tt> with indices of first monomial term of
  //! order <tt>iord=1,...,_nord</tt> in polynomial model
  const unsigned*
  posord() const
  {
    return _posord;
  ...
  poly_size
  get_binom(const unsigned n, const unsigned k) const
  {
    return _get_binom(n, k);
  }

  //! @brief Get const pointer to array of size <tt>nvar</tt> with references
  //! for all variables in Taylor model
  const double*
  refvar() const
  {
    return _refvar;
  ...
  //! @brief Get const pointer to array of size <tt>nvar</tt> with scaling
  //! coefficients in Taylor model
  const double*
  scalvar() const
  {
    return _scalvar;
  ...
  class Exceptions
  {
   public:
    ...
    enum TYPE
    {
      ...
      MAXSIZE  //!< Maximum size of polynomial model reached (monomials indexed
               //!< as unsigned)
    };
 ...
 private:
  //! @brief Set polynomial model order <tt>nord</tt> and number of variables
  //! <tt>nvar</tt>
  void _size(const unsigned nvar, const unsigned nord, const bool sparse);
 ...
 protected:
  ...
  //! @brief Array of size <tt>_nmon*_nvar</tt> with variable exponents in
  //! monomial terms. The exponent for variable <tt>ivar</tt> in monomial term
  //! <tt>imon</tt> is at location <tt>imon*nvar()+ivar</tt>.
  unsigned* _expmon;

  //! @brief Number of monomial coefficients used to size _expmon
  unsigned _expmon_size;

  //! @brief Array of <tt>(_nvar+_nord-1)*(_nord+1)</tt> contining binomial
  //! coefficients
  poly_size* _binom;
  ...
  //! @brief Array of size <tt>_nvar</tt> with reference points for the
  //! variables
  double* _refvar;
  ...
  void _set_posord(const unsigned nord);

  //! @brief Extend array <tt>_posord</tt> up to maximum order <tt>maxord</tt>
  void _ext_posord(const unsigned maxord);

  //! @brief Populate array <tt>_expmon</tt> up to order <tt>nord</tt>
  void _set_expmon(const unsigned nord);

  //! @brief Extend array <tt>_expmon</tt> up to order <tt>maxord</tt> to
  //! accomodate <tt>maxmon</tt> coefficients
  void _ext_expmon(const unsigned maxord, const bool full = false);

  //! @brief Generate variable exponents <tt>iexp</tt> for subsequent monomial
  //! order <tt>iord</tt>
  void _next_expmon(unsigned* iexp, const unsigned iord) const;

  //! @brief Get index of monomial term with variable exponents <tt>iexp</tt> in
  //! <tt>1,...,_nmon</tt>
  unsigned _loc_expmon(const unsigned* iexp) const;

  //! @brief Populate array <tt>_binom</tt> with binomial coefficients up to
  //! order <tt>nord</tt>
  void _set_binom(const unsigned nord);

  //! @brief Extend array <tt>_binom</tt> with binomial coefficients up to order
  //! <tt>nord</tt>
  void _ext_binom(const unsigned nord);

  //! @brief Get binomial coefficient \f$\left(\stackrel{n}{k}\right)\f$
  poly_size _get_binom(const unsigned n, const unsigned k) const;

  //! @brief Compute bounds for all terms of degrees <tt>iord=0,...,_nord</tt>
  //! given the Chebyshev basis function in <tt>bndbasis</tt>
  template <typename C, typename U>
  U* _get_bndord(const C* coefmon, const U* const* bndbasis,
...
inline void
PolyModel::_size(const unsigned nvar, const unsigned nord, const bool sparse)
{
...
inline void
PolyModel::_set_posord(const unsigned nord)
{
  _posord[0] = 0;
  _posord[1] = 1;
...
inline void
PolyModel::_ext_posord(const unsigned maxord)
{
...
inline void
PolyModel::_set_expmon(const unsigned nord)
{
  ...
  for (unsigned i = 1; i <= nord; i++)
  {
    ...
    for (unsigned j = _posord[i]; j < _posord[i + 1]; j++)
    {
      ...
      for (unsigned k = 0; k < _nvar; k++) _expmon[j * _nvar + k] = iexp[k];
    }
  }
  delete[] iexp;
...
inline void
PolyModel::_next_expmon(unsigned* x, const unsigned m) const
{
  // Ensure that M>=1
  assert(m);
...
*/
inline void
PolyModel::_ext_expmon(const unsigned maxord, const bool full)
{
  _ext_binom(maxord);
  _ext_posord(maxord);
  delete[] _expmon;
  if (full)
    _expmon_size = std::pow(maxord + 1, _nvar);
  else
    _expmon_size = _posord[maxord + 1];
       ...
       jmon < _expmon_size; iord++)
  {
    ...
    for (unsigned ivar = 0; ivar < _nvar; ivar++)
      iexp[ivar] = _expmon[(_nmon - 1) * _nvar + ivar];  // 0;
...
inline unsigned
PolyModel::_loc_expmon(const unsigned* iexp) const
{
  unsigned ord = 0;
...
inline void
PolyModel::_set_binom(const unsigned nord)
{
  poly_size* p;
  unsigned k;
  ...
  mc::display(_binom_size.second, _binom_size.first, _binom, _binom_size.second,
              "_binom", std::cout);
#endif
}

inline void
PolyModel::_ext_binom(const unsigned maxord)
{
...
inline poly_size
PolyModel::_get_binom(const unsigned n, const unsigned k) const
{
#ifdef MC__POLYMODEL_CHECK
...
template <typename C, typename U>
inline U*
PolyModel::_get_bndord(const C* coefmon, const U* const* bndbasis,
                       const std::set<unsigned>& ndxmon) const
{
  ...
  {
    bndord_[i] = 0.;
    for (auto jt = ndxmon.lower_bound(_posord[i]);
         jt != ndxmon.lower_bound(_posord[i + 1]); ++jt)
    {
      unsigned k           = 0;
      ...
      for (++k; k < _nvar; k++)
        if (jexp[k]) bndmon_ *= bndbasis[k][jexp[k]];
...
template <typename C, typename U>
inline U
PolyModel::_polybound_naive(const C* coefmon, const U* bndord,
                            const std::set<unsigned>& ndxmon) const
{
  U bndpol =
      (ndxmon.empty() || ndxmon.find(0) != ndxmon.end()) ? coefmon[0] : 0.;
...
////////////////////////////////////////////////////////////////////////
{
 protected:
  ...
  //! @brief Array of size <tt>_nord+2</tt> with bounds for all terms of degrees
  //! <tt>iord=0,...,_nord</tt> as well as the remainder bound at position
  //! <tt>_nord+1</tt> of variable (possibly inaccurate - see _bndord_uptd)
  mutable T* _bndord;

  //! @brief Whether the bounds in bndord are up-to-date
  mutable bool _bndord_uptd;

  //! @brief Pointer to remainder bound of variable (possibly NULL if not
  //! computed)
  T* _bndrem;
 ...
 protected:
  //! @brief Index of first monomial term of order <a>iord</a> in polynomial
  //! model
  unsigned
  _posord(const unsigned iord) const
  {
  ...
  //! monomial term <tt>imon</tt> of polynomial model
  const unsigned*
  _expmon(const unsigned imon) const
  {
  ...
  //! those in array <tt>iexp</tt> (of size <tt>nvar()</tt>)
  const unsigned
  _loc_expmon(const unsigned* iexp) const
  {
  ...
  //! @brief Get binomial coefficient \f$\left(\stackrel{n}{k}\right)\f$
  poly_size
  _get_binom(const unsigned n, const unsigned k) const
  {
  ...
  void _resize(PolyModel* env, const bool init = false);
  ...
  //! @brief Polynomial range bounder using specified bounder <a>type</a> (pure
  //! virtual)
  ...
  //! @brief Update private/protected members of variable after manipulation the
  //! coefficients (pure virtual)
 ...
 public:
   ...
   *  @{
   */
  //! @brief Constructor of variable linked to polynomial model environment
  //! <a>env</a>
  PolyVar(PolyModel* env = 0) { _size(env); }
  ...
  virtual PolyVar<T>&
  set(PolyModel* env, const bool reset = false)
  {
    _resize(env, reset);
    return *this;
  }
  ...
  //! @brief Set multivariate polynomial coefficients in variable as
  //! <tt>coefmon</tt> - only first <tt>coefmon.first</tt> coefficients are set
  virtual PolyVar<T>&
  set(std::pair<unsigned, const double*>& coefmon)
  {
    for (unsigned imon = 0; imon < (_PM ? nmon() : 1); imon++)
      _coefmon[imon] =
          (imon < coefmon.first && coefmon.second ? coefmon.second[imon] : 0.);
    _unset_bndT();
    _unset_bndpol();
    _bndord_uptd = false;
    return *this;
  ...
  //! @brief Set multivariate polynomial coefficients and remainder term equal
  //! to those in variable <tt>var</tt>, possibly defined in another Taylor
  //! model environment with less variables or with a different expansion order.
  //! Coefficients involving other variables or higher order are initialized to
  //! 0 if <tt>reset=true</tt> (default), otherwise they are left unmodified.
  //! Higher-order terms in TV are bounded and added to the remainder bound.
  virtual PolyVar<T>& set(const PolyVar<T>& var, const bool reset = true);

  //! @brief Copy multivariate polynomial coefficients from current variable
  //! into variable <tt>var</tt>, possibly defined in another polynomial model
  //! environment with less variables or with a lower expansion order. Copied
  //! coefficients are reset to 0 in current Taylor variable if
  //! <tt>reset=true</tt>, otherwise they are left unmodified (default).
  ...
  virtual T
  bound(const int type) const
  {
    if (!_bndT)
      return _polybound(type) + *_bndrem;
    else
    {
      T bndT;
      return Op<T>::inter(bndT, _polybound(type) + *_bndrem, *_bndT)
                 ? bndT
                 : _polybound(type) + *_bndrem;
    }
  }
  ...
  //! @brief Retreive bound on all terms with (total) order <tt>iord</tt> in
  //! polynomial model
  T
  bndord(const unsigned iord) const
  {
    if (!iord || (_PM && iord <= nord()))
    {
      if (!_bndord_uptd) _update_bndord();
      return _bndord[iord];
    }
    return 0.;
  }
  ...
  //! @brief Get (possibly scaled) coefficient in monomial term with variable
  //! exponents as given in <a>iexp</a>
  double coefmon(const unsigned* iexp) const;

  //! @brief Get (possibly scaled) coefficient in monomial term with variable
  //! exponents as given in <a>iexp</a>
  double& coefmon(const unsigned* iexp);

  //! @brief Get pair of size of, and const pointer to, array of (possibly
  //! scaled) monomial coefficients
  std::pair<unsigned, const double*> coefmon() const;

  //! @brief Get pair of size of, and const pointer to, array of monomial
  //! exponents
  std::pair<unsigned, const unsigned*> expmon() const;

  //! @brief Get const pointer to kth monomial exponents
  const unsigned* expmon(const unsigned k) const;

  //! @brief Get set of monomial indices in sparse representation
  const std::set<unsigned>& ndxmon() const;

  //! @brief Assignment operator for polynomial model variables
  PolyVar<T>& operator=(const PolyVar<T>&) = delete;

  //! @brief Move operator for polynomial model variables
  PolyVar<T>& operator=(PolyVar<T>&&) = delete;
  /** @} */
};
...
PolyVar<T>::_cleanup()
{
  ...
  delete _bndT;
  _coefmon = nullptr;
  _bndord = _bndrem = _bndpol = _bndT = nullptr;
  _ndxmon.clear();
}

template <typename T>
inline void
PolyVar<T>::_resize(PolyModel* env, const bool init)
{
  if (_PM == env) return;
  _cleanup();
  _size(env);
...
template <typename T>
inline PolyVar<T>&
PolyVar<T>::set(const PolyVar<T>& var, const bool reset)
{
  ...
  // Case: dense representation
  for (unsigned jmon = 0;
       _ndxmon.empty() && jmon < var.nmon() && jmon < var._posord(nord() + 1);
       jmon++)
  {
    for (unsigned ivar = 0; ivar < nvar(); ivar++)
...
template <typename T>
inline PolyVar<T>&
PolyVar<T>::get(PolyVar<T>& var, const bool reset)
{
  if (!_PM)
  {
    var._ndxmon.clear();
  ...
  unsigned* iexp = new unsigned[nvar()];
  // Case: sparse representation
  for (auto it = _ndxmon.begin();
       it != _ndxmon.end() && *it < _posord(nord() + 1); ++it)
  {
    for (unsigned ivar = 0; ivar < nvar(); ivar++)
      iexp[ivar] = (ivar < var.nvar() ? var._expmon(*it)[ivar] : 0);
    // unsigned imon = _loc_expmon(iexp);
...
template <typename T>
inline void
PolyVar<T>::_set_bndpol(const T* bndpol)
{
  if (!bndpol)
  {
...
template <typename T>
inline void
PolyVar<T>::_set_bndT(const T* bndT)
{
  if (!bndT)
  {
...
template <typename T>
inline double
PolyVar<T>::coefmon(const unsigned* iexp) const
{
...
template <typename T>
inline std::pair<unsigned, const double*>
PolyVar<T>::coefmon() const
{
  return std::make_pair((_PM ? nmon() : 1), _coefmon);
}

template <typename T>
inline std::pair<unsigned, const unsigned*>
PolyVar<T>::expmon() const
{
...
PolyVar<T>::expmon(const unsigned imon) const
{
  return (_PM && imon < nmon() ? _PM->expmon() + imon * nvar() : 0);
}

template <typename T>
inline const std::set<unsigned>&
PolyVar<T>::ndxmon() const
{
  return _ndxmon;
}
