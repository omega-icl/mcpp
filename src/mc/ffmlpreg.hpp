...
////////////////////////////////////////////////////////////////////////
{
 ...
 public:
  ...
  MLPREG() : _nin(0), _nout(0), _nhid(0), _nwei(0) {}

  //! @brief Default constructor: {#inputs, {{#neurons fist hidden layer,
  //! activation function}, ..., {#neurons last hidden layer, activation
  //! function}, {#neurons output layer, activation function}}}
  MLPREG(
  ...
  struct Options
  {
    ...
    //! @brief Default constructor
    Options() : RELU2ABS(false), SIG2EXP(false), AUTODIFF(F) {}
  ...
  template <typename U>
  void
  eval(U* y, U const* x, U const* w) const
  {
    static thread_local std::vector<std::vector<U>> Uhid;
    _eval(y, x, w, Uhid);
  }
  void
  eval(double* y, double const* x, double const* w)
  {
    _eval(y, x, w, _Dhid);
  }
  void
       ...
       BADType<double> const* w)
  {
    _eval(y, x, w, _BDhid);
    _BDhid.clear();
  }  // Would be better to assign 0.

 private:
  template <typename U>
  void _eval(U* y, U const* x, U const* w,
             std::vector<std::vector<U>>& vhid) const;
...
template <typename U>
inline void
MLPREG<T>::_eval(U* y, U const* x, U const* w,
                 std::vector<std::vector<U>>& vhid) const
{
    ...
    for (unsigned i = 0; i < nneu; ++i, pwei += ncoef)
    {
...
#ifdef MC__FFMLPREG_DEBUG
      std::cerr << "No coefficients in neuron " << i << " in layer " << l
                << ": " << ncoef << std::endl;
#endif
      for (unsigned j = 0; j < ncoef - 1; ++j)
      {
#ifdef MC__FFMLPREG_DEBUG
        std::cerr << "layer:" << l << " neuron:" << i << " input:" << j
                  << std::endl;
#endif
      ...
      switch (_struct.second[l].second)
      {
        ...
        default:
          break;
        case Options::RELU:
          vhid[l][i] = ReLU(vhid[l][i]);
          break;
        case Options::TANH:
          vhid[l][i] = Op<U>::tanh(vhid[l][i]);
          break;
        case Options::SIGMOID:
          vhid[l][i] =
              (options.SIG2EXP ? 1. / (Op<U>::exp(-vhid[l][i]) + 1.)
                               : Op<U>::tanh(vhid[l][i] * 0.5) * 0.5 + 0.5);
          break;
      }
#ifdef MC__FFMLPREG_DEBUG
        // std::cerr << "layer:" << l << " neuron:" << i << " value: " <<
        // vhid[l][i] << std::endl;
#endif
    }
  }
  ...
  for (unsigned i = 0; i < nneu; ++i, pwei += ncoef)
  {
    ...
    {
#ifdef MC__FFMLPREG_DEBUG
      std::cerr << "layer:" << _nhid << " neuron:" << i << " input:" << j
                << std::endl;
#endif
      // if( std::fabs((pwei[1+j]) < options.ZEROTOL ) continue;
    ...
    switch (_struct.second.back().second)
    {
      ...
      case Options::TANH:
        ...
        break;
      case Options::SIGMOID:
        y[i] = (options.SIG2EXP ? Op<U>::inv(Op<U>::exp(-y[i]) + 1.)
                                : Op<U>::tanh(y[i] * 0.5) * 0.5 + 0.5);
        break;
    }
#ifdef MC__FFMLPREG_DEBUG
      // std::cerr << "layer:" << _nhid << " neuron:" << i << " value: " << y[i]
      // << std::endl;
#endif
  }
}
...
inline void
MLPREG<T>::_eval
( U* y, U const* x, std::vector<std::pair<std::vector<std::vector<U>>,int>>
vcoef, std::vector<std::vector<U>>& vhid ) const
{
  // Propagate through hidden layers
  vhid.resize( nhid );
...
vcoef[l][i].size()-1 << std::endl; #endif for( unsigned j=0;
j<(vcoef[l].first)[i].size()-1; ++j ){ #ifdef MC__FFMLP_DEBUG std::cout <<
"layer:" << l << " neuron:" << i << " input:" << j << std::endl; #endif if(
std::fabs((vcoef[l].first)[i][1+j]) < options.ZEROTOL ) continue; vhid[l][i] +=
(l? vhid[l-1][j]: x[j]) * (vcoef[l].first)[i][1+j];
      }
      switch( vcoef[l].second ){
        case Options::LINEAR:  default: break; case Options::RELU:    vhid[l][i]
= ReLU( vhid[l][i] );                          break; case Options::TANH:
vhid[l][i] = Op<U>::tanh( vhid[l][i] );                   break; case
Options::SIGMOID: vhid[l][i] = (options.SIG2EXP?
                                             1. / ( Op<U>::exp( -vhid[l][i] )
+ 1. ): Op<U>::tanh( vhid[l][i]*0.5 ) * 0.5 + 0.5); break;
      }
    }
  }
...
#ifdef MC__FFMLP_DEBUG
    std::cerr << "No inputs to neuron " << i << " in layer " << nhid << ": " <<
(vcoef.back().first)[i].size()-1 << std::endl; #endif for( unsigned j=0;
j<(vcoef.back().first)[i].size()-1; ++j ){ #ifdef MC__FFMLP_DEBUG std::cout <<
"layer:" << nhid << " neuron:" << i << " input:" << j << std::endl; #endif if(
std::fabs((vcoef.back().first)[i][1+j]) < options.ZEROTOL ) continue; y[i] +=
(nhid? vhid[nhid-1][j]: x[j]) * (vcoef.back().first)[i][1+j];
    }
    switch( vcoef.back().second ){
      case Options::LINEAR:  default: break; case Options::RELU:    y[i] = ReLU(
y[i] );                          break; case Options::TANH:    y[i] =
Op<U>::tanh( y[i] );                   break; case Options::SIGMOID: y[i] =
(options.SIG2EXP? Op<U>::inv( Op<U>::exp( -y[i] ) + 1. ): Op<U>::tanh( y[i]*0.5
) * 0.5 + 0.5); break;
    }
  }
}
    ...
    : public FFOp
{
 private:
  ...
  FFVar**
  _set(unsigned const nIn, FFVar const* pIn, unsigned const nWei,
       FFVar const* pWei, MLPREG<T>* pMLP)
  {
#ifdef MC__FFMLPREG_CHECK
    ...
    FFVar** ppRes =
        insert_external_operation(*this, _pMLP->nout(), nIn, pIn, nWei, pWei);
 ...
 public:
  //! @brief Default Constructor
  FFMLPREG() : FFOp(EXTERN), _pMLP(nullptr), _ownMLP(false) {}
  ...
  // Copy constructor
  FFMLPREG(FFMLPREG<T> const& Op) : FFOp(Op)
  {
#ifdef MC__FFMLPREG_TRACE
  ...
  // Define operation
  FFVar&
  operator()(unsigned const iOut, unsigned const nIn, FFVar const* pIn,
             unsigned const nWei, FFVar const* pWei, MLPREG<T>* pMLP)
  {
#ifdef MC__FFMLPREG_CHECK
  ...
  FFVar**
  operator()(unsigned const nIn, FFVar const* pIn, unsigned const nWei,
             FFVar const* pWei, MLPREG<T>* pMLP)
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    //        return eval( nRes, static_cast<ISVar<T>*>(vRes), nVar,
    //        static_cast<ISVar<T> const*>(vVar), mVar );
    //      else if( idU == typeid( ASVar<T> ) )
    //        return eval( nRes, static_cast<ASVar<T>*>(vRes), nVar,
    //        static_cast<ASVar<T> const*>(vVar), mVar );
    ...
    throw std::runtime_error(
        "FFMLPREG::feval: **ERROR** No evaluation method with type" +
        std::string(idU.name()) + "\n");
  }

  template <typename U>
  void eval(unsigned const nRes, U* vRes, unsigned const nVar, U const* vVar,
  ...
  //  void eval
  //    ( unsigned const nRes, PolVar<T>* vRes, unsigned const nVar, PolVar<T>
        ...
        unsigned const nVar, void* vVar) const
  {
    ...
    throw std::runtime_error(
        "FFMLPREG::reval: **ERROR** No evaluation method with type" +
        std::string(idU.name()) + "\n");
  }
    ...
    : public FFOp
{
 private:
       ...
       FFVar const* pWei, MLPREG<T>* pMLP)
  {
    ...
    FFVar** ppRes = insert_external_operation(
        *this, (nIn + nWei) * _pMLP->nout(), nIn, pIn, nWei, pWei);
 ...
 public:
  //! @brief Default Constructor
  FFGRADMLPREG() : FFOp(EXTERN), _pMLP(nullptr), _ownMLP(false) {}
  ...
  // Copy constructor
  FFGRADMLPREG(FFGRADMLPREG<T> const& Op) : FFOp(Op)
  {
#ifdef MC__FFMLPREG_TRACE
    ...
    if (!Op._pMLP)
      throw std::runtime_error(
          "FFGRADMLPREG::copy constructor ** Undefined MLP\n");
...
template <typename U>
inline void
FFMLPREG<T>::eval(unsigned const nRes, U* vRes, unsigned const nVar,
                  U const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
...
template <typename T>
inline void
FFMLPREG<T>::eval(unsigned const nRes, FFDep* vRes, unsigned const nVar,
                  FFDep const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
...
template <typename T>
inline void
FFMLPREG<T>::eval(unsigned const nRes, FFVar* vRes, unsigned const nVar,
                  FFVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
                  ...
                  unsigned const* mVar) const
{
  ...
  FFVar const* const* vResDer =
      ResDer(_pMLP->nin(), vVarVal.data(), _pMLP->nwei(),
             vVarVal.data() + _pMLP->nin(), _pMLP);
...
template <typename T>
inline void
FFMLPREG<T>::deriv(unsigned const nRes, FFVar const* vRes, unsigned const nVar,
                   FFVar const* vVar, FFVar** vDer) const
{
#ifdef MC__FFMLPREG_TRACE
  ...
  FFGRADMLPREG<T> ResDer;
  FFVar const* const* vResDer =
      ResDer(_pMLP->nin(), vVar, _pMLP->nwei(), vVar + _pMLP->nin(), _pMLP);
  for (unsigned k = 0; k < nRes; ++k)
    for (unsigned i = 0; i < nVar; ++i) vDer[k][i] = *vResDer[k + nRes * i];
}

template <typename T>
inline void
FFMLPREG<T>::eval(unsigned const nRes, SLiftVar* vRes, unsigned const nVar,
                  SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
...
template <typename T>
inline void
FFGRADMLPREG<T>::eval(unsigned const nRes, double* vRes, unsigned const nVar,
                      double const* vVar, unsigned const* mVar) const
{
...
template <typename T>
inline void
FFGRADMLPREG<T>::eval(unsigned const nRes, FFDep* vRes, unsigned const nVar,
                      FFDep const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
...
template <typename T>
inline void
FFGRADMLPREG<T>::eval(unsigned const nRes, FFVar* vRes, unsigned const nVar,
                      FFVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
...
template <typename T>
inline void
FFGRADMLPREG<T>::eval(unsigned const nRes, SLiftVar* vRes, unsigned const nVar,
                      SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLPREG_TRACE
