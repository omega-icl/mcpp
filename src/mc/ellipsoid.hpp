...
////////////////////////////////////////////////////////////////////////
{
  ...
  friend Ellipsoid mtimes
    ( const Ellipsoid&E, const arma::mat&A, const arma::vec&b );
  friend Ellipsoid minksum_ea
    ( const std::vector<Ellipsoid>&E, const arma::vec&D );
  friend std::vector<Ellipsoid> minksum_ea
    ( const std::vector<Ellipsoid>&E, const std::vector<arma::vec>&D );
  friend Ellipsoid minksum_ea
    ( const Ellipsoid&E, const std::pair<arma::vec,arma::vec>&I,
  ...
  friend double dist
    ( const Ellipsoid&E, const std::pair<arma::vec,double>&HP );
...
private:
  ...
  void _reset_auxiliary()
    {
      _eigQ.first.reset(); _eigQ.second.reset();
      _sqrtQ.reset();
      _svdQ.first.reset(); _svdQ.second.first.reset(); _svdQ.second.second.reset();
      _invQ.reset();
      _PSDchecked = false;
    }
...
public:
  ...
  Ellipsoid
    ( const arma::mat& Q, const arma::vec& c=arma::vec() ):
    _Q(0.5*(Q+Q.t())), _c(c), _PSDchecked(false)
    {
      ...
      if( _c.n_elem != _Q.n_cols ){ _c.resize(_Q.n_cols); _c.zeros(); }
    }
  ...
  Ellipsoid
    ( const arma::vec& r, const arma::vec& c=arma::vec() ):
...
#endif
      if( options.PSDCHK ){
        for( arma::uword i=0; i<r.n_elem; i++ ) if( r(i) < 0 ){
  ...
  Ellipsoid& unitball
    ( const unsigned n )
    {
      _reset_auxiliary();
      _Q.eye(n,n);
      if( _c.n_elem != n ){ _c.resize(_Q.n_cols); _c.zeros(); }
      return *this;
    }

  //! @brief Define ellipsoid of dimension \f$n\f$ with center \f$c\f$ and shape matrix \f$Q\f$
  Ellipsoid& set
    ( const arma::mat& Q=arma::mat(), const arma::vec& c=arma::vec() )
    {
#ifdef MC__ELLIPSOID_DEBUG
      assert( !c.n_elem || Q.n_cols == c.n_elem );
#endif
  ...
  Ellipsoid& extend
    ( const arma::rowvec& Qi, const double& ci=0. )
    {
#ifdef MC__ELLIPSOID_DEBUG
  ...
  const std::pair< arma::vec, arma::mat >& eigQ()
    {
  ...
  unsigned rankQ()
    {
      ...
      for( arma::uword i=0; i<_Q.n_cols; i++, rank-- )
        if( _svdQ.first(_Q.n_cols-i-1) > options.RKTOLA
         && _svdQ.first(_Q.n_cols-i-1) > options.RKTOLR*_svdQ.first(0) ) break;
      return rank;
    }

  //! @brief Return singular value decomposition of shape matrix
  const std::pair< arma::vec, std::pair<arma::mat,arma::mat> >& svdQ()
    {
      if( !_svdQ.first.n_elem || !_svdQ.second.first.n_cols || !_svdQ.second.second.n_cols )
  ...
  const arma::mat& regQ()
    {
      ...
      const double eps0 = std::max( options.RKTOLA, options.RKTOLR*svdQ().first(0) );
  ...
  arma::mat align
    ( const arma::vec&v, const arma::vec&x ) const
    {
...
private:
  ...
  static void _svd
    ( const arma::mat&Q, arma::vec&S, arma::mat&U,
      arma::mat&VT );
  ...
  static double _secant
    ( const double x0, const double x1, const double xL, const double xU,
      puniv f, const arma::vec&c1, const arma::mat&W1,
      const arma::vec&c2, const arma::mat&W2, const unsigned n );
  //! @brief Golden section search method for root finding 
  static double _goldsect
    ( const double xL, const double xU, puniv f, const arma::vec&c1,
      const arma::mat&W1, const arma::vec&c2, const arma::mat&W2,
      const unsigned n );
  ...
  static double _goldsect_iter
    ( const bool init, const double a, const double fa, const double b,
      ...
      const arma::vec&c1, const arma::mat&W1, const arma::vec&c2,
  ...
  static double _ell_fusionlambda
    ( const double a, const arma::vec&c1, const arma::mat&W1,
      const arma::vec&c2, const arma::mat&W2, const unsigned n );
  ...
  static double _ell_get_lambda
    ( const arma::vec&c1, const arma::mat&W1,
      const arma::vec&c2, const arma::mat&W2, const bool isHP );
  ...
  arma::mat&sqrtQ, bool&PSDchecked )
{
  ...
  if( D.is_empty() || U.is_empty() ){ sqrtQ.reset(); return; }
...
( const arma::mat&Q, arma::mat&Qinv )
{
  ...
  arma::mat R;
  if( !arma::inv_sympd(R, S) && !arma::inv(R, S) )
    R = arma::pinv(S);
...
( const Ellipsoid&E, const arma::mat&A, const arma::vec&b=arma::vec() )
{
#ifdef MC__ELLIPSOID_DEBUG
...
( const std::vector<Ellipsoid>&E, const arma::vec&D )
{
  ...
  Ei = E.begin(); arma::vec c_EA( (*Ei)._c );
...
{
#ifdef MC__ELLIPSOID_DEBUG
  assert( E._Q.n_cols == I.first.n_elem && ( !I.second.n_elem || E._Q.n_cols == I.second.n_elem ) );
#endif
  Ellipsoid EA;
...
( const Ellipsoid&E1, const Ellipsoid&E2 )
{
  ...
  for( unsigned it=0; it<60 && hi-lo > 1e-12; ++it ){
    ...
    if( f1 < f2 ){ hi = m2; if( f1 < Kmin ) Kmin = f1; }
    else         { lo = m1; if( f2 < Kmin ) Kmin = f2; }
  }
...
//#endif
  ...
  arma::mat Winv(n,n,arma::fill::zeros); Ellipsoid::_inv( W, Winv );
  double k = 1. - a*(1-a)*arma::dot((c2-c1), (W2*Winv*W1*(c2-c1)));
  ...
  const double&TOL=machprec() )
{
  ...
  std::cout << "HP.vT E.c > 0 ? " << ( arma::dot(v, E._c) > c? 'T': 'F' ) << std::endl;
#endif
  if( dist( E, HP ) > 0 && arma::dot(v, E._c) > c )
    return E;
  if( dist( E, HP ) > 0 && arma::dot(v, E._c) < c )
    return Ellipsoid();
#ifdef MC__ELLIPSOID_DEBUG_INTERSECTION_EA
  ...
  EM.regQ(); const arma::mat& W1 = EM.invQ();
#ifdef MC__ELLIPSOID_DEBUG_INTERSECTION_EA
  std::cout << "E (reg):\n" << EM << std::endl;
  std::cout << "rank(E._Q):\n" << EM.rankQ() << std::endl;
  std::cout << "inv(E._Q):\n" << EM.invQ() << std::endl;
...
( const arma::vec&c1, const arma::mat&W1,
  const arma::vec&c2, const arma::mat&W2, const bool isHP )
{
  assert( c1.n_elem == W1.n_cols && c2.n_elem == W2.n_cols && c1.n_elem == c2.n_elem );
  ...
  if( a >= 0. && a <= 1. ) return a;
  if( isHP ) return 1.;
  arma::vec eigW1; _eigen( W1, eigW1 );
  double detW1 = Ellipsoid::_det(eigW1);
  arma::vec eigW2; _eigen( W2, eigW2 );
  ...
  const arma::vec&c2, const arma::mat&W2, const unsigned n )
{
  ...
  arma::vec eigW; _eigen( W, eigW );
  double detW = Ellipsoid::_det(eigW);
  double traW = Ellipsoid::_trace(detW*Winv*(W1-W2));
  return k*detW*traW - n*(detW*detW) * ( arma::dot(c, (2*W1*c1-2*W2*c2+(W2-W1)*c))
...
{
  Ellipsoid Ei( E );
  typename std::vector< std::pair<arma::vec,double> >::const_iterator HPi = HP.begin();
  for( ; HPi!=HP.end(); ++HPi )
...
{
  Ellipsoid Eunit;
  Eunit._c.resize(n); Eunit._c.zeros();
  Eunit._Q.set_size(n,n); Eunit._Q.eye();
  return Eunit;
}
  ...
  const double&TOL, const unsigned&MAXIT )
{
  ...
  if( MAXDET.getPhaseValue() != SDPA::pdOPT )
    throw std::runtime_error( "mc::ellintersection_ia\t SDPA failed to reach primal-dual optimum" );
