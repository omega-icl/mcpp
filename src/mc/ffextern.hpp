...
protected:
  ...
  void _backpropagate_relax
    ( PolImg<T>* img, FFOp* pOp, PolVar<T>* vRes, PolVar<T>* vVar )
    const;
...
private:
  ...
  void _resize_lhs
    ( size_t const nPts, size_t const nDim, std::vector<std::vector<size_t>>& vBin,
      int seed )
    const;

  //! @brief Linearize spectral bound relaxation at selected sample points
  void _sample_SBcuts
    ( std::vector<Specbnd<T>> const& vSBVar, std::vector<Specbnd<T>> const& vSBRes,
      std::vector<std::vector<size_t>> const& vSBBin, std::vector<double>& DVar,
      std::vector<std::vector<double>>& DCvURef, std::vector<std::vector<double>>& DCvUCoef,
      std::vector<std::vector<double>>& DCcORef, std::vector<std::vector<double>>& DCcOCoef )
    const;

  //! @brief Append polyhedral image cuts for spectral bound relaxation
  void _append_SBcuts
    ( PolImg<T>* img, FFOp* pOp, PolVar<T> const& Res, std::set<unsigned> const& depVar, PolVar<T> const* vVar,
      double& DCvURef, double const* DCvUCoef, double& DCcORef, double const* DCcOCoef )
    const;
  ...
  //! @brief Append polyhedral image cuts for disaggregated variables in piecewise-constant superposition relaxation
  void _append_PWCScuts
    ( PolImg<T>* img, FFOp* pOp, PolVar<T>& Var, std::vector<PolVar<T>>& vAux,
      std::vector<PolVar<T>>& vDis, PWCU const& uest, PWCU const& oest )
    const;
  ...
  //! @brief Append polyhedral image cuts for dependent in piecewise-constant + slope superposition relaxation
  void _append_PWCScuts
    ( PolImg<T>* img, FFOp* pOp, PolVar<T> const& Res, PolVar<T> const* vVar,
      std::vector<std::vector<PolVar<T>>> const& vAux, std::vector<std::vector<PolVar<T>>> const& vDis,
      std::vector<PWCU> const& uest, std::vector<PWCU> const& oest, std::set<unsigned int> const& dep )
    const;
  ...
  //! @brief Generate under- nad overestimator hypeprlanes in [0,1] using a hybrid corner cutting method
  template <typename KEY, typename COMP>
  void _generate_corner_cuts
    ( typename SCVar<T,KEY,COMP>::t_poly const& coefbern, std::map<KEY,unsigned,COMP> const& degmax,
      std::vector<arma::vec>& verts, arma::mat& slopes, arma::vec& shifts )
  const;

  //! @brief Append polyhedral image cuts for sparse Chebyshev model relaxation
  void _append_SCMcuts
    ( PolImg<T>* img, FFOp* pOp, PolVar<T> const& Res, PolVar<T> const* vVar,
      std::vector<PolVar<T>>& vAux, arma::mat const& A, arma::vec const& b,
      T const& R, bool const first )
    const;
...
public:
  ...
  struct Options
  {
    ...
    Options& operator=
      ...
      {
        RELAX.clear();
        for( const auto& r : opt.RELAX )
          RELAX.insert( static_cast<RELAX_TYPE>( r ) );
        //RELAX      = opt.RELAX;
...
//MIG_RTOL
    ...
    //! @brief Whether options default to the parent polyhedral relaxation environment
    bool                              POLDEF;
    //! @brief Options for polyhedral relaxation environment
    typename PolImg<T>::Options       POLIMG;

    //! @brief Number of linearization points of spectral bound relaxation - 0: N+1 points (centre point + N LHS points)
    size_t                            SBLIN;
    //! @brief Random number generator seed in LHS sampler for linearization points of spectral bound relaxation
    int                               SBSEED;
    ...
    int                               PWCREL;
    //! @brief Whether to append cuts from slopes in piecewise-constant superposition model relaxation
    bool                              PWCSLOPE;
  ...
  for( auto relax : options.RELAX ){
    switch( relax ){
      ...
      case Options::AUX:
        ...
        //std::cout << "options.POLIMG.ALLOW_NLIN = " << options.POLIMG.ALLOW_NLIN.size() << std::endl;
        //std::cout << "options.POLIMG.ALLOW_NLIN = " << options.POLIMG.ALLOW_NLIN.count(FFOp::TANH) << std::endl;
        _POLVar.resize( nin );
        _POLRes.resize( nout );
        _POLRng.clear();
        break;
    ...
    try{
      ...
      case Options::AUX:
        ...
        if( needEval ){
          ...
          std::cerr << "By-passing forward propagation of polyhedral image" << std::endl;        
#endif
        break;
...
const
{
  ...
  for( auto relax : options.RELAX ){
    ...
    case Options::SB:
      ...
      _sample_SBcuts( _SBVar, _SBRes, _SBBin, _DVar, _SBCvURef, _SBCvUCoef, _SBCcORef, _SBCcOCoef );
      // add polyhedral cuts for each output at sampling points
      for( unsigned j=0; j<nout; ++j ){
        for( unsigned k=0; k<npts; ++k ){
          // polyhedral cut generation for SB
          _append_SBcuts( img, pOp, vRes[j], _SBRes[j].D(), vVar, _SBCvURef[k][j], &_SBCvUCoef[k][j*nin],
    ...
    case Options::SCM:
      ...
      for( unsigned j=0; j<nout; ++j ){
        ...
        std::cerr << "_SCRes[" << j << "]: " << _SCRes[j] << std::endl;
#endif
        unsigned ORD = (options.SCBERNORD > options.SCMODORD? options.SCBERNORD: options.SCMODORD); 
        auto&& [coefbern, degmax] = _SCRes[j].to_bernstein( ORD );
        // Update polyhedral variable bounds
        if( !Op<T>::inter( _IRes[j], _IRes[j], _SCRes[j].bound_bernstein( coefbern ) ) )
          std::cerr << "Empty intersection of relaxation bounds: "
                    << _IRes[j] << "  " << _SCRes[j].bound_bernstein( coefbern ) << std::endl;
        vRes[j].update( _IRes[j] );
#ifdef MC__FFEXTERN_DEBUG
        std::cerr << "bndbern[" << j << "]:" << _SCRes[j].bound_bernstein( coefbern );
        //std::cerr << "coefbern[" << j << "]:" << SCVar<T>::display( coefbern );
        std::cerr << "degmax[" << j << "]:\n";
        for( auto const& [var, deg] : degmax )
          std::cerr << var << ": " << deg << std::endl;
#endif
        _generate_corner_cuts( coefbern, degmax, _SCVert, _SCSlope[j], _SCShift[j] );
#ifdef MC__FFEXTERN_DEBUG
        std::cerr << "_SCVert[" << j << "] =\n";
        ...
        _append_SCMcuts( img, pOp, vRes[j], vVar, _SCPOLVScaled, _SCSlope[j], _SCShift[j], _SCRes[j].R(), !j );
      }
      break;
    ...
    case Options::PWCS:
      ...
      for( unsigned i=0; i<nin; ++i ){
        if( !options.PWCSLOPE )
          ...
          _append_PWCScuts( img, pOp, vVar[i], _POLPWCSAux[i], _POLPWCSDis[i], _PWCSVar[i].uest()[i],
                            _PWCSVar[i].oest()[i] );
      }
      ...
      for( unsigned j=0; j<nout; ++j ){
        ...
        if( !options.PWCSLOPE ){
          ...
          _append_PWCScuts( img, pOp, vRes[j], vVar, _POLPWCSAux, _POLPWCSDis, _PWCSRes[j].uest(0),
                            _PWCSRes[j].oest(0), _PWCSRes[j].sdep() );
          if( options.PWCSHADOW )
            _append_PWCScuts( img, pOp, vRes[j], vVar, _POLPWCSAux, _POLPWCSDis, _PWCSRes[j].uest(1),
...
template< typename T, typename O >
inline void
FFEXTERN<T,O>::_resize_lhs
( size_t const nPts, size_t const nDim, std::vector<std::vector<size_t>>& vBin,
  int seed )
const
{
  if( !nPts || !nDim || ( vBin.size() == nDim &&  vBin.back().size() == nPts ) ) return;

  // Create a "bin" indices vector from 0 to npts-1
  vBin.resize( nDim );
  vBin.back().resize( nPts );
  std::iota( vBin.back().begin(), vBin.back().end(), 0 ); // Fill 0, 1, 2... nPts-1
  ...
  for( size_t k=0; k<npts; ++k ){
    ...
    for( size_t i=0; i<nin; ++i ){
      ...
      DVar[i] = ( k==0? Op<T>::mid( vSBVar[i].I() ): 
                        Op<T>::l( vSBVar[i].I() ) + Op<T>::diam( vSBVar[i].I() ) * (( _SBBin[i][k-1] + 0.5 ) / ( npts - 1. )) );
#ifdef MC__FFEXTERN_DEBUG
      std::cout << " " << DVar[i];
#endif
    }
    ...
    for( size_t j=0; j<nout; ++j ){
      ...
      if( alphal > 0 ){
        for( auto const& i : vSBRes[j].N() ){
          double const corrSlope = alphal * ( 2. * DVar[i] - Op<T>::u( vSBVar[i].I() ) - Op<T>::l( vSBVar[i].I() ) );
          DCvUCoef[k][j*nin+i] += corrSlope;
          DCvURef[k][j] += alphal * ( DVar[i] - Op<T>::u( vSBVar[i].I() ) ) * ( DVar[i] - Op<T>::l( vSBVar[i].I() ) )
                         - corrSlope * DVar[i];
        }
      }
      for( auto const& i : vSBRes[j].D() )
      ...
      if( alphau < 0 ){
        for( auto const& i : vSBRes[j].N() ){
          double const corrSlope = alphau * ( 2. * DVar[i] - Op<T>::u( vSBVar[i].I() ) - Op<T>::l( vSBVar[i].I() ) );
          DCcOCoef[k][j*nin+i] += corrSlope;
          DCcORef[k][j] += alphau * ( DVar[i] - Op<T>::u( vSBVar[i].I() ) ) * ( DVar[i] - Op<T>::l( vSBVar[i].I() ) )
                         - corrSlope * DVar[i];
        }
      }
    }
...
const
{
  ...
  for( unsigned k=0; k<options.PWCDIV; ++k ){
    ...
    img->add_cut( pOp, PolCut<T>::LE, 0., vAux[k], -oest.yU().at(k), vDis[k], 1. );
    img->add_cut( pOp, PolCut<T>::GE, 0., vAux[k], -uest.yL().at(k), vDis[k], 1. );
  }
  ...
  img->add_cut( pOp, PolCut<T>::EQ, 0., options.PWCDIV, vDis.data(), 1., Var, -1. );
}
...
const
{
  ...
  // polyhedral cuts for superposition relaxation
  auto cutL = ( !uest.empty()? *img->add_cut( pOp, PolCut<T>::LE, 0., Res, -1. ): nullptr );
  auto cutU = ( !oest.empty()? *img->add_cut( pOp, PolCut<T>::GE, 0., Res, -1. ): nullptr );
  ...
  for( auto const& i : dep ){
    ...
    for( unsigned k=0; k<options.PWCDIV; ++k, xiref += dxi ){
      ...
      if( cutL ){
        ...
        PolVar<T> ResikL( img, T( std::min(uest[i].l(), 0.), std::max(uest[i].u(), 0.) ), true );
        cutL->append( ResikL, 1. );
        if( uest[i].yL().size() == options.PWCDIV ){
          img->add_cut( pOp, PolCut<T>::GE, 0., ResikL, 1., vAux[i][k], -uest[i].yL().at(k) );
        }
        if( uest[i].sLL().size() == options.PWCDIV && uest[i].y0().size() == options.PWCDIV+1 ){
          double const siLL = uest[i].sLL().at(k);
          double const biLL = uest[i].y0().at(k) - siLL * xiref;
          img->add_cut( pOp, PolCut<T>::GE, 0., ResikL, 1., vDis[i][k], -siLL, vAux[i][k], -biLL );
        }
        if( uest[i].sLU().size() == options.PWCDIV && uest[i].y0().size() == options.PWCDIV+1 ){
          double const siLU = uest[i].sLU().at(k);
          double const biLU = uest[i].y0().at(k+1) - siLU * ( xiref + dxi );
          img->add_cut( pOp, PolCut<T>::GE, 0., ResikL, 1., vDis[i][k], -siLU, vAux[i][k], -biLU );
        }
      }
      ...
      if( cutU ){
        ...
        PolVar<T> ResikU( img, T( std::min(oest[i].l(), 0.), std::max(oest[i].u(), 0.) ), true );
        cutU->append( ResikU, 1. );
        if( oest[i].yU().size() == options.PWCDIV ){
          img->add_cut( pOp, PolCut<T>::LE, 0., ResikU, 1., vAux[i][k], -oest[i].yU().at(k) );
        }
        if( oest[i].sUL().size() == options.PWCDIV && oest[i].y0().size() == options.PWCDIV+1 ){
          double const siUL = oest[i].sUL().at(k);
          double const biUL = oest[i].y0().at(k) - siUL * xiref;
          img->add_cut( pOp, PolCut<T>::LE, 0., ResikU, 1., vDis[i][k], -siUL, vAux[i][k], -biUL );
        }
        if( oest[i].sUU().size() == options.PWCDIV && oest[i].y0().size() == options.PWCDIV+1 ){
          double const siUU = oest[i].sUU().at(k);
          double const biUU = oest[i].y0().at(k+1) - siUU * ( xiref + dxi );
          img->add_cut( pOp, PolCut<T>::LE, 0., ResikU, 1., vDis[i][k], -siUU, vAux[i][k], -biUU );
        }
      }
    }
  }
...
// ---------------------------------------------------------
// Generates a polyhedral relaxation of a polynomial in Bernstein basis with coefficients 'coefbern'
// Polyhedral cuts are in the form Ax <= b, with x = [u_0, ..., u_N-1, z]
//
// For each corner c:
...
( typename SCVar<T,KEY,COMP>::t_poly const& coefbern,
  ...
  std::vector<arma::vec>& verts,
  arma::mat& slopes, arma::vec& shifts )
const
{
  ...
  // Generate (N_points x (d+1)) matrix of control points (u_i, val) from sparse Berstein coefficients
  verts.clear();
  ...
  for( size_t c=0; c<nVer; ++c){
    ...
    arma::vec uval( nVar );
    for( size_t i=0; i<nVar; ++i )
      uval(i) = ((c >> i) & 1) ? 1.0 : 0.0;

    for( const auto& vert : verts ){
      double vval = vert.back(); // The actual z value
      double hval = cval;
      for( size_t i=0; i<nVar; ++i )
        hval += hslope(i) * (vert(i) - uval(i));
...
template< typename T, typename O >
inline void
FFEXTERN<T,O>::_append_SCMcuts
( PolImg<T>* img, FFOp* pOp, PolVar<T> const& Res, PolVar<T> const* vVar,
  std::vector<PolVar<T>>& vVSca, arma::mat const& A, arma::vec const& b,
  T const& R, bool const first )
const
{
  ...
  if( first ){
    ...
    for( unsigned i=0; i<nVar; ++i ){
      ...
      double const d = Op<T>::diam( vVar[i].range() ), l = Op<T>::l( vVar[i].range() );
  ...
  for( unsigned k=0; k<nCut; ++k ){
    ...
    if( coefVar[nVar] < 0. )
      img->add_cut( pOp, PolCut<T>::LE, b(k)-Op<T>::l(R), nVar, vVSca.data(), coefVar, Res, coefVar[nVar] );
    else
      img->add_cut( pOp, PolCut<T>::LE, b(k)+Op<T>::u(R), nVar, vVSca.data(), coefVar, Res, coefVar[nVar] );
  }
}
