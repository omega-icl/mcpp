...
mc::FFExpr is a C++ class that constructs strings representing mathematical
expressions in factorable functions. It relies on the operator overloading and
function overloading mechanisms of C++. The overloaded operators are: `+', `-',
`*', and `/'; the overloaded functions include: `exp', `log', `sqr', `pow',
`cheb', `sqrt', `fabs', `xlog', `min', `max', `cos', `sin', `tan', `acos',
`asin', `atan', `cosh', `sinh', `tanh'.

This class may be used in combination with mc::FFGraph::eval for automatic
string representation of subgraphs in DAGs.

\section sec_FFExprOpt Available Options for String Expression

The class mc::FFExpr has a static public member called mc::FFExpr::options that
...
<TABLE border="1">
<CAPTION><EM>Options in mc::FFExpr::Options: name, type and
description</EM></CAPTION> <TR><TH><b>Name</b> <TD><b>Type</b><TD><b>Default</b>
         <TD><b>Description</b>
     <TR><TH><tt>LANG</tt> <TD><tt>FFExpr::Options::TYPE</tt> <TD>
FFExpr::Options::DAG <TD>Defines the language for the string expressions
</TABLE>
...
FFExpr::Exceptions is thrown, which contains the type of error. It is the user's
...
////////////////////////////////////////////////////////////////////////
{
 ...
 public:
  ...
  FFExpr(double const& c = 0.) : _prec(0)
  {
    if (c > 0)
      _ostr << _d2s(c);
    else if (c < 0)
  ...
  explicit FFExpr(FFVar const& X) : _prec(0)
  {
    switch (options.LANG)
    {
      case Options::DAG:
  ...
  FFExpr(std::string const& X) : _prec(0)
  {
    switch (options.LANG)
    {
      case Options::DAG:
  ...
  FFExpr(FFExpr const& E) : _prec(E._prec) { _ostr << E._ostr.str(); }
  ...
  static FFExpr compose(std::string const& UNIV, FFExpr const& E);
  static FFExpr compose(std::string const& UNIV, FFExpr const& E1,
                        FFExpr const& E2);
  static FFExpr compose(std::string const& UNIV, unsigned const n,
                        FFExpr const* E);
  static FFExpr compose(std::string const& UNIV, FFExpr const& E, int const n);
  static FFExpr compose(std::string const& UNIV, FFExpr const& E,
                        double const& d);

  //! @brief Retrieve string expression for DAG dependent
  static FFExpr dep(FFVar const& dep);
  //! @brief Retrieve string expression for subgraph
  static std::vector<FFExpr> subgraph(FFGraph* dag, FFSubgraph& sg);
  /** @} */
  ...
  FFExpr&
  operator=(FFVar const& X)
  {
    return set(X);
  }

  FFExpr&
  operator=(FFExpr const& E)
  {
    if (this != &E)
    {
      _prec = E._prec;
...
inline std::ostream&
operator<<(std::ostream& out, FFExpr const& E)
{
...
inline FFExpr
operator+(double const& c, FFExpr const& E)
{
...
inline FFExpr
operator+(FFExpr const& E, double const& c)
{
#ifdef MC__FFEXPR_TRACE
...
inline FFExpr
operator+(FFExpr const& E1, FFExpr const& E2)
{
  FFExpr _E(E1);
  return _E += E2;
}
...
inline FFExpr
sum(unsigned const n, FFExpr const* E)
{
  switch (n)
  {
    case 0:
...
operator*(FFExpr const& E1, FFExpr const& E2)
{
  if (&E1 == &E2) return sqr(E1);

  FFExpr _E(E1);
  return _E *= E2;
}

inline std::vector<FFExpr>
FFExpr::subgraph(FFGraph* dag, FFSubgraph& sg)
{
  ...
  dag->eval(sg, Dep.size(), Dep.data(), EDep.data(), Var.size(), Var.data(),
            EVar.data());
  return EDep;
}

inline FFExpr
FFExpr::dep(FFVar const& vdep)
{
...
inline FFExpr
FFExpr::compose(std::string const& UNIV, FFExpr const& E)
{
...
inline FFExpr
FFExpr::compose(std::string const& UNIV, FFExpr const& E1, FFExpr const& E2)
{
...
inline FFExpr
FFExpr::compose(std::string const& UNIV, unsigned const n, FFExpr const* E)
{
  FFExpr _E;  // sets _E._prec = 0 by default
  _E.ostr() << UNIV << "( ";
...
inline FFExpr
FFExpr::compose(std::string const& UNIV, FFExpr const& E, int const n)
{
  assert(E._ostr.tellp());
  FFExpr _E;  // sets _E._prec = 0 by default
...
inline FFExpr
FFExpr::compose(std::string const& UNIV, FFExpr const& E, double const& d)
{
  assert(E._ostr.tellp());
  FFExpr _E;  // sets _E._prec = 0 by default
...
inline FFExpr
prod(unsigned const n, FFExpr const* E)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
pow(FFExpr const& E, const int n)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
cheb(FFExpr const& E, const unsigned n)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
monom(unsigned const n, FFExpr const* E, unsigned const* k)
{
  switch (n)
  {
    case 0:
      return 0.;
    case 1:
      return pow(E[0], (int)k[0]);
    default:
      return pow(E[0], (int)k[0]) * monom(n - 1, E + 1, k + 1);
  }
}
...
inline FFExpr
lmtd(FFExpr const& E1, FFExpr const& E2)
{
  if (&E1 == &E2) return (E1);
...
inline FFExpr
rlmtd(FFExpr const& E1, FFExpr const& E2)
{
  if (&E1 == &E2) return (1 / E1);
...
inline FFExpr
pow(FFExpr const& E, double const& r)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
pow(FFExpr const& E1, FFExpr const& E2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
min(FFExpr const& E1, FFExpr const& E2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
min(const unsigned n, FFExpr const* E)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
min(FFExpr const& E1, double const& d2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
min(FFExpr const& E1, int const n2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
max(FFExpr const& E1, FFExpr const& E2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
max(FFExpr const& E1, double const& d2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
max(FFExpr const& E1, int const n2)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
inline FFExpr
max(const unsigned n, FFExpr const* E)
{
  switch (FFExpr::options.LANG)
  {
    case FFExpr::Options::DAG:
...
//! for use of mc::FFEpr in DAG evaluation or as template parameter in other
//! MC++ types
template <>
struct Op<mc::FFExpr>
{
  ...
  static FFE
  lmtd(const FFE& x, const FFE& y)
  {
    return mc::lmtd(x, y);
  }
  static FFE
  rlmtd(const FFE& x, const FFE& y)
  {
    return mc::rlmtd(x, y);
  }
  static FFE
  ...
  static FFE
  hull(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  template <typename Y>
  static FFE
  min(const FFE& x, const Y& y)
  {
    return mc::min(x, y);
  }
  template <typename Y>
  static FFE
  max(const FFE& x, const Y& y)
  {
    return mc::max(x, y);
  }
  static FFE
  arh(const FFE& x, const double k)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  ...
  static FFE
  cheb(const FFE& x, const unsigned n)
  {
    return mc::cheb(x, n);
  }
  static FFE
  prod(const unsigned n, const FFE* x)
  {
    return mc::prod(n, x);
  }
  static FFE
  monom(const unsigned n, const FFE* x, const unsigned* k)
  {
    return mc::monom(n, x, k);
  }
  static bool
  inter(FFE& xIy, const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  static bool
  eq(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  static bool
  ne(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  static bool
  lt(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  static bool
  le(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  static bool
  gt(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
  static bool
  ge(const FFE& x, const FFE& y)
  {
    throw typename FFExpr::Exceptions(FFExpr::Exceptions::UNDEF);
  }
};
