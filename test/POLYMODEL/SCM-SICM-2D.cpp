...
myfunc(const T& x, const T& y)
{
  return (
      -2.23606797749979 *
      exp(64.2558879895505 * sqr(0.194030125231034 - 0.166666666666667 * x) +
          0.451453304154821 *
  ...
  //     0.0829399801155143) -
  //     0.166666666666667*y)))*exp(-2.23606797749979*sqrt(
...
main()
{
  ...
  // #if 0
  {
    ...
    for (unsigned i = 0; i < NREP; i++) F = myfunc(X, Y);
    std::cout << "\nChebyshev model (dense implementation):"
              << (mc::userclock() - tStart) / (double)NREP << " CPU-sec\n";
  }
  // #endif
  {
    try
    {
      ...
      // modSCM.append_aux();
      // for( auto const& aux : modSCM.setaux() ) std::cout << "AUX: " << *aux
      // << std::endl; for( auto const& var : modSCM.setvar() ) std::cout <<
      // "VAR: " << *var << std::endl;
      SCV CF;
      ...
      // modSCM.append_aux();
      // for( auto const& aux : modSCM.setaux() ) std::cout << "AUX: " << aux <<
      // std::endl; for( auto const& var : modSCM.setvar() ) std::cout << "VAR:
      // " << var << std::endl;
      SCV CF;
      for (unsigned i = 0; i < NREP; i++) CF = myfunc(CX, CY);
        // modSCM.reset_aux();
#endif
      ...
      for (int iX = 0; iX < NX; iX++)
      {
        ...
        {
#ifdef USE_DAG
          map<FFVar*, double, lt_FFVar> DXY = {
              {&X, XL + iX * (XU - XL) / (NX - 1.)},
              {&Y, YL + iY * (YU - YL) / (NY - 1.)}};
          double DF;
          DAG.eval(GF, 1, &F, &DF, 1, &X, &DXY[&X], 1, &Y, &DXY[&Y]);
#else
          map<unsigned, double> DXY = {{0, XL + iX * (XU - XL) / (NX - 1.)},
                                       {1, YL + iY * (YU - YL) / (NY - 1.)}};
          ...
          res << std::setw(14) << DXY[0] << std::setw(14) << DXY[1]
              ...
              << Op<I>::l(IF) << std::setw(14) << Op<I>::u(IF) << std::endl;
        }
        res << endl;
      }
#endif
    ...
    {
      cerr << "Error " << eObj.ierr() << " in DAG evaluation:" << endl
           << eObj.what() << endl
           << "Aborts." << endl;
      return eObj.ierr();
    }
...

  {
    try
    {
      ...
      for (unsigned i = 0; i < NREP; i++) CF = myfunc(CX, CY);
#endif
      std::cout << "\nChebyshev model (sparse interval implementation):"
                << (mc::userclock() - tStart) / (double)NREP << " CPU-sec\n";
      std::cout << CF;
      // std::cout << CF.project();
