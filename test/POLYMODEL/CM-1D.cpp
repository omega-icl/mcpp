...
myfunc(const T& x)
{
  return x * (x - 1.1) * (x + 2) * (x + 2.2) * (x + 2.5) * (x + 3.) *
         sin(1.7 * x + 0.5);
  // return
  // pow(x,6)+8.6*pow(x,5)+24.33*pow(x,4)+17.2*pow(x,3)-28.27*pow(x,2)-36.3*x;
}
...
////////////////////////////////////////////////////////////////////////
{
  ...
  try
  {
    ...
    // std::cout << "\nBernstein bounder:" <<
    // CVMCF.bound(CMMC::Options::BERNSTEIN) << std::endl;
    ...
    for (int iX = 0; iX < NX; iX++)
    {
      ...
      MC MCBF = CVMCF.B();
#ifdef USE_SPARSE
      double PF = CVMCF.P(std::map<unsigned, double>({std::make_pair(0, DX)}));
#else
      ...
      res << std::scientific << std::setprecision(IPREC) << std::setw(IPREC + 9)
          << DX << std::setw(IPREC + 9) << std::setw(IPREC + 9) << DF
          << std::setw(IPREC + 9) << PF << std::setw(IPREC + 9) << MCF.l()
          << setw(IPREC + 9) << MCF.u() << std::setw(IPREC + 9) << MCF.cv()
          << setw(IPREC + 9) << MCF.cc() << std::setw(IPREC + 9) << MCBF.l()
          << std::setw(IPREC + 9) << MCBF.u() << std::setw(IPREC + 9)
          << MCBF.cv() << std::setw(IPREC + 9)
          << MCBF.cc()
          //<< std::setw(IPREC+9) << MCBF.cvsub(0) << std::setw(IPREC+9) <<
          // MCBF.ccsub(0)
          << std::setw(IPREC + 9) << a * (DX - (XL + XU) / 2.) + MCRF.l()
          << std::setw(IPREC + 9) << a * (DX - (XL + XU) / 2.) + MCRF.u()
          << std::endl;
#endif
    }
    ...
    for (unsigned i = 0; i < N; i++)
    {
      ...
      std::cout << Op<I>::diam(Xred) << "  " << Op<I>::diam(CVFred.R()) << "  "
                << (log(Op<I>::diam(CVFred0.R())) -
                    log(Op<I>::diam(CVFred.R()))) /
                       (log(Op<I>::diam(CVXred0.B())) -
                        log(Op<I>::diam(CVXred.B())))
                << std::endl;
    }
#endif
  }
  ...
  {
    cerr << "Error " << eObj.ierr() << " in McCormick relaxation:" << endl
         << eObj.what() << endl
         << "Aborts." << endl;
    return eObj.ierr();
  }
