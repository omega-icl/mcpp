...
#undef  USE_OPTIM      // <-- whether to optimize the quadratic form
#define USE_REDUC      // <-- whether to search for reduction constraint in quadratic form
////////////////////////////////////////////////////////////////////////
...
{  
  ...
  t_SPoly::options.BASIS = t_SPoly::Options::MONOM;
    ...
    for( auto const& quad : SQF.MatFct() )
      QuadFct.push_back( SQF.poly( quad ) );
    SRF.set_monomials( QuadFct.size(), QuadFct.data(), &t_SPoly::mapmon );

    std::vector< t_SPoly > QuadRed;
    QuadRed.reserve( SQF.MatRed().size() );
    for( auto const& quad : SQF.MatRed() )
      QuadRed.push_back( SQF.poly( quad ) );
    SRF.set_monomials( QuadRed.size(), QuadRed.data(), &t_SPoly::mapmon, true ); // append
    SRF.search_reductions( QuadRed.size(), QuadRed.data(), &t_SPoly::mapmon );//, false );
    cout << SRF;
#endif
  }
