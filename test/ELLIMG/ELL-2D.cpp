// Select one test case at compile time with -DTEST_0, ..., -DTEST_5.
// TEST_1 is the default, matching the original file.
#if !defined(TEST_0) && !defined(TEST_1) && !defined(TEST_2) && \
    !defined(TEST_3) && !defined(TEST_4) && !defined(TEST_5)
#define TEST_2
#endif

// Define ELL2D_NO_DAG to evaluate the factorable function directly instead.
#ifndef ELL2D_NO_DAG
#define USE_DAG
#endif

// Define ELL2D_NO_SAVE_RESULTS to disable Fsam.out, Esam.out, Isam.out.
#ifndef ELL2D_NO_SAVE_RESULTS
#define SAVE_RESULTS
#endif

const int NRAD = 40;   // <-- select radius discretization here
...
// Define MC__ELL2D_USE_TEST_INTERVAL when compiling this standalone test in a
// reduced source tree that does not provide interval.hpp. In the full MC++
// tree, leave it undefined and use the usual interval back-end selection below.
...
#ifndef MC__ELLIMAGE_HEADER
#define MC__ELLIMAGE_HEADER "ellimage.hpp"
#endif
#include MC__ELLIMAGE_HEADER
...
const int NF = 2;
dcv q0(NX);
dsm Q0(NX, NX, arma::fill::zeros);
template <class T>
T
f(const T* x, const long i)
{
  using mc::sqr;
  switch (i)
  {
...
f(const T* x, const long i)
{
  ...
  switch (i)
  {
    ...
    case 0:
      return sqrt(x[0] + x[1]) + x[0] * x[1];
    case 1:
      return pow(x[0] - x[1], 2) + 3. * x[1];
    default:
      throw std::runtime_error("invalid index");
  }
}
...
test_ellintersection_ia()
{
  ...
  dgm A(m, n, arma::fill::zeros);
  dcv b(m, arma::fill::zeros);
  A(0, 0) = -1.;
  A(0, 1) = 0.;
  b(0)    = 0.;  // x >= 0
  A(1, 0) = 1.;
  ...
  for (unsigned i = 0; i < m; i++)
    HP.push_back(std::make_pair(arma::conv_to<dcv>::from(A.row(i).t()), b(i)));

  mc::Ellipsoid Eia(mc::ellintersection_ia(HP, 1e-5));

  std::pair<dcv, dgm> res = Eia.eigQ();
...
test_intersection_ea()
{
  const unsigned n = 2;
  // center         shape
  dcv q1(n, arma::fill::zeros);
  dsm Q1(n, n, arma::fill::zeros);
  q1(0)    = 0.;
  Q1(0, 0) = 1.;
  q1(1)    = 0.;
  Q1(1, 0) = 0.;
  ...
  // center         shape
  dcv q2(n, arma::fill::zeros);
  dsm Q2(n, n, arma::fill::zeros);
  q2(0)    = 1.;
  Q2(0, 0) = 1.;
  q2(1)    = 1.;
  Q2(1, 0) = 0.;
...
main()
{
  ...
  arma::vec xy_vec(NF, arma::fill::zeros), z_sam(NX, arma::fill::zeros),
      E_sam(NF, arma::fill::zeros), Fimg(NF, arma::fill::zeros);
  while (phiEr_sam <= mc::Op<I>::u(phi))
  {
    xy_vec(0) = mc::Op<I>::u(radius) * std::cos(phiEr_sam);
    xy_vec(1) = mc::Op<I>::u(radius) * std::sin(phiEr_sam);
    ...
    while (rad_sam >= mc::Op<I>::l(radius) + discr)
    {
      ...
      {
        xy_vec(0) = rad_sam * std::cos(phi_sam);
        xy_vec(1) = rad_sam * std::sin(phi_sam);
        z_sam     = img.c() + img.sqrtQ() * xy_vec;
        // Evaluate dependent variables
        double z_d[NX] = {z_sam(0), z_sam(1)};
        for (long i = 0; i < NF; ++i) Fimg(i) = f(z_d, i);
        // print
        if (phiEr_sam == mc::Op<I>::l(phi))
          fout << Fimg(0) << '\t' << Fimg(1) << std::endl;
        phi_sam += discphi;
      }
      rad_sam -= discr;
    }
    // Evaluate at center
    double z_c[NX] = {img.c()(0), img.c()(1)};
    for (long i = 0; i < NF; ++i) Fimg(i) = f(z_c, i);
