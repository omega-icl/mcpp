...
bool
is_close(double a, double b, double atol = 1e-10, double rtol = 1e-8)
{
...
void
print_check(double ad, double ref, double atol = 1e-10, double rtol = 1e-8)
{
  const bool ok = is_close(ad, ref, atol, rtol);
  const double err =
      (std::isnan(ad) || std::isnan(ref)) ? NaN() : std::fabs(ad - ref);

  cout << std::setw(16) << ad << std::setw(16) << ref << std::setw(16) << err
       << std::setw(10) << (ok ? "OK" : "FAIL");
}

std::vector<double>
make_grid(double a, double b, double h)
{
  std::vector<double> g;
...
// ---- max(x,y) ----
double
max_val(double x, double y)
{
  return std::max(x, y);
}

double
dmax_dx(double x, double y)
{
  if (x > y) return 1.0;
  if (x < y) return 0.0;
...
// ---- min(x,y) ----
double
min_val(double x, double y)
{
  return std::min(x, y);
}

double
dmin_dx(double x, double y)
{
  if (x < y) return 1.0;
  if (x > y) return 0.0;
  return NaN();  // tie: depends on your implementation
...
double
dmax_scalar_dx(double x, double c)
{
  if (x > c) return 1.0;
  if (x < c) return 0.0;
  return NaN();
...
double
dmin_scalar_dx(double x, double c)
{
  if (x < c) return 1.0;
  if (x > c) return 0.0;
  return NaN();
...
comp1_der(double x)
{
  ...
  return der_erf * (z * std::log(z)) + std::erf(x) * dz +
...
double
comp4_val(double x, double y)
{
  const double z = y + 2.5;
  return std::fabs(x - 0.5) * std::max(y, 1.0) + z * std::log(z) + x * y;
}

double
comp4_dx(double x, double y)
{
  return sign_shift(x, 0.5) * std::max(y, 1.0) + y;
}

double
comp4_dy(double x, double y)
{
  const double z    = y + 2.5;
  const double dmax = dmax_scalar_dx(y, 1.0);
  if (std::isnan(dmax)) return NaN();

  return std::fabs(x - 0.5) * dmax + (std::log(z) + 1.0) + x;
}
...
void
test_unary_F_compare(const std::string& name, FADFunc fad_f, ValFunc ref_f,
                     DerFunc ref_df, const std::vector<double>& grid)
{
  cout << "\n=================================================================="
          "==============================================\n";
  ...
  cout << "===================================================================="
          "============================================\n";
  cout << std::setw(12) << "x" << std::setw(16) << "AD f" << std::setw(16)
       << "REF f" << std::setw(16) << "|err|" << std::setw(10) << "check"
       << std::setw(16) << "AD df" << std::setw(16) << "REF df" << std::setw(16)
       << "|err|" << std::setw(10) << "check" << "\n";
...
void
test_binary_F_compare(const std::string& name, FADFunc fad_f, ValFunc ref_f,
                      DxFunc ref_dx, DyFunc ref_dy,
                      ...
                      const std::vector<double>& ygrid)
{
  ...
  cout << "===================================================================="
          "===================================================================="
          "====================\n";
  cout << std::setw(10) << "x" << std::setw(10) << "y" << std::setw(16)
       << "AD f" << std::setw(16) << "REF f" << std::setw(16) << "|err|"
       << std::setw(10) << "check" << std::setw(16) << "AD dfdx"
       << std::setw(16) << "REF dfdx" << std::setw(16) << "|err|"
       << std::setw(10) << "check" << std::setw(16) << "AD dfdy"
       << std::setw(16) << "REF dfdy" << std::setw(16) << "|err|"
       << std::setw(10) << "check" << "\n";
  ...
  for (double xv : xgrid)
  {
    for (double yv : ygrid)
    {
      ...
      cout << std::setw(10) << xv << std::setw(10) << yv;
...
void
test_unary_B_compare(const std::string& name, BADFunc bad_f, ValFunc ref_f,
                     DerFunc ref_df, const std::vector<double>& grid)
{
  cout << "\n=================================================================="
          "==============================================\n";
...
void
test_binary_B_compare(const std::string& name, BADFunc bad_f, ValFunc ref_f,
                      DxFunc ref_dx, DyFunc ref_dy,
                      const std::vector<double>& xgrid,
                      const std::vector<double>& ygrid)
{
...
main()
{
  ...
  test_unary_F_compare(
      "fabs(x)", [](const FADF& x) { return fadbad::fabs(x); }, abs_val,
      abs_der, grid_sym);

  test_unary_F_compare(
      "erf(x)", [](const FADF& x) { return fadbad::erf(x); }, erf_val, erf_der,
      grid_sym);

  test_unary_F_compare(
      "xlog(x)  [assumed x*log(x)]", [](const FADF& x)
      { return fadbad::xlog(x); }, xlog_val, xlog_der, grid_pos);

  test_binary_F_compare(
      "max(x,y)", [](const FADF& x, const FADF& y)
      { return fadbad::max(x, y); }, max_val, dmax_dx, dmax_dy, grid2, grid2);

  test_binary_F_compare(
      "min(x,y)", [](const FADF& x, const FADF& y)
      { return fadbad::min(x, y); }, min_val, dmin_dx, dmin_dy, grid2, grid2);

  test_unary_F_compare(
      "max(x,1.0)", [](const FADF& x) { return fadbad::max(x, 1.0); }, max1_val,
      dmax1_dx, grid_sym);

  test_unary_F_compare(
      "min(x,1.0)", [](const FADF& x) { return fadbad::min(x, 1.0); }, min1_val,
      dmin1_dx, grid_sym);
  ...
  test_unary_F_compare(
      "comp2(x) = max(x,1)*min(x+0.5,1) + x*x", [](const FADF& x)
      { return fadbad::max(x, 1.0) * fadbad::min(x + 0.5, 1.0) + x * x; },
  ...
  test_binary_F_compare(
      ...
      {
        return fadbad::max(x, y) * (x + y) + fadbad::min(x, 1.0) * y * y +
               fadbad::erf(x - y);
      },
  ...
  test_unary_B_compare(
      "fabs(x)", [](const BADB& x) { return fadbad::fabs(x); }, abs_val,
      abs_der, grid_sym);

  test_unary_B_compare(
      "erf(x)", [](const BADB& x) { return fadbad::erf(x); }, erf_val, erf_der,
      grid_sym);

  test_unary_B_compare(
      "xlog(x)  [assumed x*log(x)]", [](const BADB& x)
      { return fadbad::xlog(x); }, xlog_val, xlog_der, grid_pos);

  test_binary_B_compare(
      "max(x,y)", [](const BADB& x, const BADB& y)
      { return fadbad::max(x, y); }, max_val, dmax_dx, dmax_dy, grid2, grid2);

  test_binary_B_compare(
      "min(x,y)", [](const BADB& x, const BADB& y)
      { return fadbad::min(x, y); }, min_val, dmin_dx, dmin_dy, grid2, grid2);

  test_unary_B_compare(
      "max(x,1.0)", [](const BADB& x) { return fadbad::max(x, 1.0); }, max1_val,
      dmax1_dx, grid_sym);

  test_unary_B_compare(
      "min(x,1.0)", [](const BADB& x) { return fadbad::min(x, 1.0); }, min1_val,
      dmin1_dx, grid_sym);
  ...
  test_unary_B_compare(
      "comp2(x) = max(x,1)*min(x+0.5,1) + x*x", [](const BADB& x)
      { return fadbad::max(x, 1.0) * fadbad::min(x + 0.5, 1.0) + x * x; },
      comp2_val, comp2_der, grid_comp_u2);
