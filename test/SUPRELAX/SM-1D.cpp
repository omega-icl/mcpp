...
////////////////////////////////////////////////////////////////////////
{
  ...
  try
  {
    // Calculate superposition relaxations w/ continuous piecewise linear
    // univariates
    PWLSM pwlmod(1);
    ...
    vector<PWLSV> PWLSVX{PWLSV(pwlmod, 0, I(XL, XU), NPWL)}, PWLSVF(NX);
    ...
    vector<PWCSV> PWCSVX{PWCSV(pwcmod, 0, I(XL, XU), NPWC)}, PWCSVF(NX);
    ...
    for (int iX = 0; iX < NPTS; iX++)
    {
      ...
      // Output relaxations
      resfile << setw(14) << DX[0] << setw(14) << DF[0] << setw(14)
              << PWLSVF[0].l() << setw(14) << PWLSVF[0].u() << setw(14)
              << PWLSVF[0].uval({{0, DX[0]}}) << setw(14)
              << PWLSVF[0].oval({{0, DX[0]}}) << setw(14) << PWCSVF[0].l()
              << setw(14) << PWCSVF[0].u() << setw(14)
              << PWCSVF[0].uval({{0, DX[0]}}) << setw(14)
              << PWCSVF[0].oval({{0, DX[0]}}) << endl;
    }
    resfile.close();
#endif
  }
