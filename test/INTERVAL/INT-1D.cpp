...
myfunc(const T& x)
{
  return fstep(4. - x) *
             (fstep(x - 3.) *
                  (exp(4. - x) + 3. - (fstep(3. - x) * (-sqr(x - 2.5) + 4.))) +
...
////////////////////////////////////////////////////////////////////////
{
  ...
  try
  {
    ...
    I IF = myfunc(IX);
#endif
    cout << endl << "DOMAIN: " << IX << endl << "BOUNDS: " << IF << endl;
    ...
    for (int iX = 0; iX < NX; iX++)
    {
      ...
      double DF = myfunc(DX);
#endif
      res << setw(14) << DX << setw(14) << DF << setw(14) << Op<I>::l(IF)
          << setw(14) << Op<I>::u(IF) << endl;
    }
#endif
  }
  ...
  {
    cerr << "Undefined errror" << endl << "Aborts." << endl;
    return -1;
  }
#endif
#ifdef SAVE_RESULTS
