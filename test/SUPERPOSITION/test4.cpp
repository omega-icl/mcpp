...
( const T&x, const T&y)
{
  std::vector<std::vector<double>> w1 (getWeights("w1.csv"));
  std::vector<std::vector<double>> w2 (getWeights("w2.csv"));
  std::vector<std::vector<double>> b1 (getWeights("b1.csv"));
  std::vector<std::vector<double>> b2 (getWeights("b2.csv"));  
  std::vector<T> V1;
  V1.push_back(ReLU(w1[0][0]*x+w1[0][1]*y+b1[0][0])*w2[0][0]+b2[0][0]);
  for(int i=1;i<47;i++){
    V1[0] += ReLU(w1[i][0]*x+w1[i][1]*y+b1[i][0])*w2[0][i];}
