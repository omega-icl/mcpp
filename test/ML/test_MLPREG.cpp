...
test_MLP0()
{
  ...
  std::cout << "y =";
  for (unsigned i = 0; i < NN.nout(); ++i) std::cout << " " << vout[i];
  std::cout << std::endl;
  ...
  for (unsigned i = 0; i < NN.nin(); ++i)
  {
    ...
    for (unsigned j = 0; j < NN.nout(); ++j)
      std::cout << " F: " << Fvout[j].d(i) << "  B: " << Bvin[i].d(j)
                << std::endl;
  }
  for (unsigned i = 0; i < NN.nwei(); ++i)
  {
    std::cout << "dydwei[" << i << "] =";
    for (unsigned j = 0; j < NN.nout(); ++j)
      std::cout << " F: " << Fvout[j].d(NN.nin() + i)
                << "  B: " << Bvwei[i].d(j) << std::endl;
  }
  ...
  DAG.eval(Xout_op, NN.nout(), Xout.data(), vout.data(), NN.nin(), Xin.data(),
           vin.data(), NN.nwei(), Xwei.data(), vwei.data());
  ...
  DAG.eval(Xout_op, NN.nout(), Xout.data(), Fvout.data(), NN.nin(), Xin.data(),
           Fvin.data(), NN.nwei(), Xwei.data(), Fvwei.data());

  // Evaluation of forward symbolic derivatives in real arithmetic
  const mc::FFVar* dXoutdXin =
      DAG.FAD(NN.nout(), Xout.data(), NN.nin(), Xin.data());
  ...
  DAG.eval(op_dXoutdXin, NN.nout() * NN.nin(), dXoutdXin, vdXoutdXin.data(),
           NN.nin(), Xin.data(), vin.data(), NN.nwei(), Xwei.data(),
           vwei.data());
  ...
  for (unsigned i = 0; i < NN.nin(); ++i)
  {
    std::cout << "dydin[" << i << "] =";
    for (unsigned j = 0; j < NN.nout(); ++j)
      std::cout << " F: " << Fvout[j].d(i)
                << "  S: " << vdXoutdXin[j * NN.nin() + i] << std::endl;
  }
  for (unsigned i = 0; i < NN.nwei(); ++i)
  {
    std::cout << "dydwei[" << i << "] =";
    for (unsigned j = 0; j < NN.nout(); ++j)
      std::cout << " F: " << Fvout[j].d(NN.nin() + i)
                << std::endl;  //"  B: " << Bvwei[i].d(j) << std::endl;
  }
