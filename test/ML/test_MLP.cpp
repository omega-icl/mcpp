...
std::vector<std::vector<std::vector<double>>> const MLPCOEF = {
    {{0.0, 0.2, 0.3}, {-3.0, 0.5, -0.2}, {0.0, 0.2, -0.4}, {0.0, -0.5, 0.0}},
    {{0.0, 1.0, -1.0, 1.0, 1.0}}};
...
main()
{
  try
  {
    ...
    std::vector<std::vector<double>> vDX{{1, 1}, {-1, -1}, {2, 2}, {-2, -2}},
        vDY;
    DAG.veval(SgY, Y, vDY, X, vDX);
    for (size_t i = 0; i < vDX.size(); ++i)
      std::cout << "Y(" << vDX[i][0] << "," << vDX[i][1] << ") = " << vDY[i][0]
                << std::endl;
    /*
    ...
    */
    ...
    PolEnv.options.BREAKPOINT_TYPE =
        mc::PolImg<I>::Options::BIN;  // CONT;//BIN;//SOS2;
    ...
    // mc::FFOp::TANH, mc::FFOp::EXP  }; PolEnv.options.SANDWICH_MAXCUT = 6;
    ...
    std::vector<PWCSV> PWCSVX{
        PWCSV(pwcmod, 0, I(X1L, X1U), OpMLP.options.PWCDIV),
        PWCSV(pwcmod, 1, I(X2L, X2U), OpMLP.options.PWCDIV)},
        PWCSVY;
    ...
    PWLSM pwlmod(NX);
    pwlmod.options.PROD_METH =
        PWLSM::Options::PARTIAL;  // PARTIAL;//FULL;//LOG;//NONE;
    ...
    std::vector<PWLSV> PWLSVX{
        PWLSV(pwlmod, 0, I(X1L, X1U), OpMLP.options.PWLINI),
        PWLSV(pwlmod, 1, I(X2L, X2U), OpMLP.options.PWLINI)},
        PWLSVY;
    ...
    for (int iX1 = 0; iX1 < NPTS; iX1++)
    {
      for (int iX2 = 0; iX2 < NPTS; iX2++)
      {
        ...
        resfile << std::setw(14) << DX[0] << std::setw(14) << DX[1]
                ...
                << std::setw(14) << PWCSVY[0].l() << std::setw(14)
                << PWCSVY[0].u() << std::setw(14)
                << PWCSVY[0].uval({{0, DX[0]}, {1, DX[1]}}) << std::setw(14)
                << PWCSVY[0].oval({{0, DX[0]}, {1, DX[1]}}) << std::endl;
      }
      resfile << std::endl;
    }
#endif
    ...
    std::map<unsigned, double> XREF{{0, -1.497271e-02},
                                    {1, 1.617562e+00}};  // Global max
    // std::map<unsigned,double> XREF{ {0,2.034074e-01}, {1,-1.637012e+00} }; //
    // Global min
    ...
    std::vector<MC> MCwk;
    for (unsigned i = 0; i < NREPEAT; ++i) DAG.eval(SgY, MCwk, Y, MCY, X, MCX);
    ...
    std::vector<MC> MCsubX{MC(XBND0[0], XREF[0]), MC(XBND0[1], XREF[1])},
        MCsubY;
    MCsubX[0].sub(2, 0);
    MCsubX[1].sub(2, 1);

    start = std::chrono::system_clock::now();
    for (unsigned i = 0; i < NREPEAT; ++i) DAG.eval(SgY, Y, MCsubY, X, MCsubX);
    walltime = std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::system_clock::now() - start);
    ...
    walltime = std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::system_clock::now() - start);
    std::cout << "Superposition PWL walltime: "
              << (walltime.count() * 1e-6) / (double)NREPEAT << std::endl;
#endif
  }
