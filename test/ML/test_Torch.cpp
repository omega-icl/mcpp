...
int main()
{
  try{
    ...
    double const X1L = -3., X1U = 3.;
    double const X2L = -3., X2U = 3.;
    std::map<unsigned,double> XREF{ {0,2.276872e-01}, {1,-1.626030e+00} }; // Global min
    ...
    double const X1L = -3., X1U = 3.;
    double const X2L = -3., X2U = 3.;
    std::map<unsigned,double> XREF{ {0,2.685182e-01}, {1,-1.648824e+00} }; // Global min
    ...
    std::vector<PWCSV> PWCSVX{ PWCSV( pwcmod, 0, I(X1L,X1U), pwcmod.options.MAX_SUBDIV ),
                               PWCSV( pwcmod, 1, I(X2L,X2U), pwcmod.options.MAX_SUBDIV ) }, PWCSVY;
    ...
    //std::vector<PWLSV> PWLSVX{ PWLSV( pwlmod, 0, I(X1L,X1U), pwlmod.options.MAX_SUBDIV ),
    //                           PWLSV( pwlmod, 1, I(X2L,X2U), pwlmod.options.MAX_SUBDIV ) }, PWLSVY;
    std::vector<PWLSV> PWLSVX{ PWLSV( pwlmod, 0, I(X1L,X1U), 1 ),
                               PWLSV( pwlmod, 1, I(X2L,X2U), 1 ) }, PWLSVY;
...
//          double const PWL2SVYu = PWL2SVY[0].uval({{0,DX[0]},{1,DX[1]}});
//          double const PWL2SVYo = PWL2SVY[0].oval({{0,DX[0]},{1,DX[1]}});

//          distmax_PWL2SVYu = max( distmax_PWL2SVYu, DY[0] - PWL2SVYu );
//          distmax_PWL2SVYo = max( distmax_PWL2SVYo, PWL2SVYo - DY[0] );

//          double const PWL4SVYu = PWL4SVY[0].uval({{0,DX[0]},{1,DX[1]}});
//          double const PWL4SVYo = PWL4SVY[0].oval({{0,DX[0]},{1,DX[1]}});

//          distmax_PWL4SVYu = max( distmax_PWL4SVYu, DY[0] - PWL4SVYu );
//          distmax_PWL4SVYo = max( distmax_PWL4SVYo, PWL4SVYo - DY[0] );

//          double const PWL8SVYu = PWL8SVY[0].uval({{0,DX[0]},{1,DX[1]}});
//          double const PWL8SVYo = PWL8SVY[0].oval({{0,DX[0]},{1,DX[1]}});

//          distmax_PWL8SVYu = max( distmax_PWL8SVYu, DY[0] - PWL8SVYu );
//          distmax_PWL8SVYo = max( distmax_PWL8SVYo, PWL8SVYo - DY[0] );
        }
      }
...
//                << std::setw(14) << max( distmax_PWL2SVYu,         distmax_PWL2SVYo )
//                << std::setw(14) << max( min_Y - PWL2SVY[0].l(),   PWL2SVY[0].u() - max_Y )
//                << std::setw(14) << max( distmax_PWL4SVYu,         distmax_PWL4SVYo )
//                << std::setw(14) << max( min_Y - PWL4SVY[0].l(),   PWL4SVY[0].u() - max_Y )
//                << std::setw(14) << max( distmax_PWL8SVYu,         distmax_PWL8SVYo )
//                << std::setw(14) << max( min_Y - PWL8SVY[0].l(),   PWL8SVY[0].u() - max_Y )
                << std::endl;
    }
#endif
    ...
    std::vector<MC> MCsubX{ MC( XBND[0], XREF[0] ), MC( XBND[1], XREF[1] ) }, MCsubY(1);
    MCsubX[0].sub( 2, 0 );
    MCsubX[1].sub( 2, 1 );
