...
main()
{
  ...
  PolEnv.add_semilinear_cuts(nullptr, NBKPT + 2, PX, Xk.data(), PF, Fk.data(),
                             mc::PolCut<I>::EQ, PolEnv.options.BREAKPOINT_TYPE);
  std::cout << PolEnv;
  ...
  try
  {
    ...
    for (int iX = 0; iX < NX; iX++)
    {
      ...
      res << std::setw(14) << X_val << std::setw(14) << Z_val << std::setw(14)
          << mc::Op<I>::l(PF.range()) << std::setw(14)
          << mc::Op<I>::u(PF.range()) << std::setw(14) << Zcv << std::setw(14)
          << Zcc << std::endl;
#endif
    }
  }
