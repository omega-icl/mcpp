...
{
  return x[0] * x[1] *
         (x[0] * (exp(x[0]) - exp(-x[0])) - x[1] * (exp(x[1]) - exp(-x[1])));
}
...
myfunc(const T* x)
{
  T f[3] = {-pow(x[0] * x[1] - 2., 2), -pow(x[0] * x[1], 2),
            -pow(x[0] * x[1] + 2., 2)};
...
myfunc(const T* x)
{
  return mc::Op<T>::max(x[0], x[1]) + mc::Op<T>::min(2 * x[0], x[1]) +
         fstep(x[0] - x[1]);
}
...
myfunc(const T* x)
{
  return 3. * pow(1 - x[0], 2) * exp(-pow(x[0], 2) - pow(x[1] + 1, 2)) -
         10. * (x[0] / 5. - pow(x[0], 3) - pow(x[1], 5)) *
             exp(-pow(x[0], 2) - pow(x[1], 2)) -
         (1. / 3.) * exp(-pow(x[0] + 1, 2) - pow(x[1], 2));
...
void
append_cut(GRBModel& model, const mc::PolCut<I>* pCut)
{
  GRBVar* VarSOS = 0;
  double* WeiSOS = 0;
  int TypSOS     = 0;
  if (pCut->type() == mc::PolCut<I>::SOS1 ||
      pCut->type() == mc::PolCut<I>::SOS2)
  {
    VarSOS = new GRBVar[pCut->nvar()];
    WeiSOS = new double[pCut->nvar()];
    TypSOS =
        (pCut->type() == mc::PolCut<I>::SOS1 ? GRB_SOS_TYPE1 : GRB_SOS_TYPE2);
  }
  ...
  switch (pCut->type())
  {
    ...
    case mc::PolCut<I>::EQ:
      DAGCuts.insert(
          std::make_pair(pCut, model.addConstr(lhs, GRB_EQUAL, pCut->rhs())));
      break;
    case mc::PolCut<I>::LE:
      DAGCuts.insert(std::make_pair(
          pCut, model.addConstr(lhs, GRB_LESS_EQUAL, pCut->rhs())));
      break;
    case mc::PolCut<I>::GE:
      DAGCuts.insert(std::make_pair(
          pCut, model.addConstr(lhs, GRB_GREATER_EQUAL, pCut->rhs())));
      break;
    default:
...
main()
{
  ...
  mc::FFDAGEXT<I> OpF;
  OpF.options.RELAX = {
      OpF.options.PWCS};  // SCM };//SB };//PWLS };//AUX }://MC };//INT };
  OpF.options.SBLIN             = 0;
  ...
  PolEnv.options.BREAKPOINT_RTOL = PolEnv.options.BREAKPOINT_ATOL = 0e0;
  // PolEnv.options.ALLOW_DISJ = { mc::FFOp::FSTEP, mc::FFOp::MAXF,
  // mc::FFOp::MINF, mc::FFOp::FABS }; PolEnv.options.ALLOW_NLIN      = {
  // mc::FFOp::TANH, mc::FFOp::EXP  };
  PolEnv.options.SANDWICH_MAXCUT = 6;

  std::vector<I> IX{I(X0L, X0U), I(X1L, X1U)};
  std::vector<mc::PolVar<I>> PolX{mc::PolVar<I>(&PolEnv, X[0], IX[0]),
                                  mc::PolVar<I>(&PolEnv, X[1], IX[1])},
      PolF;
  ...
  try
  {
    ...
    for (auto itv = PolEnv.Vars().begin(); itv != PolEnv.Vars().end(); ++itv)
    {
      GRBVar DAGVar = GRBmodel.addVar(mc::Op<I>::l(itv->second->range()),
                                      mc::Op<I>::u(itv->second->range()), 0.0,
                                      GRB_CONTINUOUS, itv->second->name());
      ...
      switch ((*itv)->id().first)
      {
        case mc::PolVar<I>::AUXCONT:
        {
          GRBVar DAGVar = GRBmodel.addVar(mc::Op<I>::l((*itv)->range()),
                                          mc::Op<I>::u((*itv)->range()), 0.0,
                                          GRB_CONTINUOUS, (*itv)->name());
    ...
    for (unsigned iX0 = 0; iX0 < NGRID; iX0++)
    {
      for (unsigned iX1 = 0; iX1 < NGRID; iX1++)
      {
        ...
        double X_val[2] = {X0L + iX0 * (X0U - X0L) / (NGRID - 1.),
                           X1L + iX1 * (X1U - X1L) / (NGRID - 1.)};
        double Z_val    = myfunc(X_val);

        GRBmodel.update();
        GRBConstr cut0val = GRBmodel.addConstr(GRBLinExpr(jtx0->second, 1.),
                                               GRB_EQUAL, X_val[0]);
        GRBConstr cut1val = GRBmodel.addConstr(GRBLinExpr(jtx1->second, 1.),
                                               GRB_EQUAL, X_val[1]);
...
#ifdef SAVE_RESULTS
        res << std::setw(14) << X_val[0] << std::setw(14) << X_val[1]
            << std::setw(14) << Z_val << std::setw(14)
            << mc::Op<I>::l(PolF[0].range()) << std::setw(14)
            << mc::Op<I>::u(PolF[0].range()) << std::setw(14) << Zcv
            << std::setw(14) << Zcc << std::endl;
#endif
      }
#ifdef SAVE_RESULTS
      res << std::endl;
