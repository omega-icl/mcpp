...
#undef USE_POLYMOD    // <-- specify whether to use a Chebyshev expansion before
                      // relaxation
#undef ADD_BREAKPOINT  // <-- specify whether to add breakpoints to the
                       // variables
...
myfunc(const T& x)
{
  return fstep(4 - x) * (fstep(x - 3) * (exp(4 - x) + 3 -
                                         (fstep(3 - x) * (-sqr(x - 2.5) + 4))) +
                         (fstep(3 - x) * (-sqr(x - 2.5) + 4)) - (2 * x - 7)) +
         (2 * x - 7);
}
...
const double XU = 1.;  // <-- range upper bound
bool
inter(double& x, const double& y, const double& z)
{
  x = std::max(y, z);
  return true;
}
...
const double XU = .5;   // <-- range upper bound
bool
hull(const double& y, const double& z)
{
  return 0.5 * (y + z);
}
template <class T>
...
main()
{
...
#ifdef USE_MIP
  PolEnv.options.ALLOW_DISJ = {mc::FFOp::FSTEP, mc::FFOp::MAXF, mc::FFOp::MINF,
                               mc::FFOp::FABS};
#else
...
#endif
  mc::FFVar F =
      PMenv.get_bound(PMF.coefmon().second, Xrbasis, &Y[1], PM::Options::NAIVE);
  std::cout << DAG;
  ...
  try
  {
    ...
    {
      // std::cout << itv->second->name() << ": " << itv->second->has_cuts() <<
      // std::endl;
    ...
    for (int iX = 0; iX < NX; iX++)
    {
      ...
      GRBmodel.update();
      GRBConstr cutval =
          GRBmodel.addConstr(GRBLinExpr(jtvar->second, 1.), GRB_EQUAL, X_val);
...
#ifdef SAVE_RESULTS
      res << std::setw(14) << X_val << std::setw(14) << Z_val << std::setw(14)
          << mc::Op<I>::l(F_Pol.range()) << std::setw(14)
          << mc::Op<I>::u(F_Pol.range()) << std::setw(14) << Zcv
