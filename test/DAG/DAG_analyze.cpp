...
int test_selim1()
{
  ...
  std::cout << std::endl << SPE.VarElim().size() << " VARIABLES CAN BE ELIMINATED" << std::endl;
  for( auto const& [var,ctr,aux] : SPE.VarElim() ){
    std::ostringstream ext; 
    ext << " OF " << *var << " USING " << *ctr << "=0";
    DAG.output( DAG.subgraph( 1, aux ), ext.str() );
  }
