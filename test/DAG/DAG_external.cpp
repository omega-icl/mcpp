...
class FFnorm2 : public FFOp
{
 public:
  ...
  // Functor
  FFVar&
  operator()(unsigned const nVar, FFVar const* pVar) const
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    if (idU == typeid(FFVar))
      ...
      return eval(nRes, static_cast<I*>(vRes), nVar,
                  static_cast<I const*>(vVar), mVar);
    ...
    throw std::runtime_error(
        "Error: No evaluation method for FFnorm2 with type" +
        std::string(idU.name()) + "\n");
  }

  template <typename T>
  void
  eval(unsigned const nRes, T* vRes, unsigned const nVar, T const* vVar,
  ...
  void
  eval(unsigned const nRes, FFVar* vRes, unsigned const nVar, FFVar const* vVar,
       unsigned const* mVar) const
  {
    assert(nRes == 1);
  ...
  void
  eval(unsigned const nRes, SLiftVar* vRes, unsigned const nVar,
  ...
  void
  deriv(unsigned const nRes, FFVar const* vRes, unsigned const nVar,
        FFVar const* vVar, FFVar** vDer) const
  {
    ...
    for (unsigned j = 0; j < nRes; ++j)
      for (unsigned i = 0; i < nVar; ++i) vDer[j][i] = vFRes[j].d(i);
  }
...
class FFnorm12 : public FFOp
{
 public:
  // Constructors
  FFnorm12() : FFOp(EXTERN) {}

  // Functor
  FFVar&
  operator()(unsigned const idep, unsigned const nVar, FFVar const* pVar) const
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "Error: No evaluation method for FFnorm12 with type" +
        std::string(idU.name()) + "\n");
  }
...
class FFxlog : public FFOp
{
 public:
  ...
  // Functor
  FFVar&
  operator()(FFVar const& Var) const
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "Error: No evaluation method for FFxlog with type" +
        std::string(idU.name()) + "\n");
  }
  ...
  template <typename T>
  void
  eval(unsigned const nRes, McCormick<T>* vRes, unsigned const nVar,
  ...
  template <typename T>
  void
  eval(unsigned const nRes, PolVar<T>* vRes, unsigned const nVar,
        ...
        unsigned const nVar, void* vVar) const
  {
    if (idU == typeid(PolVar<I>))
      return reval(nRes, static_cast<PolVar<I>*>(vRes), nVar,
                   static_cast<PolVar<I>*>(vVar));
  ...
  template <typename T>
  bool
  reval(unsigned const nRes, PolVar<T>* vRes, unsigned const nVar,
        PolVar<T>* vVar) const
  {
    assert(nVar == 1 && nRes == 1);
    ...
    img->add_semilinear_cuts(pop, vVar[0], Op<T>::l(vVar[0].range()),
                             Op<T>::u(vVar[0].range()), vRes[0], PolCut<T>::LE,
                             loc::xlog);
    img->add_sandwich_cuts(pop, vVar[0], Op<T>::l(vVar[0].range()),
                           Op<T>::u(vVar[0].range()), vRes[0],
                           Op<T>::l(vRes[0].range()), Op<T>::u(vRes[0].range()),
...
class FFdet : public FFOp
{
 public:
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "Error: No evaluation method for FFArrh with type" +
        std::string(idU.name()) + "\n");
  }
       ...
       unsigned const* mVar) const
  {
    ...
    switch (nDim)
    {
      ...
      case 1:
        ...
        break;
      default:
        vRes[0] = FFBase::det(nDim, vVar);
        break;
    }
  }
  void
  eval(unsigned const nRes, double* vRes, unsigned const nVar,
    ...
    switch (nDim)
    {
      ...
      default:
      {
        ...
        for (unsigned i = 0; i < nDim; ++i)
          for (unsigned j = 0; j < nDim; ++j) Amat(i, j) = vVar[i + j * nDim];
       ...
       unsigned const* mVar) const
  {
    ...
    switch (nVar)
    {
      ...
      case 1:
        ...
        break;
      case 2:
        vRes[0].update(FFDep::TYPE::Q);
        break;
      default:
        vRes[0].update(FFDep::TYPE::P);
        break;
    }
  }
...
struct FFDOptBase
{
  ...
  static unsigned
  read(unsigned const dim, std::string filename, bool const disp = false)
  {
    ...
    {
      if (disp)
        std::cout << "Atomic matrix #" << _A.size() << ":" << std::endl << Ai;
      _A.push_back(Ai);
      Ai.zeros();
    };
    ...
    while (std::getline(file, line))
    {
      ...
      Ai(i, 0) = first;
      if (disp)
        std::cout << "reading (" << i << ",0): " << Ai(i, 0) << std::endl;
      for (unsigned j = 1; j < dim; j++)
      {
        if (!(iss >> Ai(i, j)))
          throw std::runtime_error("Error: Could not read input file\n");
        if (disp)
          std::cout << "reading (" << i << "," << j << "): " << Ai(i, j)
                    << std::endl;
      }
      i++;
    }
...
class FFDOpt : public FFOp, public FFDOptBase
{
 public:
  // Constructors
  FFDOpt() : FFOp(EXTERN) {}
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "Error: No evaluation method for FFDOpt with type" +
        std::string(idU.name()) + "\n");
  }
       ...
       double const* vVar, unsigned const* mVar) const
  {
    ...
    for (unsigned i = 0; i < nVar; ++i) Amat += vVar[i] * _A[i];
    // std::cout << Amat;
    double sign = 0.;
...
class FFDOptGrad : public FFOp, public FFDOptBase
{
 public:
  // Constructors
  FFDOptGrad() : FFOp(EXTERN) {}
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "Error: No evaluation method for FFDOptGrad with type" +
        std::string(idU.name()) + "\n");
  }
             ...
             fadbad::F<FFVar> const* vVar, unsigned const* mVar) const
{
  ...
  FFDOptGrad DOptGrad;
  for (unsigned i = 0; i < nVar; ++i) vRes[0].setDepend(vVar[i]);
  for (unsigned j = 0; j < vRes[0].size(); ++j)
    for (unsigned i = 0; i < nVar; ++i)
      if (!i)
        vRes[0][j] = DOptGrad(0, nVar, vVarVal.data()) * vVar[0][j];
      else
        vRes[0][j] += DOptGrad(i, nVar, vVarVal.data()) * vVar[i][j];
}

inline void
FFDOpt::deriv(unsigned const nRes, FFVar const* vRes, unsigned const nVar,
              FFVar const* vVar, FFVar** vDer) const
{
  assert(nRes == 1 && nVar == _A.size() && _A.begin() != _A.end());
  std::cout << "FFDOpt::deriv: FFVar\n";
  FFDOptGrad DOptGrad;
  for (unsigned i = 0; i < nVar; ++i) vDer[0][i] = DOptGrad(i, nVar, vVar);
}

///////////////////////////////////////////////////////////////////////////////

class FFArrh : public FFOp
{
 public:
  // Constructors
  FFArrh() : FFOp(EXTERN) {}

  FFArrh(FFArrh const& Op) : FFOp(Op) {}
  ...
  // Define operation
  FFVar&
  operator()(FFVar const& Var, double& r, bool const copy = false) const
  {
...
class FFMonod : public FFOp
{
 public:
  ...
  // Functor
  FFVar&
  operator()(FFVar const& Var, FFVar const* Cst) const
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    if (idU == typeid(FFVar))
      ...
      return eval(nRes, static_cast<SLiftVar*>(vRes), nVar,
                  static_cast<SLiftVar const*>(vVar), mVar);
    throw std::runtime_error(
        "Error: No evaluation method for FFMonod with type" +
        std::string(idU.name()) + "\n");
  }
...
test_external2()
{
  ...
  for (unsigned i = 0; i < NX; ++i)
    for (unsigned j = 0; j < NX; ++j) F[i + j * NX] = pow(X[i], (int)j);
  mc::FFdet det;
  mc::FFVar G = det(NF, F);
  std::cout << DAG;
...
test_external6()
{
  ...
  for (unsigned i = 0; i < NS; ++i)
    std::cout << "dFdS[" << i << "] = " << ddFdS[i] << std::endl;
...
test_external7()
{
  ...
  // mc::FFVar F[2] = { Arrh( X, C1 ) + Arrh( X, C2 ), Arrh( X, C1 ) - Arrh( X,
  // C2 ) };
  mc::FFVar F[2] = {Arrh(X, C1, true) + Arrh(X, C2, true),
                    Arrh(X, C1, true) - Arrh(X, C2, true)};
  std::cout << DAG;
...
test_external9()
{
  ...
  // Evaluate polynomial
  std::map<mc::FFVar const*, double, mc::lt_FFVar> Xval{
      {&DAGX[0], 0.5}, {&DAGX[1], -0.5}, {&DAGX[2], 0.75}};
  ...
  std::cout << "\nSparse multivariate polynomial derivative value from DAG "
  ...
  std::cout << "\nSparse multivariate polynomial derivative value from DAG "
               "backward symbolic differentiation:\n";
  for (unsigned i = 0; i < NX; ++i)
  ...
  /*
    ...
    I IX[NX] = { I(-3.,3.), I(-3.,3.) };
    POLV polX[NX] = { POLV( &IMG, X[0], IX[0] ), POLV( &IMG, X[1], IX[1] ) },
    polF; DAG.eval( F_op, 1, &F, &polF, NX, X, polX ); IMG.generate_cuts( 1,
    &polF ); std::cout << "F =" << IMG << std::endl;
  */
...
test_external9_1()
{
  ...
  // std::vector<mc::FFVar> F{ Sum( X, 1. ), Sum( X, {0.5,2.,3.}, 1. ) -
  // 0.5*X[0] }; std::vector<mc::FFVar> F{ exp( Sum( X, 1. ) ), Sum( X,
  // {0.5,2.,3.}, 1. ) + 0.5*X[0] };
  std::cout << DAG;
  ...
  std::cout << "\nLinear subexpression constraint propagation from DAG: "
            << flag << " passes\n";
  ...
  mc::PolImg<I> IMG;
  IMG.options.AGGREG_LQ = true;
  std::vector<POLV> PX{POLV(&IMG, X[0], IX[0]), POLV(&IMG, X[1], IX[1]),
                       POLV(&IMG, X[2], IX[2])},
      PF;
...
test_external10()
{
  ...
  for (size_t i = 0; i < vF.nFun(); ++i) F.push_back(OpVect(i, &vF));
  ...
  for (auto const& dFi : dF) std::cout << dFi << std::endl;
  ...
  for (auto const& dFdXi : ddFdX) std::cout << dFdXi << std::endl;
...
test_external12()
{
  ...
  std::vector<mc::FFVar> F{Expr(0, X, &DAGY0, 1),
                           Expr(1, X, &DAGY0, 1)};  // with DAG copy
  ...
  std::cout << "\nFunction value: [ ";
  for (auto const& Fi : dF) std::cout << Fi << " ";
  std::cout << "]" << std::endl;
  ...
  std::cout << "\nFunction derivative: [ ";
  for (auto const& dFdXi : ddFdX) std::cout << dFdXi << " ";
  std::cout << "]" << std::endl;
...
test_external13()
{
  ...
  std::vector<MC> MCX{MC(IX[0], mc::Op<I>::mid(IX[0])).sub(NX, 0),
                      ...
                      MC(IX[2], mc::Op<I>::mid(IX[2])).sub(NX, 2)},
      MCF;
  DAG.eval(opF, F, MCF, X, MCX);
  ...
  IMG.options.ALLOW_DISJ = {mc::FFOp::FABS, mc::FFOp::MAXF};
  IMG.options.ALLOW_NLIN = {mc::FFOp::TANH, mc::FFOp::EXP};
  std::vector<POLV> polX{POLV(&IMG, X[0], IX[0]), POLV(&IMG, X[1], IX[1]),
                         POLV(&IMG, X[2], IX[2])},
      polF;
...
test_external14()
{
  ...
  std::cout << "X[0] = "
            << I(-mc::Op<I>::l(IX[1]) / 2. +
                     sqrt(mc::sqr(mc::Op<I>::l(IX[1]) / 2.) - 4),
                 -mc::Op<I>::u(IX[1]) / 2. +
                     sqrt(mc::sqr(mc::Op<I>::u(IX[1]) / 2.) - 4))
            << std::endl;
  ...
  IX    = {I(-0.8, -0.3), I(6., 9.)};
  int flag =
      DAG.reval(opY, IWKF, Y, IF, X, IX, DAGY0.options.CPINF * I(-1, 1), 10);
...
std::vector<double>
addition(std::vector<double> const& x)
{
  return {x[0] + x[1]};
}
...
test_external15()
{
  ...
  std::function<std::vector<I>(std::vector<I> const&)> IEval(
      [](std::vector<I> x) -> std::vector<I> { return {x[0] * x[1]}; });
  // std::function<std::vector<double>( std::vector<double> const& )> DEval(
  // addition );
...
test_slift_external0()
{
  ...
  // ) }; mc::FFVar F[NF] = { norm2( NX, X ), norm12( 0, NX, X ), norm12( 1, NX,
  // X ) };
