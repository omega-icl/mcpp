...
test_fadiff3()
{
  ...
  // Forward AD recursive
  const mc::FFVar* dFdX_recur =
      DAG.FAD(NF, F, 1, &X[0], 1, &X[1], 1, &X[2], true);
  DAG.output(DAG.subgraph(NX * NF, dFdX_recur), " dFdX (recursive)");
...
test_fadiff4()
{
  ...
  // matrix
  mc::FFVar A[NX * NX] = {sqr(X[0]), 0.5 * X[0] * X[1], 0.5 * X[0] * X[1],
                          sqr(X[1])};
  std::cout << DAG;
...
test_fadiff5()
{
  ...
  for (auto& Sk : S) Sk.set(&DAG);

  std::vector<mc::FFVar> Si(NS), C;
  for (size_t i = 0; i < NS; i++)
  {
    for (size_t j = 0; j < i; j++) Si[j] = S[i * (i - 1) / 2 + j];
    Si[i] = 0.;
    for (size_t j = i + 1; j < NS; j++) Si[j] = S[j * (j - 1) / 2 + i];

    for (size_t j = 0; j < i; j++) C.push_back(Si[j] - E[i] - E[j] + 1);
  }

  std::cout << "Number of linear functions: " << C.size() << std::endl;
  std::cout << "DAG variables/operations: " << DAG.Vars().size() << "/"
            << DAG.Ops().size() << std::endl;
  ...
  for (auto const& Ck : C)
  {
    ...
    for (size_t k = 0; k < std::get<0>(Ckgrad); ++k)
    {
      ...
      // std::cout << "  _Avar[" << _iAfun.back() << "," << _jAvar.back() << "]
      // = " << _Avar.back() << std::endl;
    }
  ...
  std::cout << "Nonzero elements in Jacobian matrix: " << row_Cgrad.size()
            << std::endl;
...
test_fadiff_directional()
{
  ...
  const mc::FFVar* dFdXxD_recur =
      DAG.DFAD(2, &F[0], 1, &X[0], &D[0], 1, &X[1], &D[1]);
  DAG.output(DAG.subgraph(1, &dFdXxD_recur[0]), " dF0dX·D (recursive)");
  DAG.output(DAG.subgraph(1, &dFdXxD_recur[1]), " dF1dX·D (recursive)");
  std::ofstream o_dFdXxD_recur("fadiff_directional_recur.dot",
                               std::ios_base::out);
...
test_fadiff_directional2()
{
  ...
  std::vector<mc::FFVar> RHS{// Right-hand side function
                             ...
                             -Qin / V * CB + nu * R * pow(CA, alpha), Qin};

  std::vector<mc::FFVar> XQin(NX);  // State sensitivities
  for (auto& XQini : XQin) XQini.set(&DAG);

  mc::FFVar* dRHSdQin =
      DAG.DFAD(NX, RHS.data(), NX, X.data(), XQin.data(), 1, P.data(), &One);
...
test_gradient_sparse()
{
  ...
  std::vector<mc::FFVar> X(NX);
  for (auto& Xi : X) Xi.set(&DAG);
  std::vector<mc::FFVar> F{0.5 * X[0], X[0] * X[1],
                           sqrt(X[1]) * exp(X[1]) * X[1] + 1.};
  std::cout << DAG;
  ...
  std::cout << "\nNon-zero Jacobian elements (DFAD, non-recursive): " << NE
            << std::endl;
  for (unsigned ie = 0; ie < NE; ie++)
    std::cout << "(" << std::get<0>(dFdXxD_FAD)[ie] << ","
              << std::get<1>(dFdXxD_FAD)[ie] << ") "
  ...
  std::cout << "\nNon-zero Jacobian elements (FAD, non-recursive): " << NE
            << std::endl;
  for (unsigned ie = 0; ie < NE; ie++)
    std::cout << "(" << std::get<0>(dFdX_FAD)[ie] << ","
              << std::get<1>(dFdX_FAD)[ie] << ") " << std::get<2>(dFdX_FAD)[ie]
              << std::endl;
  ...
  std::cout << "\nNon-zero Jacobian elements (BAD, non-recursive): " << NE
            << std::endl;
  for (unsigned ie = 0; ie < NE; ie++)
    std::cout << "(" << std::get<0>(dFdX_BAD)[ie] << ","
              << std::get<1>(dFdX_BAD)[ie] << ") " << std::get<2>(dFdX_BAD)[ie]
              << std::endl;
...
test_hessian_sparse()
{
  ...
  std::cout << "\nNumber of non-zero elements in Jacobian matrix: " << NE
            << std::endl;
  for (unsigned ie = 0; ie < NE; ie++)
  ...
  std::cout << "\nNumber of non-zero elements in Hessian matrix: " << NE
            << std::endl;
  for (unsigned ie = 0; ie < NE; ie++)
    std::cout << "(" << std::get<0>(d2FdX2)[ie] << ","
              << std::get<1>(d2FdX2)[ie] << ") " << std::get<2>(d2FdX2)[ie]
              << std::endl;
...
test_tadiff3()
{
  ...
  std::vector<mc::FFVar> RHS{P[0] * X[0] * (1. - X[1]),
                             P[0] * X[1] * (X[0] - 1.)};
  std::cout << DAG;
  ...
  DAG.eval(NX * (NTE + 1), RHS_TAD.data(), dRHS_TAD, NX, X.data(), dX, NP,
           P.data(), dP);  //, 1, &T, &dT );
