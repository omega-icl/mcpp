...
test_eval1()
{
  ...
  cputime = -mc::cpuclock();
  for (unsigned i = 0; i < NREP; i++) DAG.eval(F_op, WK, F, dF, X, dX);
  cputime += mc::cpuclock();
  std::cout << "\nDAG evaluation, w/ preallocation, w/o variadic template: "
  ...
  cputime = -mc::cpuclock();
  for (unsigned i = 0; i < NREP; i++)
    DAG.eval(F_op, WK, NF, &F[0], &dF[0], 1, &X[0], &dX[0], 1, &X[1], &dX[1], 1,
             &X[2], &dX[2], 1, &X[3], &dX[3]);
  cputime += mc::cpuclock();
  std::cout << "\nDAG evaluation, w/ preallocation, w/ variadic template: "
            << (cputime /= NREP) << " CPU-sec\n";
  for (unsigned i = 0; i < NF; i++)
    std::cout << "F[" << i << "] = " << dF[i] << std::endl;
...
test_eval2a()
{
  ...
  std::vector<SCV> SCX{SCV(&modSCM, 0, IX[0]), SCV(&modSCM, 1, IX[1]),
                       SCV(&modSCM, 2, IX[2]), SCV(&modSCM, 3, IX[3])},
      SCF(1);
...
test_eval2()
{
  ...
  for (unsigned i = 0; i < NX; i++)
    std::cout << "X[" << i << "] = " << IX[i] << std::endl;
  cputime = -mc::cpuclock();
  for (unsigned i = 0; i < NREP; i++) DAG.eval(F_op, IWK, F, IF, X, IX);
  cputime += mc::cpuclock();
  std::cout
      << "\nDAG interval evaluation, w/ preallocation, w/o variadic template: "
      << (cputime /= NREP) << " CPU-sec\n";
  for (unsigned i = 0; i < NF; i++)
  ...
  for (unsigned NTE = NTEMIN; NTE <= NTEMAX; NTE++)
  {
    ...
    cputime += mc::cpuclock();
    std::cout << "\nDAG " << NTE
              << "th-order sparse Chebyshev model evaluation, w/ compilation: "
              << (cputime /= NREP) << " CPU-sec\n";
    for (unsigned i = 0; i < NF; i++)
      std::cout << "F[" << i << "] = " << SCF[i].P().B() << " +/- "
                << SCF[i].R() << std::endl;
  }
  ...
  for (unsigned NTE = NTEMIN; NTE <= NTEMAX; NTE++)
  {
    ...
    std::vector<SCV> SCX{SCV(&modSCM, 0, IX[0]), SCV(&modSCM, 1, IX[1]),
        ...
        SCF;
    cputime = -mc::cpuclock();
    for (unsigned i = 0; i < NREP; i++) DAG.eval(F_op, SCWK, F, SCF, X, SCX);
    cputime += mc::cpuclock();
    std::cout << "\nDAG " << NTE
              << "th-order sparse Chebyshev model evaluation, w/ "
                 "preallocation, w/o variadic template: "
              << (cputime /= NREP) << " CPU-sec\n";
    for (unsigned i = 0; i < NF; i++)
      std::cout << "F[" << i << "] = " << SCF[i].P().B() << " +/- "
                << SCF[i].R() << std::endl;
  ...
  for (unsigned NTE = NTEMIN; NTE <= NTEMAX; NTE++)
  {
    ...
    std::vector<CV> CX{CV(&modCM, 0, IX[0]), CV(&modCM, 1, IX[1]),
                       CV(&modCM, 2, IX[2]), CV(&modCM, 3, IX[3])},
        CF(NF);
    cputime = -mc::cpuclock();
    for (unsigned i = 0; i < NREP; i++)
    {
    ...
    cputime += mc::cpuclock();
    std::cout << "\nDAG " << NTE
              << "th-order dense Chebyshev model evaluation, w/ compilation: "
              << (cputime /= NREP) << " CPU-sec\n";
    for (unsigned i = 0; i < NF; i++)
      std::cout << "F[" << i << "] = " << CF[i].P().B() << " +/- " << CF[i].R()
                << std::endl;
  }
  ...
  for (unsigned NTE = NTEMIN; NTE <= NTEMAX; NTE++)
  {
    ...
    std::vector<CV> CX{CV(&modCM, 0, IX[0]), CV(&modCM, 1, IX[1]),
        ...
        CF;
    cputime = -mc::cpuclock();
    for (unsigned i = 0; i < NREP; i++) DAG.eval(F_op, CWK, F, CF, X, CX);
    cputime += mc::cpuclock();
    std::cout << "\nDAG " << NTE
              << "th-order dense Chebyshev model evaluation, w/ preallocation, "
                 "w/o variadic template: "
              << (cputime /= NREP) << " CPU-sec\n";
    for (unsigned i = 0; i < NF; i++)
      std::cout << "F[" << i << "] = " << CF[i].P().B() << " +/- " << CF[i].R()
                << std::endl;
...
test_eval3()
{
  ...
  // Compute bounds on residual sum-of-squares by specializing the DAG at 3
  ...
  for (size_t k = 0; k < xdat.size(); ++k)
  {
    ...
    X.set(xdat[k]);
    Y.set(ydat[k]);
    // Evaluate the current residual - the final 'true' argument is to append
    // the result to IF instead of overwriting IF
...
test_reval1()
{
  ...
  for (unsigned maxpass = 1; maxpass <= 10; ++maxpass)
  {
    ...
    cputime += mc::cpuclock();
    std::cout << "\nDAG interval evaluation w/ " << flag
              << " forward/backward passes: " << (cputime /= NREP)
              << " CPU-sec\n";
    std::vector<I> IX{I(-0.8, -0.3), I(6., 9.)};
...
test_reval2()
{
  ...
  try
  {
    I IX[NX] = {I(-1, 1), I(-1, 1), I(0, 4)}, IF[NF];
...
test_reval3()
{
  ...
  // Example constraint propagation from Wikipedia
  // (https://en.wikipedia.org/wiki/Interval_propagation)
  const unsigned NX = 7, NF = 4;
  mc::FFGraph DAG;
  mc::FFVar X[NX], &E = X[0], &J = X[1], &U1 = X[2], &U2 = X[3], &P = X[4],
                   &R1 = X[5], &R2 = X[6];
  for (unsigned int i = 0; i < NX; i++) X[i].set(&DAG);
  mc::FFVar F[NF] = {P - E * J, U1 - R1 * J, U2 - R2 * J, E - U1 - U2};
  auto SGF        = DAG.subgraph(NF, F);
  DAG.output(SGF);
  ...
  try
  {
    I IX[NX] = {I(23, 26),   I(4, 8),   I(10, 11), I(14, 17),
                I(124, 130), I(0, INF), I(0, INF)},
      IF[NF];
    DAG.eval(SGF, IWKF, NF, F, IF, NX, X, IX);
    std::cout << "\nDAG interval evaluation w/ forward pass only:\n";
    for (unsigned i = 0; i < NX; i++)
...
test_reval4()
{
  ...
  */
  ...
  double r[nc * nc] = {0.0,     0.7411,  0.9645, -1.0250, 0.0,
                       -1.4350, -0.9645, 2.7470, 0.0};
  double s[nc * nc] = {0.0,    -477.00,   -903.1024, 72.78, 0.0,
                       768.20, -140.9995, -1419.0,   0.0};
  /*
 ...
 */
  ...
  */
  mc::FFVar B, L[ns + 1], p[nc * ns], Lambda[nc * nc * ns],
      sum_xLambda[nc * ns], G[nc * ns], sum;
  /*
        param B := F[N_F] - D;
  */
  B = FF[pf] - D;
  ...
  */
  for (unsigned i1 = 0; i1 < nc; ++i1)
    for (unsigned i2 = 0; i2 < nc; ++i2)
      for (unsigned j = 0; j < ns; ++j)
        Lambda[nc * ns * i1 + ns * i2 + j] =
            exp(r[nc * i1 + i2] + s[nc * i1 + i2] / Te[j]);
  /*
        var sum_xLambda{i in 1..C, j in 1..N} = sum{i1 in 1..C}
     (x[i1,j]*Lambda[i,i1,j]);
  */
  for (unsigned i = 0; i < nc; ++i)
    for (unsigned j = 0; j < ns; ++j)
    {
  ...
  /*
    ...
    var gamma{i in 1..C, j in 1..N} = exp( -log(sum_xLambda[i,j]) + 1.0
                - (sum{i2 in 1..C}
    (x[i2,j]*Lambda[i2,i,j]*rcp_sum_xLambda[i2,j])) );
  */
  for (unsigned i = 0; i < nc; ++i)
    for (unsigned j = 0; j < ns; ++j)
    {
      sum = 0.;
      for (unsigned i2 = 0; i2 < nc; ++i2)
        sum += X[i2 * ns + j] * Lambda[nc * ns * i2 + ns * i + j] /
               sum_xLambda[i2 * ns + j];
  ...
  */
  for (unsigned i = 0; i < nc; ++i)
  {
    F[ieq++] = D * (K[i * ns + 0] * X[i * ns + 0]) + B * X[i * ns + (ns - 1)] -
               f[i * ns + pf];
  }
  /*
        NOTE THE UNUSUAL FORMULATION
        M_eq{j in 1..N-1, i in 1..C}:
        L[j]*x[i,j] + sum{i1 in j+1..N} f[i,i1] - B*x[i,N] -
     V*(K[i,j+1]*x[i,j+1]) = 0.0;
  */
  for (unsigned j = 0; j < ns - 1; ++j)
  {
    for (unsigned i = 0; i < nc; ++i)
    {
      ...
      F[ieq++] = L[j] * X[i * ns + j] + sum - B * X[i * ns + (ns - 1)] -
                 V * (K[i * ns + j + 1] * X[i * ns + j + 1]);
    }
  }
  /*
        SUMMATION EQUATIONS
  ...
  */
  ...
  {
    sum = -1.;
    for (unsigned i = 0; i < nc; ++i) sum += X[i * ns + j];
    F[ieq++] = sum;
  }
  ...
  */
  ...
  //  ID = I( 0.44, 0.47 );
  //  for( unsigned j=0; j<ns; ++j ){
  //    IX[j]  = I( 0.0001, 0.9999 ); IX[ns+j] = I( 0.0001, 0.9999 ); IX[2*ns+j]
  //    = I( 0.0001, 0.9999 ); IK[j]  = I( 0.97 , 40.52  );  IK[ns+j] = I(
  //    0.2445, 1.317  ); IK[2*ns+j] = I( 0.2745, 1.975  ); ITe[j] = I( 350.,
  //    351.  );//I( 336.3, 383.4  );
  //  }

  const I Ip_3stg[NP] = {I(4.51882712332260e-01, 4.53400900000000e-01),
  ...
  */
  ...
  vIp.assign(Ip_3stg, Ip_3stg + NP);
  vIf.assign(NF, I(0.));
  std::cout << "\nDAG interval evaluation using forward/backward passes:";
  int flag = 0;
  try
  {
    flag = DAG.reval(SGF, IWKF, NF, F, vIf.data(), NP, P, vIp.data(), IINF);
  }
  catch (...)
  {
    std::cout << "Failure\n";
...
test_reval5()
{
  ...
  // Example constraint propagation from MINLPLIB ex6_1_1
  // (https://www.minlplib.org/ex6_1_1.html)
  mc::FFGraph DAG;
...
test_veval1()
{
  ...
  for (DAG.options.MAXTHREAD = 1; DAG.options.MAXTHREAD <= 12;
       ++DAG.options.MAXTHREAD)
  {
    ...
    std::cout << "\nvectorized DAG evaluation on " << DAG.options.MAXTHREAD
              << " threads: " << walltime.count() * 1e-6 << " sec\n";
    ...
    std::cout << "\nvectorized DAG evaluation on " << DAG.options.MAXTHREAD
              << " threads: " << walltime.count() * 1e-6 << " sec\n";
    starttime = std::chrono::system_clock::now();
    std::cout << "v_dF: " << v_dF.front()[0] << "  " << v_dF.front()[1]
              << std::endl;
    std::cout << "v_dF: " << v_dF.back()[0] << "  " << v_dF.back()[1]
              << std::endl;
  ...
  //   std::cout << std::endl;
  // }
  // for( unsigned i=0; i<NF; i++ ) std::cout << "F[" << i << "] = " << dF[i] <<
  // std::endl;
