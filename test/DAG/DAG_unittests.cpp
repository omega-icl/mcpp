...
inline bool near( double a, double b, double tol = TOL )
{ return std::fabs(a-b) <= tol * (1. + std::fabs(b)); }

// Run a single named test.  fn should return a Result.
inline void run( std::string const& name, std::function<Result()> fn )
{
  ++total;
  Result r;
  try          { r = fn(); }
  catch(std::exception const& e){ r = {false, std::string("exception: ")+e.what()}; }
  catch(...)   { r = {false, "unknown exception"}; }

  if(r.ok){ ++passed; std::printf("  PASS  %s\n", name.c_str()); }
...
#define PASS   return test::Result{true,""}
#define FAIL(msg) return test::Result{false,(msg)}
#define REQUIRE(cond) \
  if(!(cond)) return test::Result{false, "requirement failed: " #cond}
#define REQUIRE_NEAR(a,b) \
  if(!test::near((a),(b))) { \
    std::ostringstream _s; _s<<#a<<"="<<(a)<<" != "<<#b<<"="<<(b); \
    return test::Result{false,_s.str()}; }
#define REQUIRE_THROWS(expr) \
  { bool _threw = false; \
    try{ (void)(expr); } catch(...){ _threw=true; } \
    if(!_threw) return test::Result{false,"expected exception from: " #expr}; }
...
void section_construction()
{
  ...
  TEST("clear_resets_dag") {
    mc::FFGraph dag;
    { mc::FFVar x(&dag), y(&dag); mc::FFVar f = x * y + mc::exp(x); }
    REQUIRE( dag.nvar() > 0 );
    dag.clear();
    REQUIRE( dag.nvar() == 0 );
    REQUIRE( dag.naux() == 0 );
...
void section_subgraph()
{
  ...
  TEST("subgraph_excludes_irrelevant_var") {
    ...
    bool z_found = false;
    for( auto const& op : sg.l_op )
      if( op->type == mc::FFOp::VAR &&
          op->varout[0]->id() == z.id() ) z_found = true;
    REQUIRE( !z_found );
    PASS;
  });
}
...
void section_eval()
{
  ...
  auto eval1 = [](mc::FFGraph& dag, mc::FFVar const& f,
                  mc::FFVar const& x, double xv) -> double {
    double result;
    dag.eval( 1, &f, &result, 1, &x, &xv );
    return result;
  };

  auto eval2 = [](mc::FFGraph& dag, mc::FFVar const& f,
                  mc::FFVar const& x, double xv,
                  mc::FFVar const& y, double yv) -> double {
    double result;
    mc::FFVar vars[2] = {x, y};
    double vals[2] = {xv, yv};
  ...
  // 3.7 Common subexpression: p=x*y used in two dependents evaluated simultaneously
...
void section_fad()
{
  ...
  auto evalD = [](mc::FFGraph& dag, mc::FFVar const& f,
                  mc::FFVar const& x, double xv) -> double {
    double res;
  ...
  TEST("fad_two_by_two_jacobian") {
    ...
    mc::FFVar F[2] = { x*y, x+y };
    mc::FFVar XY[2] = { x, y };
    auto J = dag.FAD( 2, F, 2, XY ); // 4 entries, row-wise: J[i*2+j] = dF[i]/dX[j]
    REQUIRE( J != nullptr );
    double vals[2] = {2., 3.};
...
void section_compose()
{
  ...
  // 6.2 compose with identity substitution: y→x, f(y)=sin(y) → sin(x) at pi/2 = 1
...
void section_edge()
{
  ...
  // 7.5 Deep expression tree does not overflow stack during subgraph construction
